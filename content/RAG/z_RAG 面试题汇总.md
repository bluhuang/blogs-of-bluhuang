...
**① 知识截止**——训练数据有截止日期，昨天发生的事它不知道。若询问"2026年3月发布的某框架有什么特性"，它要么瞎编要么说不知道。

**② 私有数据无法触达**——公司的内部文档、客户数据、业务规则，这些 LLM 从未见过，直接询问则会胡编。
...
**技术核心点**：RAG 并非替代 LLM，而是给 LLM 补充外部知识。LLM 负责理解和生成，RAG 负责提供事实依据。

### Q2: RAG的完整链路是怎样的？

大致的流程：从用户提问到最终回答的完整链路如下。
...
**技术要点**：不要只罗列七个步骤，要说清楚每一步的**关键决策点**。关键在于解释"为什么选这个方案而不选另一个"，例如：选 Milvus 而非 FAISS 的考量是什么，检索延迟要求是多少，为什么 Top-K 设 5 而不是 10。

### Q3: 传统RAG的三大痛点是什么？

常见问题：RAG 系统检索到了正确信息但生成的回答仍然拼凑感强，或者某些类型的问题回答不好。

#### 一个例子说清楚 RAG 撞墙在哪

假设有一个公司内部知识库，全是项目文档、技术方案、会议纪要。有人问：

**"我们公司所有项目的技术栈趋势是什么？"**

传统 RAG 会把问题转成向量，去向量库里找最相似的文本块。找到的是一堆零散的片段："项目 A 用了 Spring Boot""项目 B 迁移到了 Go""项目 C 在试 Rust"……然后把这些片段丢给 LLM 拼一个回答。

拼出来的是一堆事实的堆砌，并非"趋势"。因为没有一个视角能把所有项目的全貌看清楚，LLM 拿到的就是碎片，它再怎么聪明也只能拼碎片。
...
例如，"张三和李四在哪个项目上有合作"，这个信息可能分散在三个文档里——文档 1 提到张三负责项目 X，文档 2 提到李四参与了项目 X，文档 3 提到项目 X 的具体内容。传统 RAG 最多能捞到其中一个，很难同时把三个都捞出来并关联上。
...
混合检索、Rerank、Query 改写，都是在"找更好的文本块"。但有些问题需要的不是更好的文本块，而是**实体之间的关系**和**全局的结构性理解**。
...
常见问题：向量检索和关键词检索有什么区别？Embedding 的原理是什么？为什么语义相似的文本向量距离近？
...
**技术加分**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），说明有实际调优经验。
...
**技术核心点**：能说清楚纯向量检索的三个问题、关键词检索的盲区（同义词无解），以及混合检索如何互补，合并策略用 RRF。向量检索并非"什么都比关键词检索好"，两者是互补关系。正确的做法不是二选一，而是混合检索。
...
**技术核心点**：不要只说"用了 Prompt 约束"，要说出用了几种策略组合，以及效果如何。例如"Prompt 约束 + 输出自校验 + 温度调低，幻觉率从 30% 降到了 12%"。
...
常见问题：Agentic RAG 是什么？它和普通 RAG 有什么区别？
...
![](/images/RAG/6f8b11acef17e7f0.png)
...
**技术核心点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。能说出这个判断，说明有工程判断力。
...
不完全是。加一个知识图谱做辅助检索确实能提升效果，但 GraphRAG 的核心创新不在"有图谱"，而在**社区摘要的预计算**。如果只是建个图谱做实体检索，没有社区层级和预计算摘要，全局性问题还是回答不好。
