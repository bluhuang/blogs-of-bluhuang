...
**答法要点**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。重点不是"我用了 Milvus"，而是"我为什么选 Milvus 不选 FAISS，检索延迟要求多少，为什么 Top-K 设 5 不是 10"。

### Q3: 传统RAG的三大痛点是什么？

RAG 系统可能会遇到检索到了但答不对的情况，某些类型的问题答不好；或者 RAG 检索到了正确信息，但生成的回答还是拼凑感很强。
...
比如"开发者 A 和开发者 B 在哪个项目上有合作"，这个信息可能分散在三个文档里——文档 1 提到开发者 A 负责项目 X，文档 2 提到开发者 B 参与了项目 X，文档 3 提到项目 X 的具体内容。传统 RAG 最多能捞到其中一个，很难同时把三个都捞出来并关联上。
...
**加分点**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），说明你真实践过。
...
你已经用了混合检索，为什么还要 Rerank？检索结果不够好吗？
...
**核心点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。能说出这个判断，就体现了工程判断力。
