...
**答法**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。关键不是"我用了 Milvus"，而是"我为什么选 Milvus 不选 FAISS，检索延迟要求多少，为什么 Top-K 设 5 不是 10"。
...
比如"员工A和员工B在哪个项目上有合作"，这个信息可能分散在三个文档里——文档 1 提到员工A负责项目 X，文档 2 提到员工B参与了项目 X，文档 3 提到项目 X 的具体内容。传统 RAG 最多能捞到其中一个，很难同时把三个都捞出来并关联上。
...
"某公司收购了另一公司"在文档 1 里，"另一公司和第三家公司有合作"在文档 2 里。那某公司和第三家公司之间有没有间接关系？传统 RAG 答不了——因为每个 chunk 是独立 embedding 的，chunk 之间没有"关系"这个概念。
...
**进阶点**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），表明你确实调过。
...
**核心点**：能说清楚纯向量检索的三个问题、关键词检索的盲区（同义词无解），以及混合检索如何互补，合并策略用 RRF。向量检索并不是「什么都比关键词检索好」，两者是互补关系。正确的做法不是二选一，而是混合检索。
...
**答法**："检索是粗筛快捞，Rerank 是精排提准。检索用 Bi-Encoder 快但粗，Rerank 用 Cross-Encoder 慢但准。先用检索从百万级捞 Top-20，再用 Rerank 精排取 Top-5，这是生产环境的标配流程。"
...
**核心点**：不要只说"用了 Prompt 约束"，要说出你用了几种策略组合，以及效果如何。比如"Prompt 约束 + 输出自校验 + 温度调低，幻觉率从 30% 降到了 12%"。
...
**核心点**：能说出检索侧和生成侧的指标，并知道如何用 Ragas 等框架自动化评估。
...
**核心点**：能说清楚 chunk 大小的权衡（大→信息稀释，小→上下文丢失），以及 overlap 的作用。最好能举出你实际调参的经历，比如"chunk_size 从 1000 降到 500，召回率提升了 15%"。
...
**核心点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。能说出这个判断，说明你有工程判断力。
