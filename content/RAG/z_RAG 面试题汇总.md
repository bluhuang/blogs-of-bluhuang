...
**技术要点**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。例如：为什么选 Milvus 不选 FAISS，检索延迟要求多少，为什么 Top-K 设 5 不是 10。

### Q3: 传统RAG的三大痛点是什么？

传统 RAG 系统可能存在“检索到了但答不对”的情况，或者“检索到了正确信息，但生成的回答还是拼凑感很强”。如何理解这个问题？
...
**技术加分**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），说明真正调过参数。
...
已经使用混合检索，为什么还要 Rerank？检索结果不够好吗？
