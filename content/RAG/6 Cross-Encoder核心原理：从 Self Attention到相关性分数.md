...
Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。不直接使用 Cross‑Encoder 做检索的原因是：其计算复杂度为 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
| **某商业API Rerank 4** | 闭源商业API | 精度领先，多语言，按量付费 | 最快落地，不折腾运维 |
