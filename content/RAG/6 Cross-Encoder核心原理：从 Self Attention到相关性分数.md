...
> Cross‑Encoder 的目的不是替换检索，而是弥补检索排序的不足。之所以不用 Cross‑Encoder 直接做检索，是因为其计算复杂度为 O(|Q|×|D|)，无法在百万级文档上实时计算；必须先用 Bi‑Encoder 做粗筛。
...
| **闭源API Rerank 4** | 闭源 API | 精度领先，多语言，按量付费 | 最快落地，不折腾运维 |
...
> **选型建议**：选择 Rerank 模型应从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE，追求精度但不想自托管可选闭源API，学习入门用 MiniLM。
