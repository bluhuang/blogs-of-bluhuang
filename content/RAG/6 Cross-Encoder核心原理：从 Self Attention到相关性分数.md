...
> **注意**：Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。为什么不用 Cross‑Encoder 直接做检索？核心原因是：Cross‑Encoder 的计算复杂度是 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
> **关键点**：Bi‑Encoder 与 Cross‑Encoder 选择的关键是**速度与精度的权衡**。能用 Bi‑Encoder 解决的场景不要上 Cross‑Encoder，能用规则解决的不要上模型。
...
| **Rerank 4（闭源 API）** | 闭源 API | 精度领先，多语言，按量付费 | 最快落地，不折腾运维 |
| **marco‑MiniLM‑L‑6‑v2** | 22M | 超轻量，CPU 可跑，MIT 协议 | 英文通用，边缘部署，学习入门 |
| **Reranker‑4B（多语言大模型）** | 4B | 100+ 语言，Apache 2.0，32K 上下文 | 多语言 + 长文档 + 开源 |
| **ColBERT v2** | 110M | Token‑级后期交互，高吞吐 | 大规模英文知识库精排 |

> **选型建议**：选 Rerank 模型时，从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE，追求精度但不想自托管可选闭源 API，学习入门用 MiniLM。
