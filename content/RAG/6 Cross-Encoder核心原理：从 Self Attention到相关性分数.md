...
> Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。为什么不用 Cross‑Encoder 直接做检索？核心在于：Cross‑Encoder 的计算复杂度是 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
| **某商业闭源 Rerank 模型** | 闭源 API | 精度领先，多语言，按量付费 | 最快落地，不折腾运维 |
| **ms‑marco‑MiniLM‑L‑6‑v2** | 22M | 超轻量，CPU 可跑，MIT 协议 | 英文通用，边缘部署，学习入门 |
| **某开源多语言大模型 Rerank** | 4B | 100+ 语言，Apache 2.0，32K 上下文 | 多语言 + 长文档 + 开源 |
| **ColBERT v2** | 110M | Token‑级后期交互，高吞吐 | 大规模英文知识库精排 |

> **选型建议**：如何选 Rerank 模型？从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE，追求精度但不想自托管可选某商业闭源 API，学习入门用 MiniLM。
