...
> 提示：把 Rerank 理解为两个阶段——“初筛（海选）+精排（决赛）”。检索是“快手阿姨”，30 秒抓 10 个苹果（看外表）；Rerank 是“品果大师”，接过 10 个苹果仔细检查（摸一摸、闻一闻、掂一掂），告诉你哪 3 个最好吃。
...
> 提示：正确用法是**串联**而非替代——Bi‑Encoder 先快速召回候选集，Cross‑Encoder 再精细排序。用一个通俗例子说明：“Bi‑Encoder 是海选，速度第一；Cross‑Encoder 是决赛，精度第一。”
...
> 提示：问“如何选择 Rerank 模型？”可从四个维度回答：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。再补充具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想折腾选托管 API。
...
> 提示：RAGAS 的 context_precision = 1.0 表示相关文档全部排在不相关文档前面，这正是 Rerank 的核心优化目标。
...
> 提示：Rerank 同时带来**降本增效**——减少无关文档的 Token 消耗，降低 LLM 推理成本，同时提升生成质量。
...
- Top 5 Reranking Models to Improve RAG Results（英文技术网站）
- Rerank 模型对比平台
