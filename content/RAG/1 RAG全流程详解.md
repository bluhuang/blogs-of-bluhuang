...
2. **私有数据无法触达**：企业内部文档、客户数据、业务规则，LLM 从未见过，直接问就会胡说。
...
**常见提问**：“你们用的什么 Embedding 模型？为什么选它？和 OpenAI 的 ada-002 对比过吗？”
...
**典型回答**：“中文场景选 bge-large-zh，因为 MTEB 中文榜单排名靠前，而且开源可以本地部署，不用走 API。如果是英文场景或对延迟不敏感，OpenAI 的 embedding 更方便。”
...
**形象比喻**：
- Bi-Encoder 像初筛简历：只看关键词和学历，快速过滤。
...
**典型回答**："检索是粗筛快捞，Rerank 是精排提准。检索用 Bi-Encoder 快但粗，Rerank 用 Cross-Encoder 慢但准。先用检索从百万级捞 Top-20，再用 Rerank 精排取 Top-5，这是生产环境的标配流程。"
...
**核心要点**：不要只说“用了 Prompt 约束”，要说出多种策略组合及效果。例如“Prompt 约束 + 输出自校验 + 温度调低，幻觉率从 30% 降到了 12%”。
