...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以实现一些特殊功能，类似于用户定义函数（UDF）。
...
1. 该参数传入方法名：第二个参数是所创建 SQL 函数的名称。通常采用 UTF8 编码，但对于 `sqlite3_create_function16()`，应采用本机字节序的 UTF16 编码（参见 [sqlite3_create_function16()](https://sqlite.org/c3ref/create_function.html)）。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 个 UTF8 字节。任何创建超出此长度函数名的尝试都将导致 [SQLITE_MISUSE](https://sqlite.org/rescode.html#misuse) 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，如果两个调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，`int` 类型。
2. 参数大小范围默认值为 -1（可变参数），最大为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
