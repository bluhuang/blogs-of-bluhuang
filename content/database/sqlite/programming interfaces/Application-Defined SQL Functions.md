...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，即用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 通过这些接口，可以在 SQL 查询中使用自定义函数，这些函数可由 C 语言编写，或通过绑定在其他语言（如 Python、Java）中实现。
...
1. 该参数传入函数名：第二个参数是所创建 SQL 函数的名称。名称通常为 UTF8 格式，但对于 `sqlite3_create_function16()`，名称应为本地字节序的 UTF16 格式。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF8）。尝试创建更长的函数名将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可多次使用相同的函数名调用该接口。例如，若两次调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数个数，类型为 int。
2. 参数取值范围为 -1 到 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
