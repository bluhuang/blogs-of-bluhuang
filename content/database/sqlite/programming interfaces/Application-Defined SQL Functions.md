...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，用于实现特殊的自定义功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 这些函数可用于创建可在 SQL 查询中使用的自定义功能，既可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数的名称。名称通常为 UTF-8 编码，但对于 `sqlite3_create_function16()`，名称应以本地字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节（UTF-8）。任何尝试创建更长函数名的操作都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同的函数名但参数个数不同，则会注册该函数的两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，`int` 类型。
2. 参数大小范围为 -1（表示可变），默认为 127（最大值受 `SQLITE_MAX_FUNCTION_ARG` 限制，为 32767）。
