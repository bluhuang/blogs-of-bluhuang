...
# 1 简介
1. SQLite支持用户实现自定义回调函数，以扩展数据库功能，例如用户自定义函数（UDF）。
2. 自定义的SQLite UDF可以是标量函数、聚合函数或窗口函数。
3. 可以在SQL查询中使用这些自定义函数，它们既可以通过C语言编写，也可以通过绑定在其他语言（如Python、Java等）中实现。
...
- **函数类型**：
    - 如果 `xFunc` 非 NULL，则注册标量函数。
    - 如果 `xStep` 和 `xFinal` 非 NULL，则注册聚合函数。
- **应用数据**：通过 `pApp` 传递用户数据。
...
1. 该参数传入函数名：第二个参数是所创建的 SQL 函数的名称。名称通常采用 UTF8 编码，但在 `sqlite3_create_function16()` 中，名称应为 UTF16 编码并遵循本地字节序。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF8）。任何尝试创建更长名称的函数都会导致 `SQLITE_MISUSE` 错误。
3. UDF注册支持重载：可以多次使用相同的函数名调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数数量不同，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 int。
2. 参数个数范围为 -1 到 127（最大值为 `SQLITE_MAX_FUNCTION_ARG` 的 32767）。
