...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，从而提供类似用户定义函数（UDF）的特定功能。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 可以创建在 SQL 查询中使用的自定义函数。这些函数可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数的名称。名称通常采用 UTF8，但对于 [sqlite3_create_function16()](https://sqlite.org/c3ref/create_function.html) 则应以本机字节序的 UTF16 表示。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 个 UTF8 字节。任何创建更长函数名的尝试都将导致 [SQLITE_MISUSE](https://sqlite.org/rescode.html#misuse) 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 int
2. 参数取值范围为 -1，默认最大为 127（实际最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）
