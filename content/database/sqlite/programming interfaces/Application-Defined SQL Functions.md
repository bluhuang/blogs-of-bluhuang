...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以实现特定的自定义功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 这些函数可以在 SQL 查询中使用，通常用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称。通常名称使用 UTF8 编码，但对于 `sqlite3_create_function16()`，名称应为本地字节序的 UTF16。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 个 UTF8 字节。尝试创建名称更长的函数将导致 `SQLITE_MISUSE` 错误。
