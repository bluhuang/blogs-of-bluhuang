...
# 1 简介
1. SQLite 允许用户实现自定义回调函数，以提供特殊功能，类似于用户自定义函数（UDF）。
2. 自定义 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些函数用于在 SQL 查询中创建自定义操作，可通过 C 语言编写，或通过绑定在其他语言（如 Python、Java）中实现。
...
- **函数类型**：
    - 若 `xFunc` 非 NULL，则注册为标量函数。
    - 若 `xStep` 和 `xFinal` 非 NULL，则注册为聚合函数。
- **应用数据**：通过 `pApp` 参数传递用户数据。
...
1. 该参数指定函数名称，第二个参数为创建的 SQL 函数名。名称通常为 UTF-8 编码，但对于 `sqlite3_create_function16()` 应使用本地字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节（UTF-8 编码）。如果创建的名称超过此长度，将返回 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：同一函数名可多次调用创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，`int` 类型。
2. 参数个数范围从 -1 到 127，最大值为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
