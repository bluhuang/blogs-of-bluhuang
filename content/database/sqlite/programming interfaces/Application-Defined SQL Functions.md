...
# 1 简介
1. SQLite 允许用户实现自定义回调函数，以扩展特殊功能，即用户自定义函数（User Defined Function, UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数使用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。

# 2 相关接口

## sqlite3_create_function 系列函数
...
1. 该参数传入方法名：第2个参数是要创建的 SQL 函数的名称。通常采用 UTF8 编码，但 [`sqlite3_create_function16()`](https://sqlite.org/c3ref/create_function.html) 需使用本地字节序的 UTF16。
2. 函数名长度限制为255字节：SQL 函数名的最大长度为 255 个 UTF8 字节。尝试创建更长名称的函数将导致 [`SQLITE_MISUSE`](https://sqlite.org/rescode.html#misuse) 错误。
3. UDF 注册支持重载：可使用相同函数名多次调用 SQL 函数创建接口。例如，若两次调用函数名相同但参数个数不同，则会注册该 SQL 函数的两个变体，各接受不同数量的参数。
