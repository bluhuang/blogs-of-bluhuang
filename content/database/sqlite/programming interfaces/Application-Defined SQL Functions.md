...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以实现特定的自定义功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 可以创建在 SQL 查询中使用的自定义函数，这些函数可以使用 C 语言编写，或通过绑定在其他语言（如 Python、Java 等）中实现。
...
int sqlite3_create_function(
  ...
  int nArg,               /* 参数个数 (-1 表示可变参数) */
  int eTextRep,           /* 首选文本编码 */
  void *pApp,             /* 应用数据指针 */
  void (*xFunc)(sqlite3_context*,int,sqlite3_value**), /* 标量函数 */
  void (*xStep)(sqlite3_context*,int,sqlite3_value**), /* 聚合步骤函数 */
  void (*xFinal)(sqlite3_context*)                     /* 聚合结束函数 */
);
```
...
- **函数类型**：
    - 若 `xFunc` 非 NULL，注册为标量函数
    - 若 `xStep` 和 `xFinal` 非 NULL，注册为聚合函数
- **应用数据**：通过 `pApp` 传递用户数据
...
int sqlite3_create_window_function(
  ...
  int eTextRep,           /* 首选文本编码 */
  void *pApp,             /* 应用数据指针 */
  void (*xStep)(sqlite3_context*,int,sqlite3_value**), /* 步骤函数 */
  void (*xFinal)(sqlite3_context*),                    /* 结束函数 */
  void (*xValue)(sqlite3_context*),                    /* 取值函数 */
  void (*xInverse)(sqlite3_context*,int,sqlite3_value**), /* 逆函数 */
  void (*xDestroy)(void*)                              /* 析构函数 */
);
```
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数的名称。该名称通常采用 UTF-8 编码，而对于 `sqlite3_create_function16()`，则应采用本地字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF-8）。任何尝试创建名称更长的函数均会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同的函数名但参数数量不同，则会注册该 SQL 函数的两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 int。
2. 参数取值范围为 -1 到 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。当设置为 -1 时，表示可变参数。
