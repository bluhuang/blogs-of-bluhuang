...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，用于实现特殊的自定义功能，类似于用户定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 这些函数可以在 SQL 查询中创建并使用，它们可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是所创建的 SQL 函数的名称。名称通常使用 UTF8 编码，但 `sqlite3_create_function16()` 要求使用 UTF16 本地字节序。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF8）。尝试创建更长名称的函数将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：同一函数名可多次调用创建接口。例如，若两次调用参数数量不同，则会注册同一 SQL 函数的两个变体，每个变体接受不同数量的参数。
