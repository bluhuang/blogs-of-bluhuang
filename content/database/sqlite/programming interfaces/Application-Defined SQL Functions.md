...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以提供类似用户自定义函数（UDF）的特殊功能。
...
1. 该参数传入方法名：第二个参数是所创建 SQL 函数的名称。通常使用 UTF8 编码，但对于 `sqlite3_create_function16()`，应使用本地字节顺序的 UTF16。
2. 函数名长度限制为 255 字节：SQL 函数名称的最大长度为 255 字节（UTF8）。尝试创建更长的函数名称会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可多次使用相同的函数名称调用 SQL 函数创建接口。例如，若两次调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，`int` 类型。
2. 参数数量取值范围为 -1 到 127（最大为 `SQLITE_MAX_FUNCTION_ARG`，值为 32767）。-1 表示可变参数。
