...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以实现特殊的自定义功能，类似于用户定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 通过创建可在 SQL 查询中使用的自定义函数，这些函数可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
## 4.1 zFunctionName
1. 该参数传入方法名：第二个参数是所创建的 SQL 函数的名称。通常使用 UTF8，但对于 `sqlite3_create_function16()`，名称应使用本机字节序的 UTF16。
2. 函数名长度限制为 255 字节（UTF8）。尝试创建名称更长的函数将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可使用相同的函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同的函数名但参数数量不同，则会注册该 SQL 函数的两个变体，各自接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 int。
2. 参数范围为 -1（可变参数）到 127，最大值为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
...
- **取值**：
    ...
    4. `SQLITE_UTF16` (0x04)：使用本机字节序的 UTF-16
