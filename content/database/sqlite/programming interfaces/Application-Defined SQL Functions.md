...
1. SQLite 支持用户实现自定义回调函数，从而提供类似 User Defined Function（UDF）的特殊功能。
2. 自定义 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 这些函数可用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。
...
- **函数类型**：
  - 若 `xFunc` 非 NULL，注册标量函数。
  - 若 `xStep` 和 `xFinal` 非 NULL，注册聚合函数。
- **应用数据**：通过 `pApp` 传递用户数据。
...
1. 该参数传入方法名：第 2 个参数是待创建的 SQL 函数名。对于 `sqlite3_create_function16()`，函数名应为 UTF-16 编码并采用自然字节序。
2. 函数名长度限制为 255 字节（UTF-8）。若尝试创建更长名字的函数，会返回 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以对同一函数名调用多次创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册该 SQL 函数的两种变体，每种接受不同数量的参数。

## 4.2 nArg

1. 表示参数数量，`int` 类型。
2. 参数个数范围为 -1（表示可变参数）到 127（默认为 127，最大为 `SQLITE_MAX_FUNCTION_ARG` 即 32767）。
