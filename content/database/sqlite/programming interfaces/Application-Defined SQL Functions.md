...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以实现特殊功能，类似于用户定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数可通过 SQL 查询使用，实现方式可以是 C 语言，也可通过其他语言（如 Python、Java 等）的绑定实现。
...
#### 特点
- **窗口函数专用**：专门用于注册窗口函数（SQLite 3.25.0 及以上版本）
- **额外回调**：需要实现更多回调函数
- **反向处理**：通过 `xInverse` 支持窗口函数性能优化
...
1. 该参数传入方法名：`zFuncName` 为所创建的 SQL 函数的名称。通常使用 UTF-8 编码，但对于 `sqlite3_create_function16()`，名称应使用本地字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节（UTF-8）。任何尝试创建超过此长度函数名的操作将返回 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：允许以相同函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 `int`
2. 参数数量范围为 -1（表示可变参数）到 127（默认为 127，最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）
