...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以创建类似于用户定义函数（UDF）的功能。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数可以在 SQL 查询中使用，可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数名称。名称通常为 UTF-8 编码，但在 `sqlite3_create_function16()` 中应为本地字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF-8）。尝试创建更长的函数名会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：相同的函数名可以多次调用创建接口。例如，若两次调用使用相同的函数名称但参数个数不同，则会注册该 SQL 函数的两个变体，每个接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，`int` 类型。
2. 参数取值范围为 -1（表示可变参数）到默认最大值 127（实际最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
