...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以提供特定的自定义功能，类似于用户定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数可在 SQL 查询中使用，使用 C 语言编写，或通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入函数名。第二个参数是所创建的 SQL 函数的名称。名称通常为 UTF-8 编码，但对于 `sqlite3_create_function16()`，名称应为本地字节序的 UTF-16。
2. 函数名长度限制为 255 字节（UTF-8）。尝试创建名称更长的函数将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可使用相同的函数名多次调用 SQL 函数创建接口。若两次调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 `int`。
2. 参数取值范围为 -1 到 127（默认值），最大值为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
