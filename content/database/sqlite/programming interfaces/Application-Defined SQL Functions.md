...
1. SQLite 支持用户实现自定义回调函数（User-Defined Functions, UDF），用于扩展特定功能。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 开发者可以使用 C 语言直接编写，或通过绑定在其他语言（如 Python、Java）中实现。
...
1. 此参数传入函数名：第二个参数是所创建 SQL 函数的名称。名称通常使用 UTF8 编码，而对于 `sqlite3_create_function16()`，则需使用本地字节序的 UTF16 编码。
2. 函数名长度限制为 255 字节（UTF8）。尝试创建名称更长的函数将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，如果两次调用的函数名相同但参数个数不同，则会注册该 SQL 函数的两个重载版本，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 `int`。
2. 参数个数范围为 -1（可变参数）到 127（默认最大值，实际最大可达 `SQLITE_MAX_FUNCTION_ARG` 的 32767）。
