...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以提供特殊功能，类似于用户定义函数（User Defined Function）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数可在 SQL 查询中使用，可以通过 C 语言编写，或者通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是所创建 SQL 函数的名称。通常为 UTF8 编码，但在 `sqlite3_create_function16()` 中应为本地字节序的 UTF16。
2. 函数名长度限制为 255 字节（UTF8），若尝试创建超长名称则返回 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同函数名多次调用创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 int。
2. 参数大小范围为 -1（可变参数），默认为 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
