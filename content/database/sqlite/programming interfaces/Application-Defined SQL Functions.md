...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以扩展特殊功能，类似于用户定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 创建可在 SQL 查询中使用的自定义函数，这些函数可用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称，通常使用 UTF8 编码，但 `sqlite3_create_function16()` 需使用本地字节序的 UTF16。
2. 函数名长度限制为 255 字节（UTF8），若尝试创建超过此长度的函数名，将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可多次使用相同函数名调用创建接口，例如两次调用参数个数不同，则会注册两个不同参数数量的函数变体。

## 4.2 nArg
1. 表示参数数量，`int` 类型
2. 参数大小范围为 -1 到 127，最大为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
