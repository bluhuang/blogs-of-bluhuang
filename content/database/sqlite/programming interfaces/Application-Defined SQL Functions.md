...
3. 可以在 SQL 查询中创建并使用自定义函数。这些函数可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称。名称通常使用 UTF-8 编码，但对于 `sqlite3_create_function16()` 使用本地字节序的 UTF-16。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF-8）。任何创建更长函数名的尝试都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但不同的参数个数，则会注册该 SQL 函数的两个变体，每个接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，int 类型。
2. 参数大小的范围为 -1（可变参数），默认为 127；最大值为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
