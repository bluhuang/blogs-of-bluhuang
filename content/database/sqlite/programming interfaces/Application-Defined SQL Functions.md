...
# 1 简介
1. SQLite支持用户实现自定义回调方法，以提供特殊的自定义功能，类似于用户定义函数（UDF）。
2. 自定义的SQLite UDF可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数可在SQL查询中使用，既可以用C语言编写，也可以通过绑定在其他语言中实现（如Python、Java等）。
...
- **函数类型**：
    - 如果`xFunc`非NULL，注册标量函数
    - 如果`xStep`和`xFinal`非NULL，注册聚合函数
- **应用数据**：通过`pApp`传递用户数据
...
#### 特点
- **UTF-16 支持**：函数名使用UTF-16编码
- **向后兼容**：为需要UTF-16函数名的应用提供支持
- **功能相同**：与`sqlite3_create_function()`功能相同，仅函数名编码不同
- **使用场景**：
    - Windows API集成（Windows原生使用UTF-16）
    - 需要处理宽字符的遗留系统
...
1. 此参数传入函数名：第二个参数是要创建的SQL函数的名称。名称通常为UTF8编码，但对于[sqlite3_create_function16()](https://sqlite.org/c3ref/create_function.html)，名称应为本地字节序的UTF16。
2. 函数名长度限制255字节：SQL函数名的最大长度为255个UTF8字节。尝试创建更长的函数名将导致[SQLITE_MISUSE](https://sqlite.org/rescode.html#misuse)错误。
3. UDF注册支持重载：可以使用相同的函数名多次调用SQL函数创建接口。例如，如果两次调用具有相同的函数名但不同的参数个数，则会注册该SQL函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，int类型。
2. 参数大小范围为-1，默认值为127（最大为`SQLITE_MAX_FUNCTION_ARG`，值为32767）。

## 4.3 **eTextRep** 文本编码
- **类型**：`int` - 位掩码
- **作用**：指定函数的文本编码偏好
- **取值**：
    1. **SQLITE_UTF8** (0x01)：UTF-8编码
    2. **SQLITE_UTF16LE** (0x02)：UTF-16小端序
    3. **SQLITE_UTF16BE** (0x03)：UTF-16大端序
    4. **SQLITE_UTF16** (0x04)：使用本地字节序的UTF-16
