...
3. 这些自定义函数可在 SQL 查询中使用，既可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称。通常为 UTF8 编码，但对于 `sqlite3_create_function16()` 则应为本地字节序的 UTF16。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF8）。任何尝试创建长度超过此限制的函数都将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：同一个函数名可以多次调用创建接口。例如，如果两次调用具有相同的函数名称但参数个数不同，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，int 类型
2. 参数大小范围为 -1（表示可变参数），默认最大值为 127（实际最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
