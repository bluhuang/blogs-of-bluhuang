...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称。通常使用 UTF8 编码，但 `sqlite3_create_function16()` 要求函数名使用原生字节序的 UTF16 编码。
2. 函数名长度限制为 255 字节（UTF8）。任何尝试创建名称更长的函数都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数个数不同，则会注册两个该函数的不同变体，每个变体接受不同数量的参数。

## 4.2 nArg

1. 表示参数数量，`int` 类型。
2. 参数个数范围为 -1（可变参数）到默认最大值 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
