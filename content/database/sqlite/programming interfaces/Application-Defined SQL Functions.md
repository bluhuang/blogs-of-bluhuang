...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，从而提供一些特殊的自定义功能，类似于用户自定义函数（User-Defined Function, UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 这些函数可以在 SQL 查询中使用。它们可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。

# 2 相关接口

## sqlite3_create_function 系列
### 2.1 sqlite3_create_function()
#### 基本信息
...
- **函数类型**：
    - 如果 `xFunc` 非 NULL，注册为标量函数
    - 如果 `xStep` 和 `xFinal` 非 NULL，注册为聚合函数
- **应用数据**：通过 `pApp` 传递用户数据
- **内存管理**：无自动清理机制，需要手动管理内存

### 2.2 sqlite3_create_function16()
...
### 2.3 sqlite3_create_function_v2()
#### 基本信息
...
### 2.4 sqlite3_create_window_function
#### 基本信息
```
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数的名称。通常使用 UTF-8 编码，但对于 `sqlite3_create_function16()`，名称应使用本机字节序的 UTF-16。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF-8）。尝试创建名称更长的函数将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以多次使用相同的函数名调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 int
2. 参数数量范围：-1 表示可变参数，默认最大为 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）
