...
1. SQLite 支持用户实现自定义回调方法，以实现一些特殊功能，即用户自定义函数（User Defined Function，UDF）。
2. 自定义的 SQLite UDF 可以是标量函数（scalar functions）、聚合函数（aggregate functions）或窗口函数（window functions）。
3. 可以创建可在 SQL 查询中使用的自定义函数，这些函数既可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数的名称。名称通常为 UTF8 编码，但对于 `sqlite3_create_function16()`，名称应为本地字节序的 UTF16 编码。
2. 函数名长度限制为 255 字节（UTF8）。尝试创建名称更长的函数会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以多次使用相同的函数名调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数个数不同，则会注册该 SQL 函数的两个变体，各自接受不同数量的参数。

## 4.2 nArg

1. 表示参数数量，`int` 类型。
2. 参数数量范围从 -1（表示可变参数）到 127（默认值），最大可为 `SQLITE_MAX_FUNCTION_ARG`（32767）。
