...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以完成特定的定制功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些函数可在 SQL 查询中使用，既可用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。
...
- **使用场景**：
    - 集成 Windows API（Windows 原生使用 UTF-16）。
    - 处理宽字符的遗留系统。
...
1. 该参数传入方法名：第2个参数是要创建的 SQL 函数的名称。通常为 UTF8 编码，但 `sqlite3_create_function16()` 要求使用本地字节序的 UTF16。
2. 函数名长度限制为 255 字节（UTF8）。若尝试创建更长的函数名，将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以多次以相同函数名调用创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册该 SQL 函数的两个变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，`int` 类型。
2. 参数数量的取值范围为 -1 到 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
