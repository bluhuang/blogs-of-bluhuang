...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以实现一些特殊的自定义功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数（scalar functions）、聚合函数（aggregate functions）或窗口函数（window functions）。
3. 这些自定义函数可以在 SQL 查询中使用，它们可以用 C 语言编写，也可以通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 此参数传入方法名：第二个参数是所创建的 SQL 函数的名称。通常使用 UTF-8 编码，但对于 [sqlite3_create_function16()](https://sqlite.org/c3ref/create_function.html)，应使用本地字节序的 UTF-16。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 个 UTF-8 字节。尝试创建名称更长的函数将导致 [SQLITE_MISUSE](https://sqlite.org/rescode.html#misuse) 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数数量不同，则会注册该 SQL 函数的两个变体，每个变体接受不同的参数个数。

## 4.2 nArg
1. 表示参数数量，int 类型。
2. 参数大小的取值范围为 -1，默认值为 127（最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
