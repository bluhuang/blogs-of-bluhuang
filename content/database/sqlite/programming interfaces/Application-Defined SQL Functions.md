...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以便扩展数据库功能，这些回调类似于用户定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 可以创建在 SQL 查询中使用的自定义函数，这些函数通常用 C 语言编写，也可以通过绑定在其他编程语言（如 Python、Java 等）中实现。
...
- **功能相同**：与 `sqlite3_create_function()` 功能一致，仅函数名编码不同
- **适用场景**：
...
1. 此参数传入函数名：第二个参数是要创建的 SQL 函数的名称。名称通常使用 UTF8 编码，但在 `sqlite3_create_function16()` 中应使用本地字节序的 UTF16 编码。
2. 函数名长度限制为 255 字节（UTF8）。尝试创建名称更长的函数将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：相同的函数名可以被多次调用注册。例如，若两次调用具有相同的函数名但参数个数不同，则会注册两个不同参数数量的函数变体。

## 4.2 nArg
1. 表示参数个数，类型为 int。
2. 参数取值范围为 -1 到 127（最大为 `SQLITE_MAX_FUNCTION_ARG` 的 32767），其中 -1 表示可变参数。
...
- **参数**：
    1. `sqlite3_context*`：函数上下文，用于设置结果和错误信息
    2. `int`：实际参数个数
