...
1.  SQLite 支持用户实现自定义回调方法，以实现特定的自定义功能，类似于用户自定义函数（User Defined Function）。
2.  自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3.  可以创建可在 SQL 查询中使用的自定义函数，这些函数用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。
...
1.  该参数传入方法名：第二个参数是正在创建的 SQL 函数的名称。名称通常为 UTF8 编码，但 [`sqlite3_create_function16()`](https://sqlite.org/c3ref/create_function.html) 要求名称采用本机字节序的 UTF16。
2.  函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF8）。尝试创建更长的函数名将导致 `SQLITE_MISUSE` 错误。
3.  UDF 注册支持重载：可使用相同的函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册该 SQL 函数的两个变体，各接受不同数量的参数。

## 4.2 nArg

1.  表示参数数量，`int` 类型。
2.  参数个数可设为 -1（表示可变参数），最大值为 `SQLITE_MAX_FUNCTION_ARG`（127，实际允许最大值为 32767）。
