...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以扩展特殊功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 可以创建在 SQL 查询中使用的自定义函数，这些函数可用 C 语言编写，或通过绑定在其他语言（如 Python、Java 等）中实现。
...
#### 特点
- **窗口函数专用**：专用于注册窗口函数（SQLite 3.25.0+）。
- **额外回调**：需实现更多回调函数。
...
1. 该参数传入方法名：第二个参数是待创建的 SQL 函数的名称。名称通常采用 UTF8 编码，除了在 `sqlite3_create_function16()` 中应使用本地字节序的 UTF16。
2. 函数名长度限制为 255 字节（UTF8）。任何尝试创建名称更长的函数都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以多次使用相同的函数名调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数个数不同，则会注册该函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，int 类型。
2. 参数大小范围为 -1（可变），默认最大值为 127（实际最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
