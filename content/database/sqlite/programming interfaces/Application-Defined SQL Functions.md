...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以实现特殊自定义功能，类似于用户自定义函数（UDF）。
...
1. 该参数传入方法名：第二个参数是所创建的 SQL 函数的名称。名称通常使用 UTF-8 编码，但对于 `sqlite3_create_function16()`，需要使用本地字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节（UTF-8）。任何创建更长的函数名的尝试都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可使用相同函数名多次调用 SQL 函数创建接口。例如，如果两次调用具有相同的函数名但参数个数不同，则会注册两个不同参数版本的 SQL 函数。

## 4.2 nArg
1. 表示参数数量，类型为 int。
2. 参数范围：-1（表示可变参数）到 127（默认最大值，实际最大为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）。
