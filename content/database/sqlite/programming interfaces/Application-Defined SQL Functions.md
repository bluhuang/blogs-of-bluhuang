...
3. 用户可创建在 SQL 查询中使用的自定义函数。这些函数可以用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第二个参数是正在创建的 SQL 函数的名称。名称通常使用 UTF-8 编码，但对于 `sqlite3_create_function16()`，名称应使用本地字节序的 UTF-16 格式。
2. 函数名长度限制为 255 字节 UTF-8。尝试使用更长名称创建函数会返回 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可使用相同函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同的函数名但参数个数不同，则会注册两个不同参数个数的函数变体。

## 4.2 nArg

1. 表示参数数量，int 类型。
2. 参数取值范围为 -1 至 `SQLITE_MAX_FUNCTION_ARG`（32767）。若指定 -1，表示可变参数，实际最大参数个数默认为 127。
