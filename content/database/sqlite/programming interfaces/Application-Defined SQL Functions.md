...
# 1 简介
1. SQLite 支持用户实现自定义回调，以实现一些特殊的自定义功能，类似于用户自定义函数 (UDF)。
...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称。名称通常采用 UTF8 编码，但 `sqlite3_create_function16()` 要求使用本机字节序的 UTF16 编码。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节（UTF8）。任何尝试创建更长的函数名都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：相同函数名可以多次调用 SQL 函数创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册两个不同参数数量的函数变体。

## 4.2 nArg
1. 表示参数数量，`int` 类型
2. 参数个数范围从 -1 到 127（最大为 `SQLITE_MAX_FUNCTION_ARG` 的 32767）。
