...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以扩展特殊功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 这些函数可在 SQL 查询中直接使用，支持用 C 语言编写，也可通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入函数名：第二个参数是要创建的 SQL 函数的名称。通常使用 UTF-8 编码，但对于 `sqlite3_create_function16()`，名称应以本机字节序的 UTF-16 编码。
2. 函数名长度限制为 255 字节：SQL 函数名的最大长度为 255 字节的 UTF-8。任何尝试创建超过此长度的函数都会导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：同一函数名可以多次调用 SQL 函数创建接口。例如，若两次调用参数数量不同，则注册该 SQL 函数的两种变体，分别接受不同数量的参数。

## 4.2 nArg
1. 表示参数数量，类型为 `int`。
2. 参数范围：最小值 -1（表示可变参数），最大值默认为 127，实际最大为 `SQLITE_MAX_FUNCTION_ARG` (32767)。
