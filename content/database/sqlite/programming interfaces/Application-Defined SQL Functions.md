...
# 1 简介
1. SQLite 支持用户实现自定义回调方法，以实现特殊功能，类似于用户自定义函数（User Defined Function，UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数或窗口函数。
3. 这些自定义函数可以使用 C 语言编写，或通过绑定在其他语言（如 Python、Java 等）中实现。
...
1. 该参数传入方法名：第2个参数是正在创建的 SQL 函数名。通常采用 UTF-8 编码，但 `sqlite3_create_function16()` 接口要求使用本地字节序的 UTF-16 格式。
2. 函数名长度限制为 255 字节的 UTF-8。若尝试创建名称更长的函数，将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，若两次调用具有相同函数名但参数个数不同，则会注册该 SQL 函数的两个变体，分别接受不同数量的参数。
