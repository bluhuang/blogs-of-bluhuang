...
# 1 简介
1. SQLite 支持用户实现自定义回调函数，以实现特殊的自定义功能，类似于用户自定义函数（UDF）。
2. 自定义的 SQLite UDF 可以是标量函数、聚合函数，也可以是窗口函数。
3. 通过该机制，可以在 SQL 查询中使用自定义函数，这些函数可由 C 语言编写，或通过绑定在其他语言（如 Python、Java 等）中实现。
...
- **使用场景**：
    - 与 Windows API 集成（Windows 原生使用 UTF-16）
    - 需要处理宽字符的遗留系统
...
1. 该参数传入方法名：第二个参数是要创建的 SQL 函数的名称。名称通常为 UTF8 编码，但对于 `sqlite3_create_function16()`，名称需采用本地字节序的 UTF16 编码。
2. 函数名长度限制为 255 字节（UTF8）。尝试创建超过此长度的函数名将导致 `SQLITE_MISUSE` 错误。
3. UDF 注册支持重载：可以使用相同的函数名多次调用 SQL 函数创建接口。例如，若两次调用使用相同的函数名但不同的参数个数，则会注册该 SQL 函数的两个变体，每个变体接受不同数量的参数。

## 4.2 nArg
1. 表示参数个数，`int` 类型
2. 参数个数的取值范围为 -1（表示可变参数），默认最大值为 127（实际最大值为 `SQLITE_MAX_FUNCTION_ARG`，即 32767）
