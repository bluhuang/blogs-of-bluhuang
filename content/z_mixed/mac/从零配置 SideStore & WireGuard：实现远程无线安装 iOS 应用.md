...
> **目标**：在 Mac 和 iPhone 之间配置完全无线、可续签的 iOS App 安装环境，使构建工具能远程生成 `.ipa`，并通过手机上的 SideStore 直接安装，无需 USB。
...
### 🤔 为什么这样做？
这些文件下载源都在 GitHub，国内访问不稳定，通过 Mac 终端下载更快、更可靠，且集中存放便于管理。
...
> 🧑‍💻 **此阶段必须手动完成**，因为无法通过自动化工具操作 App Store。
...
5. 至此，环境配置完成！现在可以通过 SideStore 安装任何 `.ipa` 文件。
