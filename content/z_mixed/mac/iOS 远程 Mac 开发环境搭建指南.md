...
> 目标：让任何人能根据本指南，用 iPhone 远程连接家里的 Mac，通过终端或 Web 界面进行代码开发。

## 1. 准备工作

- 一台 Mac（保持开机，连接家庭 Wi-Fi）
- 一部 iPhone（可使用 5G 或其他 Wi-Fi）
- 两台设备登录同一个 Apple ID（非必需，但方便）
...
2. 安装后，启动 Tailscale，使用你的账号（Google/Microsoft/GitHub）登录
3. 菜单栏出现 Tailscale 图标，确保状态显示为 `Connected`

### 2.2 开启 Mac 的远程登录
- 打开 **系统设置 → 通用 → 共享**
- 开启 **远程登录** 开关
- 下方“允许访问”建议设为“所有用户”，或仅允许你的 Mac 用户名
...
安装后，若提示 `command not found`，添加 PATH：
...
- 打开 App，使用 **同一个账号** 登录
- 确保顶部开关呈现绿色 `Connected`

### 3.2 安装 Termius（SSH 客户端）
- 在 App Store 搜索“Termius”并安装（免费版足够使用）
...
### 4.1 连接前检查
- Mac 上的 Tailscale 已启动且状态为 `Connected`
- iPhone 上的 Tailscale 已打开且状态为 `Connected`

### 4.2 使用 Termius SSH 连接 Mac
- 打开 Termius，点击保存的主机 → Connect
- 输入密码（若未保存密码）
...
> 若不运行 caffeinate，Mac 可能在长时间无操作后进入睡眠状态，导致连接断开。
...
按 `Ctrl+B` 再按 `D`，返回普通 SSH 界面，之后可直接关闭 Termius。
...
- **连接不上**：检查 Tailscale 是否均显示 `Connected`；检查 Mac 远程登录是否已开启。
- **速度非常慢**：查看 `tailscale status` 是否显示 `relay "xxx"`。若是，说明走中继，需参考笔记二中的解决方案。
