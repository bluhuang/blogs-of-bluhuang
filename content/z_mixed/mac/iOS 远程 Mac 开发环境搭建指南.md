...
1. 访问官网 [https://tailscale.com/download](https://tailscale.com/download) → 下载 macOS 版安装包
2. 安装后启动 Tailscale，用你的账号（Google/Microsoft/GitHub）登录
...
安装后若提示 `command not found`，添加 PATH：
...
> 若不运行 caffeinate，Mac 可能在长时间无操作后睡眠，导致连接断开。
...
- **速度非常慢**：查看 `tailscale status` 是否显示 `relay "xxx"`。若是，说明走中继，需要参考其他笔记中的解决方案。
