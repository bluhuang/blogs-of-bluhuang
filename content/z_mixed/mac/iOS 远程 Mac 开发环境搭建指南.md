...
2. 安装后启动 Tailscale，通过你的账号（Google / Microsoft / GitHub）登录。
...
> 若不运行 caffeinate，Mac 可能在长时间无操作后自动睡眠，导致连接断开。
...
- **连接不上**：检查 Tailscale 是否均为 Connected；检查 Mac 远程登录是否开启。
- **速度非常慢**：查看 `tailscale status` 是否显示 `relay "xxx"`。如果是，说明走中继，需要参考相关笔记中的解决方案。
