...
Morris 遍历是一种**使用线索二叉树（threaded binary tree）思想**的二叉树遍历算法，由 Morris 在 1979 年提出。其**核心特点**是：
- **空间复杂度 O(1)**：不使用递归栈或队列，只利用二叉树中大量的空闲指针（右指针）来临时记录遍历的后继节点，从而实现遍历。
- **时间复杂度 O(n)**：每个节点最多被访问两次。
Morris 遍历可用于**前序、中序、后序**遍历，其中最经典和常用的是**中序遍历**。
...
#### 2. 中序遍历（经典）
### 核心思想
可以将 Morris 遍历理解为：在二叉树中，当遇到有左子树时，需要先遍历左子树再返回。由于没有使用栈，可以通过建立临时线索来记录返回路径。具体做法是：在进入左子树之前，找到左子树的最右节点（即中序遍历下当前节点的前驱），将其右指针指向当前节点，作为返回的“线索”。遍历完左子树后，通过该线索返回，并恢复指针。
