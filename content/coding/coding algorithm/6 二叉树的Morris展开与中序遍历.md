...
Morris 遍历是一种利用线索二叉树（threaded binary tree）思想的二叉树遍历算法，于 1979 年提出。其**核心特点**如下：
...
在普通递归或迭代遍历中，需要额外的栈来保存返回路径，空间复杂度为 O(h)（h 为树高）。Morris 遍历巧妙地利用了叶子节点的空右指针，将其指向中序遍历下的后继节点，从而在遍历过程中无需栈即可回到上层节点。当访问完该节点后，再恢复空指针，保证树的结构不变。
