...
- **空间复杂度 O(1)**：不使用递归栈或队列，只利用二叉树中空闲的右指针临时记录后继节点，从而实现遍历。
...
在普通递归或迭代遍历中，需要额外的栈来保存返回路径，空间复杂度为 O(h)（h 为树高）。Morris 遍历利用了叶子节点的空右指针，将其指向中序遍历下的后继节点，从而在遍历过程中无需栈即可回到上层节点。访问完该节点后恢复空指针，保证树的结构不变。
...
- 随后 `cur` 移动到下一个右节点（`cur = cur->right`），重复。
...
#### 2. 中序遍历（经典）
### 核心思想
想象在走一个迷宫（二叉树），每次遇到有左岔路时，先走完左边再回来。没有栈（不能记路），怎么办？  
可以在进入左边之前，在左路的最深处（最右边的节点）放一根绳子，绳子另一头系在当前节点上。走完左边后，顺着绳子就能回来。回来后再解开绳子，然后继续往右走。
