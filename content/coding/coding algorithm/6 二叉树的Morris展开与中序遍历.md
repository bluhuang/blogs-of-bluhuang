...
Morris 遍历可用于**前序、中序、后序**遍历，最经典且最常用的是**中序遍历**。

### 二、为什么可以用 Morris 遍历？

在常规递归或迭代遍历中，需要额外的栈来保存返回路径，空间复杂度为 O(h)（h 为树高）。Morris 遍历巧妙地利用叶子节点的空右指针，将其指向中序遍历下的后继节点，从而在遍历过程中无需栈即可回到上层节点。访问完该节点后，再恢复空指针，保证树的结构不变。
