...
Morris 遍历是一种**使用线索二叉树（threaded binary tree）思想**的二叉树遍历算法，由 J. H. Morris 在 1979 年提出。其**核心特点**包括：
- **空间复杂度 O(1)**：不使用递归栈或队列，仅利用二叉树中空闲的指针（右指针）临时记录遍历的后继节点，从而实现遍历。
- **时间复杂度 O(n)**：每个节点最多被访问两次。
Morris 遍历可用于**前序、中序、后序**遍历，其中**中序遍历**最为经典和常用。

### 二、为什么可以用 Morris 遍历？

在普通递归或迭代遍历中，需要额外的栈来保存返回路径，空间复杂度为 O(h)（h 为树高）。Morris 遍历巧妙地利用了叶子节点的空右指针，将其指向中序遍历下的后继节点，从而在遍历过程中无需栈就能回到上层节点。访问完该节点后，再恢复空指针，确保树的结构不变。

**适用条件**：
- 要求 O(1) 空间遍历或修改二叉树
- 允许临时修改树结构
- 要求不能使用递归或栈（递归为 O(h) 空间）
