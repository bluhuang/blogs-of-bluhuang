...
Morris 遍历可用于**前序、中序、后序**遍历，其中以**中序遍历**最为经典和常用。

### 二、为什么可以用 Morris 遍历？

在常规的递归或迭代遍历中，需要额外的栈来保存返回路径，空间复杂度为 O(h)（h 为树高）。Morris 遍历巧妙地利用了叶子节点的空右指针，将其指向中序遍历下的后继节点，从而在遍历过程中不需要栈就能回到上层节点。当访问完该节点后，再恢复空指针，保证树的结构不变。
...
#### 2. 中序遍历（经典）
### 核心思想
想象你在探索一座迷宫（二叉树），每当遇到左侧岔路，你想先遍历左边再返回。然而你没有栈（不能记录路径），该怎么办呢？  
