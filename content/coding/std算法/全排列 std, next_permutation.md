...
1. 从右向左找到第一个升序对（确定要变化的位置）。
2. 从右向左找到第一个大于该位置元素的数（确定交换的目标）。
3. 交换，然后反转后面的部分。
...
## 为什么最后可以反转？
因为交换的两个元素，就是找到的正好比原先 i 要大一点点的数字，交换后元素3（j位置）的左边都比它大（不然不会交换 j），右边都比它小（因为是找到第一个比3大的元素进行交换），所以交换后右边的这些元素仍然保持降序，反转后会变成升序。
