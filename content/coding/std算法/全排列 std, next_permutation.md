...
`next_permutation` 算法的核心是**三步**：

1. 从右找到第一个升序对（确定要变的位置）。
2. 从右找到第一个大于该位的数（确定交换的位置）。
3. 交换，然后反转后面的部分。
...
| ---------- | --------------------------------------------------------------------------------------- | --------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                       |                                         |
| **1. 找 i** | 从右向左找 `nums[i] < nums[i+1]`：<br>`2` 无右；`4>2` 不成立；`5>4` 不成立；`3<5` ✅，所以 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，【元素3后面的数已呈降序，不可能再变大】 |
...
## 为什么最后可以反转？
因为交换的两个元素，就是找到的正好比原先 i 要正好大一点的数字，交换后元素3（j位置）的左边都比它大（否则不会交换 j），右边都比它小（因为是找到第一个比3大的元素进行的交换），所以交换后，右边的这些元素仍保持降序，反转后会变成升序。

## 为什么步骤必须这样？

- **找 i**：保证改动的是尽可能靠右的位置，这样整体增大最小。
- **找 j**：保证换到 i 位置的数是右边**最小的大数**，这样增大刚好一点点。
- **反转**：因为 i 右边原本是降序（最大顺序），现在 i 变大后，右边需要变成升序（最小顺序）才能得到刚好大的下一个。
