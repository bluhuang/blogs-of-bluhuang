...
## 2.1 一句话总结原理
从右向左找到第一个可以变大的元素的位置，将其替换为右边比它大的最小数字，然后将右边剩余部分重新排成最小顺序。
...
| ---------- | --------------------------------------------------------------------------------------- | --------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                       |                                         |
| **1. 找 i** | 从右向左找 `nums[i] < nums[i+1]`：<br>`2` 无右；`4>2` 不成立；`5>4` 不成立；`3<5` ✅，所以 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，【元素3后面的数已经都是降序，不可能再变大】 |
...
### 为什么最后可以反转？
因为交换的两个元素，恰好是比原先 i 位置正好大一点的数字。交换后，元素3（j 位置）的左边都比它大（否则不会选择 j），右边都比它小（因为找到的是第一个比3大的元素进行交换），所以交换后右边的这些元素仍然保持降序，反转后会变成升序。

### 为什么步骤必须这样？

- **找 i**：保证改动的是尽可能靠右的位置，这样整体增大幅度最小。
- **找 j**：保证换到 i 位置的数是右边**最小的大数**，这样只增大一点点。
- **反转**：因为 i 右边原本是降序（最大顺序），现在 i 变大后，右边需要变成升序（最小顺序）才能得到刚好比当前大的下一个排列。
