...
## 2.1 一句话总结原理
从右向左找到第一个可以变大的元素的位置，将其替换成右边比它大的最小数字，然后将右边剩余部分重新排列成最小顺序。

`next_permutation` 算法的核心在于**三步**：

1. 从右向左寻找第一个升序对（确定要变动的位）。
2. 从右向左寻找第一个大于该位的数（确定交换的位）。
3. 交换，然后反转后面的部分。
...
| ---------- | --------------------------------------------------------------------------------------- | --------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                       |                                         |
| **1. 找 i** | 从右向左找 `nums[i] < nums[i+1]`：<br>`2` 无右；`4>2` 不成立；`5>4` 不成立；`3<5` ✅，所以 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，该位置右侧的数已呈降序，无法再增大 |
...
## 为什么最后可以反转？
因为交换的两个元素正是比原先 i 位置稍大的数字。交换后，原 i 位置的数（即现在的 j 位置）左边的数都比它大（否则不会交换 j），右边的数都比它小（因为是从右向左找到的第一个大于 3 的元素）。因此，交换后右侧部分仍保持降序，反转后即变为升序。

## 为什么步骤必须这样？

- **找 i**：保证改动尽可能靠右，使整体增大幅度最小。
- **找 j**：保证交换到 i 位置的数是右边**最小的较大数**，从而实现仅增大一点点。
- **反转**：因为 i 右边原本是降序（最大顺序），当 i 变大后，右边需要变成升序（最小顺序）才能得到紧邻的下一个排列。
