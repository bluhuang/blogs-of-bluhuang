...
## 2.1 一句话总结原理
从右向左找到第一个可以变大的元素的位置，将其换成右边比它大的最小数字，然后将右边剩余部分重新排列为最小顺序。

`next_permutation` 算法的精髓在于**三步**：

1. 从右找第一个升序对（确定要变动的位置）。
2. 从右找第一个大于该位置的数（确定交换的位置）。
3. 交换，然后反转后面的部分。
...
## 为什么最后可以反转？
因为交换的两个元素，正是找到的恰好比原先 i 位置大一点的数字，交换后元素3（j 位置）的左边都比它大（否则不会交换 j），右边都比它小（因为找到的是第一个比 3 大的元素进行的交换），所以交换后，右边的这些元素依然保持降序，反转后会变成升序。
