...
1. 从右找到第一个升序对（确定需要变动的位置）。
2. 从右找到第一个大于该位置元素的数（确定交换的目标）。
3. 交换，然后反转后面的部分。
...
## 为什么最后可以反转？
因为交换的两个元素，正是找到的比原先 i 位置元素刚好大一点的数字。交换后，元素3（j 位置）的左边都比它大（否则不会交换 j），右边都比它小（因为是从右找到第一个比 3 大的元素进行交换）。因此交换后，右边的这些元素依然保持降序，反转后会变成升序。

## 为什么步骤必须这样？

- **找 i**：保证改动的是尽可能靠右的位置，从而整体增大最小。
