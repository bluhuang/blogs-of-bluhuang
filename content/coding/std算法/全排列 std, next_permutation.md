...
`next_permutation` 算法的核心有三步：

1. 从右向左找到第一个升序对（确定要调整的位置）。
...
### 为什么最后可以反转？
因为交换的两个元素，就是找到的正好比原先 i 稍大一点的数字。交换后，元素3（j 位置）的左边都比它大（不然不会交换 j），右边都比它小（因为是找到第一个比3大的元素进行交换）。所以交换后，右边的这些元素依然保持降序，反转后会变成升序。
