...
## 2.1 一句话总结原理
从右向左找到第一个可增大的元素位置，将其替换为右边比它大的最小数字，然后将右边剩余部分重新排列为最小顺序。
...
| ------------ | ------------------------------------------------------------------------------------------- | ----------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                           |                                           |
| **1. 找 i**  | 从右向左找 `nums[i] < nums[i+1]`：<br>`2` 无右；`4>2` 不成立；`5>4` 不成立；`3<5` ✅，所以 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，元素3后面的数都已经是降序，不可能再变大 |
