...
1. 从右向左寻找第一个升序对（确定需要变动的位）。
2. 从右向左寻找第一个大于该位的数（确定与之交换的位）。
3. 交换这两个数，然后反转后面的部分。
...
| ---------- | --------------------------------------------------------------------------------------- | --------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                       |                                         |
| **1. 找 i** | 从右向左遍历，寻找满足 `nums[i] < nums[i+1]` 的位置：  <br>`2` 无右侧元素；`4>2` 不满足；`5>4` 不满足；`3<5` ✅，因此 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，元素3右侧的数已呈降序，无法再增大 |
| **2. 找 j** | 在 i 右侧的区间 `[5,4,2]` 中，从右向左寻找第一个大于 3 的数：  <br>`2` 不大于；`4` 大于，因此 `j = 3`（指向 `4`）               | 找到刚好比 3 大的数 4                           |
| **3. 交换**  | 交换 `nums[1]` 和 `nums[3]` → `[1, 4, 5, 3, 2]`                                            |                                         |
| **4. 反转**  | 将 i+1（索引2）至末尾的区间 `[5,3,2]` 反转 → `[2,3,5]`                                                  | 最终得到 `[1,4,2,3,5]`                        |
结果 `14235` 确实是 `13542` 的下一个排列。

## 为什么最后可以反转？
因为交换的两个元素正好使得 i 位置变得比原先大一点。交换后，原来 j 位置的元素（即原 3）左侧的所有数都比它大（否则不会选择 j），右侧的所有数都比它小（因为是从右向左找到的第一个大于原 i 的数），因此交换后右边的序列仍保持降序，反转后便成为升序。

## 为什么步骤必须这样？

- **找 i**：保证改动尽可能靠右，使整体增大幅度最小。
- **找 j**：保证换到 i 位置的数是右边**最小的大数**，使增大刚好一点点。
- **反转**：因为 i 右边原本是降序（最大顺序），现在 i 变大后，右边需要变成升序（最小顺序）才能得到恰好的下一个排列。
