...
1. 从右向左找到第一个升序对（确定要移动的位置）。
2. 从右向左找到第一个大于该位的数（确定交换的位置）。
3. 交换，然后反转后面的部分。
...
| ---------- | --------------------------------------------------------------------------------------- | --------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                       |                                         |
| **1. 找 i** | 从右向左找 `nums[i] < nums[i+1]`：  <br>`2` 无右；`4>2` 不成立；`5>4` 不成立；`3<5` ✅，所以 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，【元素3后面的数都已经是降序，无法再变大】 |
...
## 为什么最后可以反转？
因为交换的两个元素是找到的刚好比原 i 位置元素大一点点的数字，交换后，元素3（即原来的j位置）左边的数都比它大（否则不会交换j），右边的数都比它小（因为是从右边找到第一个大于3的元素进行交换），所以交换后右边的元素仍然保持降序，反转后变为升序。

## 为什么步骤必须这样？

- **找 i**：保证改动的是尽可能靠右的位置，使得整体增大最小。
- **找 j**：保证换到 i 位置的数是右边**最小的大数**，使得增大恰好一点点。
