...
## 2.1 一句话总结原理
从右向左找到第一个可以变大的元素的位置，将其替换为右边比它大的最小数字，然后把右边剩余部分重新排成最小顺序。

`next_permutation` 算法的精髓就是**三步**：

1. 从右找第一个升序对（确定要动的位置）。
2. 从右找第一个大于该位的数（确定交换的位置）。
3. 交换，然后反转后面的部分。
...
| ---------- | --------------------------------------------------------------------------------------- | --------------------------------------- |
| 初始         | `[1, 3, 5, 4, 2]`                                                                       |                                         |
| **1. 找 i** | 从右向左找 `nums[i] < nums[i+1]`：  <br>`2` 无右；`4>2` 不成立；`5>4` 不成立；`3<5` ✅，所以 `i = 1`（指向 `3`） | 找到第一个可增大的位置 i=1，【元素3后面的数都已经是降序，不可能再变大了】 |
...
## 为什么最后可以反转？
因为交换的两个元素，正好是比原先 i 位置元素大一点的数字。交换后，原 j 位置（即现在的 i 位置）的左边都比它大（否则不会交换 j），右边都比它小（因为是找到第一个比原 i 元素大的元素进行交换），所以交换后右边的这些元素仍然保持降序，反转后会变成升序。

## 为什么步骤必须这样？

- **找 i**：确保改动的位置尽可能靠右，从而整体增大最小。
- **找 j**：确保换到 i 位置的数是右边**最小的大数**，这样增大刚好一点点。
- **反转**：因为 i 右边原本是降序（最大顺序），现在 i 变大后，右边需要变成升序（最小顺序）才能得到刚好更大的下一个。
