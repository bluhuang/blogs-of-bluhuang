...
ReAct（Reasoning + Acting）是于 2022 年提出的一种智能体框架，核心创新在于让 LLM 交替生成**推理轨迹**与**任务操作**，模拟人类解决问题时的"思考-行动-观察"循环。

### 🧠 前言：从 CoT 到 ReAct
	COT：Chain-of-Thought, 思维链
在仅具备基础对话能力的阶段，大模型更像一个“一次性回答机”：
`User: 问题 → LLM: 一次性生成答案`
即使添加了 Memory、RAG，智能体也只是多了“能记”和“会查”：
- Memory：记住过去发生了什么（多轮对话、历史任务状态）
- RAG：在回答前查询知识库或互联网
但这仍然是“问一答一”的模式，缺少真正的多步决策与行动能力。
ReAct（Reasoning + Acting） 正是为解决此问题而提出：
...
# 2 为什么需要 ReAct
在 ReAct 出现之前，利用 LLM 解决复杂任务主要有两种独立方法，各有局限：[来源](https://systems-analysis.ru/int/index.php?title=ReAct_Prompting_%E2%80%94_ReAct_%E6%8F%90%E7%A4%BA%E6%B3%95&oldid=6038)
- **思维链（CoT）**：模型生成逐步的逻辑推理，但完全依赖内部知识，常导致事实错误和"幻觉"。
- **仅行动规划（Act-Only）**：模型生成行动与外部环境交互，但缺乏高层规划，处理错误时表现不佳。
ReAct 结合了两者的优点：模型能够推理，并将其推理"植根于"外部世界的事实，根据反馈调整行动。

# 3 ReAct 的工作机制："思考—行动—观察"循环

## 3.1 工作机制

[Pasted image 20260413144128.png]
...
Observation 1: 交流电（AC）是由一位著名发明家开发和推广的。
Thought 2: 现在我需要找出该发明家的出生地。
Action 2: Search[交流电发明家出生地]
Observation 2: 该发明家出生于斯米连村（现属克罗地亚）。
Thought 3: 斯米连不是首都。
...
| 元素 | 是否改变环境 | 主要作用 | 示例 |
|------|--------------|----------|------|
| **Thought（思考）** | ❌ 不改变环境 | 作为"内心独白"，规划下一步、整理信息、解释为何调用某个工具 | "我需要先查一下2024年奥运会的举办城市。" |
...
某些框架（如基于 LCEL 的运行图）所做的，本质上就是将这个“循环 + 解析 + 工具调用”**封装成一个可复用的运行逻辑**，并提供不少内置工具和 Prompt 模板。