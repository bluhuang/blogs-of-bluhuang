...
ReAct（Reasoning + Acting）是由研究机构于 2022 年提出的智能体框架，核心创新在于让大语言模型交替生成**推理轨迹**与**任务操作**，模拟人类在解决问题时的“思考-行动-观察”循环。
...
`User: 问题 → LLM: 一次性生成答案`
即便加入了 Memory、RAG，智能体也仅仅多了“能记”和“会查”的能力：
- Memory：记住过去发生的内容（多轮对话、历史任务状态）
- RAG：在回答前查询知识库或互联网

但这仍然是“问一答一”的模式，缺乏真正的多步决策与行动能力。

ReAct（Reasoning + Acting）正是为解决这一问题而提出的：在推理过程中，显式地交替输出“思考内容（Thought）”和“行动指令（Action）”，再利用环境反馈（Observation）更新后续推理。

> 一句话概括：**ReAct 让大语言模型一边进行自我推理，一边调用工具，是一种更高级的提示方法。**

# 2 为什么需要 ReAct

在 ReAct 出现之前，利用大语言模型解决复杂任务主要有两种独立方法，各有局限：

- **思维链（CoT）**：模型生成逐步的逻辑推理，但完全依赖内部知识，常常导致事实错误和“幻觉”。
- **仅行动规划（Act-Only）**：模型生成行动与外部环境交互，但缺乏高层规划，在错误处理时表现不佳。

ReAct 将两者的优点结合：模型能够进行推理，并使其推理“植根于”外部世界的事实，根据反馈调整后续行动。
...
- **可解释性**：生成的推理轨迹让人类能够理解决策过程。
- **减少幻觉**：基于外部事实而非内部知识。

ReAct 让大语言模型一边进行自我推理，一边调用工具，是一种更高级的提示技术。

# 4 ReAct 实现

下面展示一个简化版 ReAct 循环（伪代码，逻辑关键）。
...
LangChain 所做的，本质上就是将这个“循环 + 解析 + 工具调用”**封装成一个可复用的 LCEL 运行图**，并提供大量内置工具和 Prompt 模板。