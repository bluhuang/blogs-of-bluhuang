...
ReAct（Reasoning + Acting）是由某研究机构于 2022 年提出的智能体框架，核心创新是让 LLM 交替生成**推理轨迹**与**任务操作**，模拟人类解决问题时的“思考-行动-观察”循环。

### 🧠 前言：从 CoT 到 ReAct
COT：Chain-of-Thought，思维链

在仅有基础对话能力时，大模型更像一个“一次性回答机”：
`User: 问题 LLM: 一次性生成答案`
即便加入 Memory、RAG，系统也只是多了“能记”和“会查”：
- Memory：记住过去的状态（多轮对话、历史任务状态）
- RAG：在回答前查询知识库或互联网

但这仍然是“问一答一”的模式，缺少真正的多步决策与行动能力。

ReAct（Reasoning + Acting）正是为了解决这一问题提出：在推理过程中，显式地交替输出“思考内容（Thought）”和“行动指令（Action）”，再利用环境反馈（Observation）更新后续推理。

> 一句话概括：ReAct 让 LLM 一边自言自语地推理，一边调用工具，是一种更高级的 Prompting 技术。

# 2 为什么需要 ReAct

在 ReAct 出现之前，利用 LLM 解决复杂任务主要有两种彼此独立的方法，各有局限：

- **思维链（CoT）**：模型生成逐步的逻辑推理，但完全依赖内部知识，常导致事实错误和“幻觉”。
- **仅行动规划（Act-Only）**：模型生成行动与外部环境互动，但缺乏高层规划，处理错误时表现不佳。

ReAct 结合了两者的优点：模型能够进行推理，并将其推理“植根于”外部世界的事实，根据反馈调整行动。

# 3 ReAct 的工作机制：“思考—行动—观察”循环

## 3.1 工作机制

![](/images/AI/0 基础/Agent/Pasted image 20260413144128.png)
...
| 元素 | 是否改变环境 | 主要作用 | 示例 |
|------|-------------|----------|------|
| **Thought（思考）** | ❌ 不改变环境 | 作为“内心独白”，规划下一步、整理信息、解释调用某个工具的原因 | “我需要先查一下2024年奥运会的举办城市。” |
...
| **Final Answer** | ❌ 自身不再行动 | 标志推理/行动序列结束，给出最终回答 | “2024年奥运会” |

## 3.3 ReAct 的优势

- **信息获取**：与外部环境交互，获取实时信息。
- **动态调整**：根据环境反馈更新操作计划，处理异常情况。
- **可解释性**：生成的推理轨迹使人类能理解决策过程。
- **减少幻觉**：基于外部事实而非内部知识。
...
1. **构建 prompt**：系统提示 + 问题 + 之前的 Thought/Action/Observation 轨迹；
2. **让 LLM 输出下一步**：
   - 要么继续输出 Thought + Action；
   - 要么直接给出 Final Answer；
3. **解析 Action**，调用对应工具，补上 Observation；
4. **循环直到终止**。

LangChain 所做的，本质上就是把这个“循环 + 解析 + 工具调用”**封装成一个可复用的 LCEL 运行图**，并提供丰富的内置工具和 Prompt 模板。