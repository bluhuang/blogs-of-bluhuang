...
1. **问题背景**：大模型被“关在盒子里”，只能输入输出文字，无法与外部世界交互。Tool 给大模型“装上手”，让其能执行真实操作。
...
让 AI 帮你整理电脑里的文件，它却给你写出一整套整理方案，分类规则头头是道，但最后还得你自己一个个打开文件夹、手动移动；让它帮你查今天的天气，它说「我的知识截止到某年某月，无法获取实时天气」；让它帮你订明天的机票，它只能告诉你去哪个网站、怎么操作，然后你默默打开了订票网站。
...
很多同学以为「Agent 就是更厉害的大模型」。这个理解有些偏差。**Agent 本质上是我们开发者写的一套程序**，它不是更聪明的 AI，而是一个全程在线的「调度员」，负责在用户、大模型、工具之间协调任务、传递信息、推进流程。
...
大模型不是程序员，它不会去扫描你的代码库，发现「哦你这里有个 `get_weather` 函数」。它能读的，只有你放进它对话上下文里的文字。
...
这部分 **大模型看不到**，由 Agent 最终执行。大模型只负责「决策调用哪个工具、传什么参数」，至于这个工具内部如何实现，大模型完全不关心。
...
这段代码比较常见，就是调了一个天气 API，把结果整理成字典返回。注意：**大模型看不到这段代码**，它只负责决定「要调这个工具、传什么参数」，至于你用什么 API、代码怎么写，大模型完全不关心。
...
* **`parameters` 字段**：定义了两个参数， `city` （城市名，字符串类型）和 `date` （日期，字符串类型），两个都在 `required` 里，也就是「必填格子」，大模型构造调用指令时不会漏填。每个参数的 `description` 里还给了例子（「北京、上海、广州」「2025-03-21」），这是一个好习惯，大模型按什么格式填参数全靠这里的说明。
...
后面几章会继续深入这条链路：
* **Function Calling**：大模型是怎么把「我要调哪个工具、传什么参数」这个决策，用标准化格式告诉 Agent 的，这是工具调用的底层机制，本章我们只是看到了结果，下一章讲清楚原理。
