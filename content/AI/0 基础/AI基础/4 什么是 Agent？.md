...
6. **Agent vs Workflow**：
   - Workflow：决策者是用户（硬编码），执行路径固定，适合重复性高、步骤明确的场景。
...
前两章讲了大模型的底层逻辑，也讲了怎么写好 Prompt。到现在，已经知道大模型是个"超级大脑"，能理解意图、会逻辑推理、能生成高质量内容。

但它只有嘴、没有手：它能说"你应该查一下慢查询日志"，但它自己查不了；它能分析"这个 API 可能是瓶颈"，但它发不了报警、关不了开关、也打不开任何一个文件。

学会了 Prompt，能精准控制大模型的输出，但有一个问题 Prompt 解决不了：就算大模型能给出完美的分析，它依然没法主动去查数据库、发通知、调用 API。"说得再好"，也只是在说。
...
* **自主（Autonomously）**：不需要人在每一步发指令。用户给出一个目标，它自己判断下一步做什么
...
大模型单独使用，相当于一个坐在椅子上只会出主意的顾问。用户问什么它答什么，但它不会起身去做任何事。Agent 是给这个顾问配了一双手：它不仅能给建议，还能拿起电话查数据、发邮件、去仓库核实情况，然后根据新信息继续推进。
...
传统脚本和 Workflow 是写死的逻辑，"如果 CPU > 80% 就发警报，如果连接超时就重启服务"。这些流程是提前写好的，遇到没预料到的情况，它们不知道该怎么办。一个新类型的故障，脚本一行代码都没有，Workflow 一个分支都没有，就是束手无策。

Agent 的价值在于：把大模型的 **灵活推理能力** 和真实工具的 **执行能力** 结合在一起，让系统能处理那些 **无法提前穷举所有情况** 的复杂任务。
...
* **动态决策**：每一步该怎么走，是大模型实时推理出来的，不是提前写好的。发现是慢查询，它就去查慢查询日志；如果发现是网络问题，它就会去查网络相关的工具。路径是活的，不是死的。

* **何时停止**：大模型判断任务已完成时输出"最终答案"，或者达到预设的最大轮数上限。两个条件任意满足一个，循环结束。

这就是"自主"的含义：用户给出任务目标，它自己决定怎么走到终点。
...
![](<attachments/什么是 Agent？-9b779638ec1f5dcada30ce8c1b298fef.png>)
...
**Workflow（工作流）** 是提前写好所有步骤和分支逻辑的流程：先做 A，再做 B，如果 B 的结果是 X 就走流程 C，否则走流程 D。大模型只是其中某个步骤里被调用一次，整体流程是硬编码的。

**Agent** 是只给任务目标，大模型自己实时决定每一步做什么、调用什么工具、根据结果决定