...
![什么是 Agent？](什么是 Agent？-eec824743f1dba3aab176b28c318a555.png)
...
大模型单独使用，相当于一个坐在椅子上只会出主意的顾问。你问什么它答什么，但它不会起身去做任何事。Agent 是给这个顾问配了双手：他不仅能给建议，还能拿起电话查数据、发邮件、去仓库核实情况，然后根据新信息继续推进。
...
3. **没有持久记忆**：每次调用对模型都是全新的，不能积累任务中间状态。
...
![核心组件](什么是 Agent？-096af9599dfc28d36ce8c9586d3b51b1.png)

1. **大模型（LLM），大脑**：负责理解任务、分析当前状态、决定下一步做什么。所有的"推理"都在这里发生，该查什么、查完之后怎么解读、下一步往哪走，全是大模型来判断。

2. **工具（Tools），双手**：一个个可以被调用的真实函数：查数据库、调用搜索引擎、读写文件、发通知、调用 API……大模型告诉 Agent "调用这个工具、传这些参数"，代码层面真正去执行。工具的结果会返回给大模型，供它继续推理。大模型不"直接"操作外部系统，它通过工具来"动手"。
...
4. **执行循环（Loop），节拍器**：Agent 的"引擎"。不断重复"思考 → 行动 → 观察"这个循环，直到任务完成。没有这个循环，Agent 只是一次性的问答，无法串联多步骤。这个循环是 Agent 和普通大模型调用之间最根本的区别。
...
![ReAct 循环](什么是 Agent？-3d73134d1ae1ee3417e86d412f3d0e72.png)
...
![Plan and Execute 的痛点](什么是 Agent？-24f0a668b5e94edcb36dba3a103ba662.png)
...
![Plan and Execute 概览](什么是 Agent？-9b779638ec1f5dcada30ce8b1c298fef.png)
...
![Plan and Execute 执行流程](什么是 Agent？-3f10a22a70a34464b0b4dee0e977eb82.png)
...
![完整流程](什么是 Agent？-2c18f6e922ac932e787d79bf617d8e43.png)
...
![Agent vs Workflow](什么是 Agent？-11c6849028bf3031b040e59eff10ffc3.png)
