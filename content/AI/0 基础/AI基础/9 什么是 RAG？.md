...
***
前几章我们把 Agent 的工具体系搭清楚了：Function Calling 让大模型能开口下指令，MCP 让工具标准化接入，Skills 让专业知识按需加载。至此，Agent 能干活了，工具也能跑起来了。

但有一个根本问题，前面几章都没正面回答： **大模型压根看不到你的私有数据，怎么办？**

你的团队内部有几千页运维手册、几百个历史故障报告、一套只有内部才知道的排查 SOP，这些大模型从未见过。你让它帮你做 OnCall 排查，它能给出很通用的思路，但就是无法给出针对你们系统的精准答案。
