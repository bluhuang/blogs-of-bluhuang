...
企业内部有几千页运维手册、几百个历史故障报告、一套只有内部才知道的排查 SOP，这些大模型从来没见过。你让它帮你做 OnCall 排查，它能给出很通用的思路，但就是没法给出针对你们系统的精准答案。
...
大模型是用历史数据训练的，训练完之后它的知识就冻结了。比如某个大模型的训练数据截止到某个时间点，之后发生的事情它一概不知。当你问「系统今天凌晨发生了什么故障」，它根本没有这份信息，只能瞎猜。

**第二类：私有数据**

企业的内部文档、产品设计方案、运维手册、客服问答库，这些都是保密的，不可能出现在大模型的训练数据里。模型从没见过这些内容，自然没法基于它们来回答问题。

**第三类：实时数据**

今天刚写完的技术方案、刚产生的 bug 日志、刚更新的配置文件，这些是动态变化的，即便大模型更新了训练数据也追不上。Agent 需要访问的，往往就是这类「最新鲜」的数据。
...
企业的内部文档可能有几百个文件，光是运维手册就几十万字，根本塞不进去。更别提那些有几千个历史工单的故障数据库了。
