...
2. **为什么需要 RAG**：大模型知识静态、封闭（有截止日期、无私有数据、无实时数据），全量塞入 Prompt 受限于上下文窗口、成本高、注意力稀释。RAG 实现“按需检索 + 动态注入”。
...
大模型是用历史数据训练的，训练完之后它的知识就冻结了。例如，某个模型的训练数据截止到 2025 年某个时间点，之后发生的事情它一概不知。你去问它「我们系统今天凌晨发生了什么故障」，它根本没有这份信息，只能瞎猜。
...
这就是在 MCP 和 Agent 章节里预告 RAG 时所说的： **「把知识库检索封装成工具」**。
