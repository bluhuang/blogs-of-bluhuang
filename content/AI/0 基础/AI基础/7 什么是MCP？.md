...
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某个AI桌面应用、某个代码编辑器），负责发起任务。
...
假设你正在开发一个 Agent，需要它能读取某个代码仓库、查询公司数据库、操作本地文件系统、还能发团队通信工具消息。
...
光这四件事，就够你忙好几天。更麻烦的是，这些集成代码只能用在你这个项目里，团队中其他人做类似的 Agent，还得重新来一遍。

如果你的 Agent 还需要同时支持多个大模型（今天接某个AI、明天接另一个AI、后天接某个模型），问题就更大了：

**10 个工具 × 5 个大模型 = 50 套集成代码**

不同模型的 Function Calling 格式还可能有细微差别，你要针对每个模型写适配层。哪天某个代码仓库的 API 升级了，50 套代码你一个个去改，这还是基础设施代码，不是真正做产品该花时间的地方。

整个生态里，无数开发者都在重复做着同一件事：把常见的服务（代码仓库、团队通信工具、数据库、文件系统……）接入自己的 AI 应用。每个人写一套，互相之间完全无法复用。这就是没有统一标准时的现实，大家都在重复造同一个轮子。
...
Host 就是你最终使用的那个 AI 应用，可以是某个AI桌面应用、带 AI 功能的某个代码编辑器、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。

### Client（客户端）

Client 是 Host 内部的一个组件，专门负责管理和 MCP Server 的连接。你可以把它理解为一个「连接器」，Host 说「我要调用代码仓库工具」，Client 就负责找到对应的 MCP Server、建立连接、发送请求、拿回结果。
...
用「帮我查一下某个代码仓库最近的 commit」这个任务，走一遍完整的 MCP 调用流程：
...
1. **用户提问**：在 Host（比如某个AI桌面应用）里输入「帮我查一下某个代码仓库最近的 commit」
...
5. **Server 执行**：代码仓库 MCP Server 调用代码仓库 API，拿到最近的 commit 列表

6. **结果回传**：Server 把结果按 MCP 协议格式回传给 Client，Client 转交给 Host

7) **Host 生成回复**：大模型拿到结果，整理成自然语言回复给用户

整个过程中，Host 和背后的大模型完全不需要知道代码仓库 API 的任何细节，它只管说「我要调代码仓库工具」，剩下的事情 MCP Server 全权负责。这就是「解耦」的价值：工具的实现细节，和 AI 的调用决策，完全分离。
...
* **MCP 是什么**：某公司推出的开放标准协议，定义了 AI 应用和工具服务之间如何标准化通信，