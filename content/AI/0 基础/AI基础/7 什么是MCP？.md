...
1. **MCP 是什么**：某AI公司推出的开放标准协议（Model Context Protocol），定义了 AI 应用与工具服务之间的标准化通信方式，被誉为“AI 工具世界的 USB‑C”。
2. **为什么需要 MCP**：解决没有统一标准时，每个工具需单独集成、多模型场景下 N×M 套重复代码的问题。MCP 使工具写一次、全平台可用，将工作量降为 N+M。
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某AI桌面应用、某代码编辑器），负责发起任务。
...
如果你的 Agent 还需要同时支持多个大模型（今天接 GPT-4、明天接 GPT-5、后天接 Qwen），问题就更大了：
...
这就是某AI公司在 2024 年 11 月推出 **MCP（Model Context Protocol，模型上下文协议）** 的背景。
...
Host 就是你最终使用的那个 AI 应用，可以是某AI桌面应用、带 AI 功能的某代码编辑器、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
1. **用户提问**：在 Host（比如某AI桌面应用）里输入「帮我查一下 React 仓库最近的 commit」
...
* **MCP 是什么**：某AI公司推出的开放标准协议，定义了 AI 应用和工具服务之间如何标准化通信，是 AI 工具世界的「USB-C」。
