...
如果你的 Agent 还需要同时支持多个大模型（今天接一个模型、明天接另一个、后天再接一个），问题就更大了：
...
举个生活中的例子：在 USB-C 统一之前，各家设备厂商各有各的充电口，各品牌之间完全不兼容，出门得带三四根不同的充电线。USB-C 出来之后，只要设备支持 USB-C，任何 USB-C 的线都能用，充电器、数据线全部通用。
...
Host 就是你最终使用的那个 AI 应用，可以是带 AI 功能的桌面应用、带 AI 功能的编辑器、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
1. **用户提问**：在 Host（比如某个 AI 桌面应用）里输入「帮我查一下 React 仓库最近的 commit」
