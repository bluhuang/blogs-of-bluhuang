...
1. **MCP 是什么**：开放的标准化协议（Model Context Protocol），定义了 AI 应用与工具服务之间的标准化通信方式，被誉为“AI 工具世界的 USB‑C”。
...
假设你正在开发一个 Agent，需要它能读取代码仓库、查询数据库、操作本地文件系统、还能发送消息。
...
整个生态里，无数开发者都在重复做着同一件事：把各种常见服务接入自己的 AI 应用。每个人写一套，互相之间完全无法复用。这就是没有统一标准时的现实，大家都在重复造同一个轮子。

***

## MCP 的出现：给 AI 工具世界定一个标准

这就是推出 **MCP（Model Context Protocol，模型上下文协议）** 的背景。

MCP 要解决的核心问题，可以用一句话概括： **把工具的「写好」和「用起来」彻底拆开。**

![](什么是MCP？-d3a0c94d2277a61d5dbb04adf28824d7.png)

举个生活中的例子：在 USB-C 统一之前，各家设备各有各的接口，各品牌之间完全不兼容，出门得带三四根不同的线。USB-C 出来之后，只要设备支持 USB-C，任何 USB-C 的线都能用。

**MCP 对于 AI 工具世界的意义，就和 USB-C 对于接口的意义一模一样。**
...
4) **Client 路由到对应 Server**：Client 根据工具名，找到负责相应能力的 MCP Server，把请求发过去

5. **Server 执行**：MCP Server 调用对应 API，拿到最近的 commit 列表

6. **结果回传**：Server 把结果按 MCP 协议格式回传给 Client，Client 转交给 Host

7) **Host 生成回复**：大模型拿到结果，整理成自然语言回复给用户

整个过程中，Host 和背后的大模型完全不需要知道具体 API 的任何细节，它只管说「我要调那个工具」，剩下的事情 MCP Server 全权负责。这就是「解耦」的价值：工具的实现细节，和 AI 的调用决策，完全分离。
...
* **MCP 是什么**：开放的标准化协议，定义了 AI 应用和工具服务之间如何标准化通信，是 AI 工具世界的「USB-C」。
