...
- **只有MCP，没有Function Calling**：  
    Host能通过MCP调用工具，但大模型输出的工具名和参数可能不统一（比如“用check_the_weather工具，城市=上海明天”），Host没法可靠地转换成MCP请求。
...
如果你的 Agent 还需要同时支持多个大模型（今天接一个、明天接另一个），问题就更大了：
...
这就是某机构在 2024 年 11 月推出 **MCP（Model Context Protocol，模型上下文协议）** 的背景。
