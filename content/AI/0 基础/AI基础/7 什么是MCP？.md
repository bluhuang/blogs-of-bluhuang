...
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某AI桌面应用、VS Code），负责发起任务。
...
如果你的 Agent 还需要同时支持多个大模型（今天接某模型、明天接另一模型、后天接又一模型），问题就更大了：
...
Host 就是你最终使用的那个 AI 应用，可以是某AI桌面应用、带 AI 功能的 VS Code、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
