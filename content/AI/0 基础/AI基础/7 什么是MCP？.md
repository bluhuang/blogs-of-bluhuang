...
假设你正在开发一个 Agent，需要它能读取 GitHub 代码仓库、查询公司数据库、操作本地文件系统、还能发即时通讯工具消息。
...
如果你的 Agent 还需要同时支持多个大模型（今天接一个模型、明天接另一个、后天再接另一个），问题就更大了：
...
整个生态里，无数开发者都在重复做着同一件事：把 GitHub、即时通讯工具、数据库、文件系统……这些常见服务接入自己的 AI 应用。每个人写一套，互相之间完全无法复用。这就是没有统一标准时的现实，大家都在重复造同一个轮子。

***

## MCP 的出现：给 AI 工具世界定一个标准

这就是某 AI 公司在 2024 年 11 月推出 **MCP（Model Context Protocol，模型上下文协议）** 的背景。
...
1. **用户提问**：在 Host（比如某 AI 桌面应用）里输入「帮我查一下 React 仓库最近的 commit」
