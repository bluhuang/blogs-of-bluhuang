...
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某桌面 AI、某编辑器），负责发起任务。
...
假设你正在开发一个 Agent，需要它能读取某代码仓库、查询公司数据库、操作本地文件系统、还能发消息到协作平台。
...
如果你的 Agent 还需要同时支持多个大模型（今天接某模型、明天接另一模型、后天接另一模型），问题就更大了：

**10 个工具 × 5 个大模型 = 50 套集成代码**

不同模型的 Function Calling 格式还可能有细微差别，你要针对每个模型写适配层。哪天某个工具的 API 升级了，50 套代码你一个个去改，这还是基础设施代码，不是真正做产品该花时间的地方。
...
这就是某公司在某年某月推出 **MCP（Model Context Protocol，模型上下文协议）** 的背景。
...
Host 就是你最终使用的那个 AI 应用，可以是某桌面 AI、带 AI 功能的编辑器、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
Server 是对外暴露工具能力的轻量级服务程序。一个 MCP Server 通常负责一类工具，比如专门处理某仓库的 MCP Server、专门操作本地文件的 MCP Server、专门查询数据库的 MCP Server。
...
用「帮我查一下某仓库最近的 commit」这个任务，走一遍完整的 MCP 调用流程：

![](什么是MCP？-5f1c866d5bd35e64c756dd9f6ed5ee51.png)

1. **用户提问**：在 Host（比如某桌面 AI）里输入「帮我查一下某仓库最近的 commit」
2. **Host 分析任务**：大模型判断需要调用某工具，生成 Function Call 格式的调用指令
3. **Client 接收指令**：Host 把调用指令交给内置的 MCP Client
4. **Client 路由到对应 Server**：Client 根据工具名，找到负责该能力的 MCP Server，把请求发过去
...
整个过程中，Host 和背后的大模型完全不需要知道该工具 API 的任何细节，它只管说「我要调某个工具」，剩下的事情 MCP Server 全权负责。这就是「解耦」的价值：工具的实现细节，和 AI 的调用决策，完全分离。
