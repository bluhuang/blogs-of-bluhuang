...
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某桌面端、VS Code），负责发起任务。
...
* 自己研究这个工具的 API 文档
* 把 API 封装成函数
* 给每个函数写 Function Calling 定义（name、description、parameters）
* 处理认证、错误处理、数据格式转换
...
Host 就是你最终使用的那个 AI 应用，可以是某桌面端、带 AI 功能的 VS Code、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
1. **用户提问**：在 Host（比如某桌面端）里输入「帮我查一下 React 仓库最近的 commit」
