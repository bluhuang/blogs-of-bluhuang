...
这就是 MCP（Model Context Protocol，模型上下文协议）于 2024 年 11 月推出的背景。
...
Host 就是你最终使用的那个 AI 应用，可以是桌面客户端、带 AI 功能的 VS Code、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
1. **用户提问**：在 Host（比如桌面客户端）里输入「帮我查一下 React 仓库最近的 commit」
