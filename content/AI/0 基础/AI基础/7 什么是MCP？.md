...
不同模型的 Function Calling 格式还可能有细微差别，你要针对每个模型写适配层。哪天某工具的 API 升级了，50 套代码你一个个去改，这还是基础设施代码，不是真正做产品该花时间的地方。
...
MCP 要解决的核心问题，可以用一句话概括： **将工具的「开发」与「调用」彻底解耦。**

![](attachments/什么是MCP？-d3a0c94d2277a61d5dbb04adf28824d7.png)
...
![](attachments/什么是MCP？-b356960ae62209c7aff10d731b204617.png)
...
![](attachments/什么是MCP？-03570e6969812c513243d5e9e0d1bb44.png)
...
![](attachments/什么是MCP？-646507fd5709adf52c70db42fff09419.png)
...
![](attachments/什么是MCP？-a3ce8c4020e14607885212500f44c540.png)
...
用「帮我查一下某代码仓库最近的 commit」这个任务，走一遍完整的 MCP 调用流程：

![](attachments/什么是MCP？-5f1c866d5bd35e64c756dd9f6ed5ee51.png)

1. **用户提问**：在 Host（比如某AI桌面应用）里输入「帮我查一下某代码仓库最近的 commit」
...
整个过程中，Host 和背后的大模型完全不需要知道该 API 的任何细节，它只管说「我要调某代码仓库工具」，剩下的事情 MCP Server 全权负责。这就是「解耦」的价值：工具的实现细节，和 AI 的调用决策，完全分离。
