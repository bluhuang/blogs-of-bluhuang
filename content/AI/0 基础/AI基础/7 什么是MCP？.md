...
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某AI桌面应用、代码编辑器），负责发起任务。
...
假设你正在开发一个 Agent，需要它能读取代码仓库、查询公司数据库、操作本地文件系统、还能发通讯工具消息。
...
如果你的 Agent 还需要同时支持多个大模型（今天接某个模型、明天接另一个模型、后天再接其他模型），问题就更大了：

**10 个工具 × 5 个大模型 = 50 套集成代码**

不同模型的 Function Calling 格式还可能有细微差别，你要针对每个模型写适配层。哪天代码仓库 API 升级了，50 套代码你一个个去改，这还是基础设施代码，不是真正做产品该花时间的地方。

整个生态里，无数开发者都在重复做着同一件事：把常见平台、通讯工具、数据库、文件系统……这些常见服务接入自己的 AI 应用。每个人写一套，互相之间完全无法复用。这就是没有统一标准时的现实，大家都在重复造同一个轮子。
...
Host 就是你最终使用的那个 AI 应用，可以是某AI桌面应用、带 AI 功能的代码编辑器、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
用「帮我查一下代码仓库上某个项目最近的提交」这个任务，走一遍完整的 MCP 调用流程：

![](什么是MCP？-5f1c866d5bd35e64c756dd9f6ed5ee51.png)

1. **用户提问**：在 Host（比如代码编辑器AI扩展）里输入「帮我查一下代码仓库上某个项目最近的提交」
...
5. **Server 执行**：代码仓库 MCP Server 调用相应的 API，拿到最近的提交列表
