...
假设你正在开发一个 Agent，需要它能读取某个代码仓库、查询公司数据库、操作本地文件系统、还能发消息。
...
如果你的 Agent 还需要同时支持多个大模型（今天接一个模型、明天接另一个模型、后天接再一个模型），问题就更大了：
...
举个生活中的例子：在 USB-C 统一之前，各家手机厂商各有各的充电口，互不兼容，出门得带三四根不同的充电线。USB-C 出来之后，只要设备支持 USB-C，任何 USB-C 的线都能用，充电器、数据线全部通用。
...
Host 就是你最终使用的那个 AI 应用，可以是某桌面应用、带 AI 功能的 VS Code、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
Server 是对外暴露工具能力的轻量级服务程序。一个 MCP Server 通常负责一类工具，比如专门处理某个代码仓库的 MCP Server、专门操作本地文件的 MCP Server、专门查询数据库的 MCP Server。
...
1. **用户提问**：在 Host（比如某桌面应用）里输入“帮我查一下某个仓库最近的 commit”
2. **Host 分析任务**：大模型判断需要调用某个工具，生成 Function Call 格式的调用指令
3. **Client 接收指令**：Host 把调用指令交给内置的 MCP Client
4. **Client 路由到对应 Server**：Client 根据工具名，找到负责相应能力的 MCP Server，把请求发过去
5. **Server 执行**：相关 MCP Server 调用对应 API，拿到最近的 commit 列表
6. **结果回传**：Server 把结果按 MCP 协议格式回传给 Client，Client 转交给 Host
7. **Host 生成回复**：大模型拿到结果，整理成自然语言回复给用户

整个过程中，Host 和背后的大模型完全不需要知道具体 API 的任何细节，它只管说“我要调某个工具”，剩下的事情 MCP Server 全权负责。这就是“解耦”的价值：工具的实现细节，和 AI 的调用决策，完全分离。
