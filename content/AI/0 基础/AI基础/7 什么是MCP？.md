...
1. **MCP 是什么**：一种开放标准协议（Model Context Protocol），定义了 AI 应用与工具服务之间的标准化通信方式，被誉为“AI 工具世界的 USB‑C”。
2. **为什么需要 MCP**：解决没有统一标准时，每个工具需单独集成、多模型场景下 N×M 套重复代码的问题。MCP 使工具写一次、全平台可用，将工作量降为 N+M。
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如桌面客户端、VS Code），负责发起任务。
...
- **只有 MCP，没有 Function Calling**：  
    Host 能通过 MCP 调用工具，但大模型输出的工具名和参数可能不标准（比如“用 check_the_weather 工具，城市=上海明天”），Host 没法可靠地转换成 MCP 请求。
...
上一章讲完了 Function Calling，大模型通过标准化的 JSON 格式，告诉 Agent 要调用哪个工具、传什么参数。到这里，你已经理解了工具是什么、工具怎么被调用起来。
...
假设你正在开发一个 Agent，需要它能读取 GitHub 代码仓库、查询数据库、操作本地文件系统、还能发送消息。
...
光这四件事，就够你忙好几天。更麻烦的是，这些集成代码只能用在你这个项目里，团队里其他成员做类似的 Agent，还得重新来一遍。

如果你的 Agent 还需要同时支持多个大模型（今天接一个、明天接另一个、后天再接一个），问题就更大了：
...
整个生态里，无数开发者都在重复做着同一件事：把 GitHub、消息、数据库、文件系统……这些常见服务接入自己的 AI 应用。每个人写一套，互相之间完全无法复用。这就是没有统一标准时的现实，大家都在重复造同一个轮子。

***

## MCP 的出现：给 AI 工具世界定一个标准

这就是 MCP（Model Context Protocol，模型上下文协议）推出的背景。

MCP 要解决的核心问题，可以用一句话概括： **把工具的“写好”和“用起来”彻底拆开。**

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/%E4%BB%80%E4%B9%88%E6%98%AFMCP%EF%BC%9F-d3a0c94d2277a61d5dbb04adf28824d7.png)

举个生活中的例子：在 USB-C 统一之前，各家厂商各有各的充电口，标准不一，各品牌之间完全不兼容，出门得带三四根不同的充电线。USB-C 出来之后，只要设备支持 USB-C，任何 USB-C 的线都能用，充电器、数据线全部通用。

**MCP 对于 AI 工具世界的意义，就和 USB-C 对于充电口的意义一模一样。**

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/%E4%BB%80%E4%B9%88%E6%98%AFMCP%EF%BC%9F-b356960ae62209c7aff10d731b204617.png)
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/%E4%BB%80%E4%B9%88%E6%98%AFMCP%EF%BC%9F-03570e6969812c513243d5e9e0d1bb44.png)

### Host（宿主）

Host 就是你最终使用的那个 AI 应用，可以是桌面客户端、带 AI 功能的编辑器、或者你自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/%E4%BB%80%E4%B9%88%E6%98%AFMCP%EF%BC%9F-646507fd5709adf52c70db42fff09419.png)
...
学到这里，很多学习者会冒出一个问题：上一章学了 Function Calling，说大模型通过 Function Calling 告诉 Agent 调用哪个工具；这章又学了 MCP，说 Agent 通过 MCP 来调用工具。这两个东西，到底有什么区别？MCP 是不是把 Function Calling 给替代了？

完全不是。 **Function Calling 和 MCP 解决的是不同层面的问题，它们是配合关系，不是替代关系。** 我们来一步步理清楚。

**第一步：确认 Function Calling 工作在哪个层面**

Function Calling 解决的问题是：大模型做出“要调用这个工具”的决策之后，怎么把这个决策用标准化 JSON 格式传给 Agent。这是 **大模型和 Agent 之间** 的通信协议，负责“大模型怎么开口下指令”这一段。
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/%E4%BB%80%E4%B9%88%E6%98%AFMCP%EF%BC%9F-a3ce8c4020e14607885212500f44c540.png)

Function Calling 负责上半段：大模型用 Function Calling 格式告诉 Agent“调用哪个工具、传什么参数”。

MCP 负责下半段：Agent 通过 MCP 协议，找到对应的 MCP Server，把工具真正执行起来。

**用一个具体例子把两者串联起来**

还是查天气的场景，用户问“某地明天天气怎样”：

1. 大模型通过 **Function Calling** 返回调用指令，“调 check_weather，city=某地”。这是 Function Calling 层，大模型在开口下指令。
...
用“帮我查一下 GitHub 上某个仓库最近的 commit”这个任务，走一遍完整的 MCP 调用流程：

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/%E4%BB%80%E4%B9%88%E6%98%AFMCP%EF%BC%9F-5f1c866d5bd35e64c756dd9f6ed5ee51.png)

1. **用户提问**：在 Host（比如桌面客户端）里输入“帮我查一下某个仓库最近的 commit”
...
整个过程中，Host 和背后的大模型完全不需要知道 GitHub API 的任何细节，它只管说“我要调用 GitHub 工具”，剩下的事情 MCP Server 全权负责。这就是“解耦”的价值：工具的实现细节，和 AI 的调用决策，完全分离。
...
* **MCP 是什么**：一种开放标准协议，定义了 AI 应用和工具服务之间如何标准化通信，是 AI 工具世界的“USB-C”。
