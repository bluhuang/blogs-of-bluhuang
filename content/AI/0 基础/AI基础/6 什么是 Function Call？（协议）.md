...
上一章你已经知道，一个工具 = 函数本体 + 名称 + 描述 + 参数定义，大模型靠工具描述来判断是否调用这个工具。

但这里有个关键问题还没解决：**大模型做出「要调这个工具」的决策之后，它如何告诉 Agent 去执行？**

大模型只会“说话”，它的输出永远是文字。但 Agent 要调用工具，必须知道：调哪个工具？传什么参数？格式是什么？这中间的信息交换，靠什么来保证准确？

这就是 Function Calling 要解决的核心问题。在讲它是什么之前，我们先来看看没有它的时候，开发者需要面对怎样的困境。
...
听起来还好？但问题是，大模型是个自由发挥的选手，你很难约束它。你让它查「上海明天的天气」，它可能给你返回：
...
这就是传统 System Prompt 模式的本质问题， **全靠 AI 猜**：猜是否要调工具、猜参数格式、猜回复结构。猜对了正常运行，猜错了整个流程就崩溃。

而开发者呢？就得写一大堆正则表达式和解析代码，尝试从 AI 五花八门的回复中“扒”出工具名和参数。工具越多，解析代码比业务代码还长。而且换一个模型，还得重写一遍。这种开发体验无疑是一种噩梦。
...
打个比方：工具是外卖骑手（有姓名、有技能、能送餐），Function Calling 是美团的派单系统（规定了订单如何下达、骑手怎么接单、送达后如何确认）。骑手和派单系统缺一不可，但它们解决的是完全不同层面的两件事：骑手是「能力」，派单系统是「通信规范」。

所以 Tool 章讲的「工具四要素」，是从概念层告诉你一个工具应该有什么；本章讲的 Function Calling，是从协议层告诉你这些要素以什么标准化格式在大模型和 Agent 之间流转。
...
这一步是我们开发者要写的，相当于给大模型一份严格规范的工具说明书，大模型会完全按照这份说明书，判断是否调用工具、如何调用工具。
...
* `description` ：工具功能说明， **是大模型判断「要不要调这个工具」的核心依据**，写得越精准，大模型做出正确决策的概率越高

* `parameters` ：告诉大模型这个工具需要哪些参数、什么类型、什么格式，大模型会严格按照这里的约束来生成参数值

* `required` ：必填参数清单，如果用户未提供必填信息，大模型会主动询问用户，而不是随意猜测
...
注意 `function_call` 这个字段，它是一个固定标识，告诉 Agent： **这不是给用户看的回复，这是一条工具调用指令，你去执行它**。Agent 看到这个字段，就知道要去工具注册表里找到 `check_weather` 这个函数，把 `city` 和 `date` 作为参数传进去。

你会发现，这里的 `date` 字段大模型自动把「明天」转换成了 `2026-03-19` ，完全符合我们在工具定义里约定的 `YYYY-MM-DD` 格式。这就是标准化规范的威力，大模型知道格式要求，会自动转换，不再把模糊的「明天」直接丢给工具。

### 第三部分：工具返回结果（Tool Response），Agent 把结果回传给大模型

Agent 拿到大模型的调用指令后，去执行对应的工具，然后把工具的执行结果，按规范格式回传给大模型。大模型基于这个结果，决定下一步是继续调用其他工具，还是直接给用户返回最终答案。
...
| 开发成本   | 极高，需要写大量解析、纠错、重试逻辑     | 极低，直接解析固定格式，无需额外处理兼容问题                          |
| 模型兼容性  | 极差，换个模型就需重写整套提示词       | 极好，主流大模型通用，一套工具定义可跨模型使用                         |
