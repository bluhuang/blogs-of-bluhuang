...
有了 Function Calling，大家都很兴奋，开始给 AI 疯狂写各种工具（查天气、查 MySQL、读 Git 仓库等）。

但很快，新的问题出现了：**接口完全不兼容。**

某位开发者用 Python 写了个查数据库的工具，另一位用 Java 搞了个读本地文件的工具。由于没有统一的标准，你的 AI 应用想要接入这些现成的工具，就得挨个去适配它们千奇百怪的数据格式和通信逻辑。这就像你买了个新手机，却发现大家用的充电线有圆头、扁头、方头，简直让人崩溃。
...
举个例子，假设你要让 AI 帮你写一份项目日报。你手里虽然已经有了 MCP 提供的“读取数据库”和“读取任务管理工具”的标准化工具，但你每次跟 AI 聊天，还是得像个唐僧一样反复叮嘱。
...
这个问题问得非常好！其实很多人刚接触时都会有这个错觉。来掰扯掰扯它俩到底是什么关系、有什么区别：
...
### 实战加餐：以某款AI编程工具为例，看看 Skills 到底有多强？

光说不练假把式。今年（2026年）某AI公司发布的终端编程工具，直接把 Skills 机制带火了。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 正如前面所说，一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具中，它是这么写的：
