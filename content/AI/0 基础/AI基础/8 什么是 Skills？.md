...
6. **实战示例（某 AI 编码工具）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
...
### 实战加餐：以某 AI 编码工具为例，看看 Skills 到底有多强？

光说不练假把式。某机构发布的终端编程工具，直接把 Skills 机制带火了。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
该工具会自动扫描 `skills/` 目录，读取 `SKILL.md` 理解整个工作流，然后默默在后台调用脚本、压缩图片、移动文件，最后在终端里回复你：“搞定了！图片大小从 631KB 压缩到了 56KB，已经放在 public 目录啦！”
