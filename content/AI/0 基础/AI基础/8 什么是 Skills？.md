...
6. **实战示例（某编程工具）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
...
* **从工程上看：临时工与标准资产。** 你在对话框里敲的 Prompt 是“临时工”，上下文一长它就忘了，下次还得重敲。而 Skill 是一份写在项目目录里的配置文件（通常是 YAML 或 Markdown），它是可以提交到版本控制仓库里的 **代码资产**。它把个人经验变成了整个团队都可以直接复用的 **标准作业程序（SOP）**。
...
### 实战加餐：以某编程工具为例，看看 Skills 到底有多强？

光说不练假把式。某公司发布的终端编程工具，直接把 Skills 机制带火了。咱们就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 咱们刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具的实践中，它是这么写的：
