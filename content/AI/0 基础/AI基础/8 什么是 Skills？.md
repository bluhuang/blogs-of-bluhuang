...
6. **实战示例（某编程助手）**：在项目 `.skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
...
### 实战加餐：以某编程助手为例，看看 Skills 到底有多强？

光说不练假把式。某公司发布的终端编程神器，直接把 Skills 机制带火了。咱们就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 咱们刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。在示例中，它是这么写的：
...
3. **怎么使用？** 配置好之后，你在终端里，只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：
