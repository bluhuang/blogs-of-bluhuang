...
6. **实战示例（某AI编程工具）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
7. **核心价值**：Skills 让 AI 从“临时问一句”升级为“按标准化流程自动执行”，降低重复沟通成本，实现经验沉淀和团队协作。

***
### 第一阶段：AI 的“破壁人” ， Function Calling（函数调用）

先回想一下早期的 AI 对话系统。那时的它就像是一个 **被关在小黑屋里的超级学霸**：虽然上知天文下知地理，但他没网（没法查实时数据），也没手（没法帮你真正去执行操作，比如发邮件、查数据库）。
...
你想让 AI 查今天某个城市的天气，它自己查不到，但你可以给它提供一个 `get_weather` 的函数说明书。AI 看到你的需求和说明书后，不会直接回答天气，而是输出一段 JSON 代码：“我决定调用 `get_weather` ，参数是 `某城市` ”。 然后你的代码拿着这个参数去真正调用 API，查到结果后再喂给 AI，AI 最后总结成人类语言回复你。
...
有了 Function Calling，大家都很兴奋，开始给 AI 疯狂写各种工具（查天气、查数据库、读仓库等等）。

但很快，新的灾难来了： **接口完全不兼容。**

有人用 Python 写了个查数据库的工具，有人用 Java 搞了个读本地文件的工具。由于没有统一的标准，你的 AI 应用想要接入这些现成的工具，就得挨个去适配它们千奇百怪的数据格式和通信逻辑。这就像你买了个新手机，却发现大家用的充电线有圆头、扁头、方头，简直让人崩溃。
...
举个例子，假设你要让 AI 帮你写一份项目日报。你手里虽然已经有了 MCP 提供的“读取数据库”和“读取任务管理系统”的标准化工具，但你每次跟 AI 聊天，还是得像个唐僧一样反复叮嘱。
...
3. 提取任务进度（自动调用 mcp-task-reader 获取任务状态）
4. 生成图表和摘要
...
```
...
从上面的对比图能看出来，一个优秀的 Skill，结构是非常清晰的，通常分为两部分：
...
### 实战加餐：以某AI编程工具为例，看看 Skills 到底有多强？

光说不练假把式。某AI公司发布的终端编程工具，直接把 Skills 机制带火了。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具中，它是这么写的：
...
3. **怎么使用？** 配置好之后，在该工具的终端里，只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：

AI 会自动扫描 `skills/` 目录，读取 `SKILL.md` 理解整个工作流，然后默默在后台调用脚本、压缩图片、移动文件，最后在终端里回复你：“搞定了！图片大小从 631KB 压缩到了 56KB，已经放在 public 目录啦！”
