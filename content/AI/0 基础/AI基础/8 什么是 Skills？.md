...
为了让 AI 查今天深圳的天气，它自己查不到，但你可以给它提供一个 `get_weather` 的函数说明书。AI 看到你的需求和说明书后，不会直接回答天气，而是输出一段 JSON 代码：“我决定调用 `get_weather` ，参数是 `深圳`”。然后你的代码拿着这个参数去真正调用 API，查到结果后再喂给 AI，AI 最后总结成人类语言回复你。
