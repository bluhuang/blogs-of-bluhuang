...
6. **实战示例（某终端编程助手）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
7. **核心价值**：Skills 让 AI 从“临时问一句”升级为“按标准化流程自动执行”，降低重复沟通成本，实现经验沉淀和团队协作。

***
### 第一阶段：AI 的“破壁人” ， Function Calling（函数调用）

回想早期的 ChatGPT 时代。那时的它就像是一个 **被关在小黑屋里的超级学霸**：虽然上知天文下知地理，但它没网（没法查实时数据），也没手（没法帮你真正去执行操作，比如发邮件、查数据库）。
...
有了 Function Calling，大家都很兴奋，开始给 AI 疯狂写各种工具（查天气、查数据库、读版本仓库等等）。
...
举个例子，假设你要让 AI 帮你写一份项目日报。你手里虽然已经有了 MCP 提供的“读取数据库”和“读取任务跟踪系统”的标准化工具，但你每次跟 AI 聊天，还是得像个唐僧一样反复叮嘱。
...
tools: 
  - mcp-task-tracker-reader      # 挂载所需的 MCP 工具
  - mcp-database-query
---
...
3. 提取任务进度（自动调用 mcp-task-tracker-reader 获取任务状态）
4. 生成图表和摘要
...
```
...
从上面的对比图可以看到，一个优秀的 Skill，结构通常分为两部分：
...
光说不练假把式。某公司发布的终端编程工具，直接把 Skills 机制带火了。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具中，它是这么写的：
...
3. **怎么使用？** 配置好之后，在该编程助手的终端里，只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：

助手会自动扫描 `skills/` 目录，读取 `SKILL.md` 理解整个工作流，然后默默在后台调用脚本、压缩图片、移动文件，最后在终端里回复你：“搞定了！图片大小从 631KB 压缩到了 56KB，已经放在 public 目录啦！”
