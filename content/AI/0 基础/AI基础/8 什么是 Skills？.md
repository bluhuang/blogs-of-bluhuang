...
6. **实战示例（某 AI 编程助手）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
...
有了 Function Calling，大家都很兴奋，开始给 AI 疯狂写各种工具（查天气、查 MySQL、读 GitHub 等）。
...
从上面的对比图可以看到，一个优秀的 Skill，结构是非常清晰的，通常分为两部分：
...
可能看到这里，有读者心里犯嘀咕了：“我看你这 `SKILL.md` 里面，主体部分不还是写了一堆自然语言的步骤吗？这不就是个长一点的 Prompt（提示词）吗？跟我在对话框里敲字有什么区别？”
...
### 实战加餐：以某终端编程工具为例，看看 Skills 到底有多强？

光说不练假把式。某终端编程神器（如某 AI 编程助手）直接将 Skills 机制带火了。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 刚才说过，一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具中，它是这么写的：
...
3. **怎么使用？** 配置好之后，在该 AI 编程助手的终端里，只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：
