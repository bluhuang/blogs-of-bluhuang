...
咱们先回想一下早期的大模型。那时的它就像是一个 **被关在小黑屋里的超级学霸**：虽然上知天文下知地理，但他没网（没法查实时数据），也没手（没法帮你真正去执行操作，比如发邮件、查数据库）。
...
### 实战加餐：以某终端编程工具的 Skills 为例，看看 Skills 到底有多强？

光说不练假把式。今年（2026年）某团队发布的终端编程工具，直接把 Skills 机制带火了。咱们就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 咱们刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具里，它是这么写的：
