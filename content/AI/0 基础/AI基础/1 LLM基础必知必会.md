---
title: "LLM基础必知必会"
image: "/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4302.png"
categories: ["AI"]
author: "BluHuang"
...
- **Base 模型（基础模型）**  
  在海量、多样化的纯文本语料上进行训练，目标是最小化下一个词的预测误差。它本质上是一个“文本续写器”，生成的后续内容并不一定遵循对话或指令格式。例如，输入“你好，我”，它可能补全“是一个学生”，而非回答“有什么可以帮您”。
...
- **Chat 模型（对话模型）**  
  ...
  **补充解释：**
  - **有监督微调（SFT）**：一种用标注数据调整模型的方法，数据是“（用户指令，正确回复）”配对。模型在预训练（Base 模型）已掌握语言的基础上，通过比较自身输出与标准答案的差异，逐步调整参数，使新指令也能生成类似风格的回复。本质上是对标准答案的模仿。
  - **强化学习（RLHF，Reinforcement Learning from Human Feedback）**：在 SFT 之后，模型可能仍会生成不安全或不理想的回复。RLHF 引入一个由人类对大量不同回复的偏好打分训练得到的“奖励模型”。LLM 通过尝试生成回复，并根据奖励模型分数进行优化，学会更符合人类偏好的行为（例如更有用、更无害）。本质上是通过奖惩机制让模型自行摸索更好的回答策略。

### 1.3 多模态模型

多模态 LLM 同时处理文本和其他模态信息（图像、音频、视频）。通过在多种类型的数据上联合训练，模型能够学习不同模态之间的对齐关系，从而完成单文本模型无法实现的任务，例如：图片描述生成、根据视频内容回答问题、从音频中提取信息等。

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4305.png)
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4306.png)
...
**什么是模型推理？**  
模型推理是指利用**已经训练好**的模型，根据输入（如提示词）来生成新内容的过程。在此阶段，模型参数不再更新，仅进行前向计算。可将其理解为“模型在回答问题时的工作状态”。

**自回归生成的核心步骤（下图流程）：**

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4313.png)

1. **分词**：分词器将输入文本拆分为模型词汇表中的 token，每个 token 对应一个唯一 ID。
2. **嵌入**：每个 token ID 被转换为稠密的 embedding 向量，所有向量组成输入矩阵。
3. **Transformer 处理**：输入矩阵经过多层 Transformer 网络，每层包含自注意力机制和前馈网络，用于捕捉序列中 token 之间的依赖关系。
...
**更通俗的理解**：模型一次只预测下一个最合适的词，然后将其加入，再预测下一个，如此反复，像打字一样逐句写完。

### 2.2 提示词（Prompt）

提示词是与 LLM 交互的主要方式。通过精心设计的输入，可以引导模型生成期望的内容。许多开源模型支持 System Prompt（系统提示），用于设定模型的角色、行为准则和输出格式，提供更精细的控制。

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4307.jpeg)
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4308.png)
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4309.png)
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4310%201.png)

### 2.6 模型量化

模型量化将模型中的权重和激活值从高精度（如 32 位浮点数，FP32）转换为低精度（如 8 位整数，INT8）表示。

- **优点**：大幅减少模型体积和内存占用，降低推理计算量，使模型可在资源受限设备（如个人电脑、移动端）上运行。
- **缺点**：可能带来一定的精度损失。

常用的量化工具包括 `bitsandbytes` (bnb)、GPTQ、AWQ 等。

![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4311.png)
...
![](/images/AI/0%20%E5%9F%BA%E7%A1%80/AI%E5%9F%BA%E7%A1%80/IMG_4312.png)
