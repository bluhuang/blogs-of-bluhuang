...
Harness Engineering 这个词最早被正式叫响，是在 2026 年 2 月 5 号一篇博客里。作者是某位知名工程师。
...
博客发出来一周后，某知名公司紧接着也发布了一篇官方博客，标题就叫《Harness engineering: leveraging Codex in an agent-first world》。
...
你平时用那些对话工具也好，用代码助手也好，看到的都是一个对话框，输入一句话，下面给你输出一段话。但如果把这层外壳扒掉，里面其实就一个东西：一个特别大的参数文件，存在硬盘上，加载到显卡的内存里。
...
这就是为啥你用不同的 AI IDE，效果会差挺多。模型是同一个，但每家的上下文工程策略不一样，所以最后的体验也不一样。

讲到这儿，我想专门聊一下某家机构最近搞的 **Agent Skills**，因为它特别能体现「上下文工程不是塞得越多越好」这个理念。
