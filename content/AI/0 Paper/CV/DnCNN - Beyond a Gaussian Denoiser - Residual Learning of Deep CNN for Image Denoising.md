...
传统去噪方法（如 BM3D）基于图像的自相似性——在一张图像中寻找相似的图像块，然后加权平均去噪。这种方法虽然有效，但本质上是在“手工设计规则”，通用性有限，不同场景需要不同的参数配置。
...
DnCNN 的**损失函数**是：
$L=∥网络输出−真实噪声∥$
整个网络唯一的任务是：**输出一个纯噪声图 R(y)R(y)**。网络的每一层、每一个卷积核，都在为这个目标服务。

###### 2. 为什么网络会把“干净图像”丢掉？（核心原理）

输入是 y=干净图+噪声y=干净图+噪声。输入信号里混杂了两种成分：
