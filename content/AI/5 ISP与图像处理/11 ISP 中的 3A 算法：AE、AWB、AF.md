...
| 缩写 | 全称 | 解决的问题 |
|------|------|-----------|
| **AE** | Auto Exposure（自动曝光） | 画面太亮还是太暗？——控制进光量 |
| **AWB** | Auto White Balance（自动白平衡） | 颜色偏黄还是偏蓝？——校正色温 |
| **AF** | Auto Focus（自动对焦） | 画面模糊还是清晰？——控制镜头位置 |

这三个算法共同决定了最终图像的**亮度、颜色和清晰度**，是 ISP 中核心的控制逻辑。
![[Pasted image 20260707155825.png]]
### 1.2 3A 在 ISP 中的位置
...
**AE 的目标**：让画面的整体亮度达到一个“目标值”，使图像看起来明暗适中。
...
> 这里的直方图其实就是统计信息：**X 轴**显示**亮度级别**；**Y轴**显示**像素数量**

ISP 硬件每秒会生成数十帧这样的统计报告给 AE 算法。你一看形状就能判断画面状态：
