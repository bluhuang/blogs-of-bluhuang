...
**关键点**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。考察点不是"我用了某个向量库"，而是"为什么选它不选另一个，检索延迟要求多少，为什么 Top-K 设 5 不是 10"。
...
| 算法 | 原理 | 特点 |
|------|------|------|
| HNSW | 多层跳表图，从上层粗搜到下层精搜 | 查询快，内存占用大，向量库默认 |
| IVF | 先聚类，只搜最近的几个簇 | 可控精度，适合超大规模 |
| PQ（乘积量化） | 压缩向量维度，降低内存 | 内存省，精度有损 |

**进阶点**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），说明真正调过。
...
| 商业 Rerank API | 效果好，英文为主 |
| bce-reranker-base_v1 | 中文场景，轻量级 |

**关键点**："检索是粗筛快捞，Rerank 是精排提准。检索用 Bi-Encoder 快但粗，Rerank 用 Cross-Encoder 慢但准。先用检索从百万级捞 Top-20，再用 Rerank 精排取 Top-5，这是生产环境的标配流程。"
...
**核心要点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。能说出这个判断，说明有工程判断力。
