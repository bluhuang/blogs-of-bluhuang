...
**关键点**：不要只罗列七个步骤，要说明每一步的**关键决策点**。例如，选择某种向量数据库的原因、检索延迟要求、Top-K 为什么选某个值等。

### Q3: 传统RAG的三大痛点是什么？

考虑场景：RAG 检索到了正确信息，但生成的回答还是拼凑感很强。
...
**加分点**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），说明有过实际调参经验。
...
- Cross-Encoder 像技术审核：仔细看项目细节，判断真实匹配度。
...
**核心要点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。能说出这个判断，说明具备工程判断力。
