...
**② 私有数据无法触达**——组织的内部文档、客户数据、业务规则，这些 LLM 从来没见过，直接问就是胡说。
...
**技术答法**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。重点不是"我用了某向量库"，而是"我为什么选它，检索延迟要求多少，为什么 Top-K 设 5 不是 10"。

### Q3: 传统RAG的三大痛点是什么？

常见的技术讨论场景中，会问到："RAG 系统有没有遇到检索到了但答不对的情况？什么类型的问题答不好？"或者"RAG 检索到了正确信息，但生成的回答还是拼凑感很强，你怎么理解这个问题？"

#### 一个例子说清楚 RAG 撞墙在哪

假设你有一个组织内部知识库，里面全是项目文档、技术方案、会议纪要。有人问了一个问题：
...
比如"成员A和成员B在哪个项目上有合作"，这个信息可能分散在三个文档里——文档 1 提到成员A负责项目 X，文档 2 提到成员B参与了项目 X，文档 3 提到项目 X 的具体内容。传统 RAG 最多能捞到其中一个，很难同时把三个都捞出来并关联上。
...
"实体A收购了实体B"在文档 1 里，"实体B和实体C有合作"在文档 2 里。那实体A和实体C之间有没有间接关系？传统 RAG 答不了——因为每个 chunk 是独立 embedding 的，chunk 之间没有"关系"这个概念。
...
技术探讨中常问："向量检索和关键词检索有什么区别？"以及"Embedding 的原理是什么？为什么语义相似的文本向量距离近？"
...
| 算法 | 原理 | 特点 |
|------|------|------|
| HNSW | 多层跳表图，从上层粗搜到下层精搜 | 查询快，内存占用大，多数向量数据库默认 |
| IVF | 先聚类，只搜最近的几个簇 | 可控精度，适合超大规模 |
| PQ（乘积量化） | 压缩向量维度，降低内存 | 内存省，精度有损 |

**技术加分**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），说明你真调过。
...
**技术核心点**：能说清楚纯向量检索的三个问题、关键词检索的盲区（同义词无解），以及混合检索如何互补，合并策略用 RRF。向量检索并不是「什么都比关键词检索好」，两者是互补关系。正确的做法不是二选一，而是混合检索。

### Q5: 混合检索的权重怎么调？Top-K设多少？模糊问题怎么处理？

**Q：混合检索权重怎么调？向量检索和 BM25 各占多少？**
...
技术讨论中会问："你已经用混合检索了，为什么还要 Rerank？检索结果不够好吗？"
...
技术讨论中常问："RAG 检索到了正确信息，LLM 还是编造了不存在的内容，怎么办？"

幻觉是 RAG 项目最大的工程挑战，务必关注。
...
**技术核心点**：不要只说"用了 Prompt 约束"，要说出用了几种策略组合，以及效果如何。比如"Prompt 约束 + 输出自校验 + 温度调低，幻觉率从 30% 降到了 12%"。
...
**技术核心点**：能说出检索侧和生成侧的指标，并知道如何用 Ragas 等框架自动化评估。
...
**技术核心点**：能说清楚 chunk 大小的权衡（大→信息稀释，小→上下文丢失），以及 overlap 的作用。最好能举出实际调参的经历，比如"chunk_size 从 1000 降到 500，召回率提升了 15%"。
...
技术讨论中会问："你了解 Agentic RAG 吗？它和普通 RAG 有什么区别？"
...
**技术核心点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。能说出这个判断，说明你有工程判断力。
...
选 LightRAG。理由：新闻每分钟都在更新，增量插入是刚需；用户查询既有具体的（"某事件的最新进展"）也有偏全局的（"本周科技行业的热点话题"），LightRAG 的四种查询模式都能覆盖；而且新闻平台通常对成本敏感，LightRAG 的索引成本只有 GraphRAG 的一半左右。
