...
**① 知识截止**——训练数据有截止日期，昨天发生的事它不知道。询问"2026年3月发布的 XX 框架有什么特性"，它要么瞎编要么说不知道。
...
**关键点**：RAG 不是替代 LLM，而是给 LLM 补充外部知识。LLM 负责理解和生成，RAG 负责提供事实依据。

### Q2: RAG的完整链路是怎样的？

一个完整的 RAG 项目需要清晰描述从用户提问到最终回答的链路。
...
**技术要点**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。重点不是"用了 Milvus"，而是"为什么选 Milvus 不选 FAISS，检索延迟要求多少，为什么 Top-K 设 5 不是 10"。

### Q3: 传统RAG的三大痛点是什么？

传统 RAG 系统可能遇到检索到了但答不对的情况，或者检索到了正确信息，但生成的回答拼凑感强。
...
比如"两位员工在哪个项目上有合作"，这个信息可能分散在三个文档里——文档 1 提到某人负责项目 X，文档 2 提到另一人参与了项目 X，文档 3 提到项目 X 的具体内容。传统 RAG 最多能捞到其中一个，很难同时把三个都捞出来并关联上。
...
**技术要点**：能说出 HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），表明有过实际调优经验。
...
**关键点**：纯向量检索的三个问题、关键词检索的盲区（同义词无解），以及混合检索如何互补，合并策略用 RRF。向量检索并不是「什么都比关键词检索好」，两者是互补关系。正确的做法不是二选一，而是混合检索。

### Q5: 混合检索的权重怎么调？Top-K设多少？模糊问题怎么处理？

**混合检索权重怎么调？向量检索和 BM25 各占多少？**
...
**解释**：检索是粗筛快捞，Rerank 是精排提准。检索用 Bi-Encoder 快但粗，Rerank 用 Cross-Encoder 慢但准。先用检索从百万级捞 Top-20，再用 Rerank 精排取 Top-5，这是生产环境的标配流程。
...
RAG 检索到了正确信息，LLM 还是编造了不存在的内容，怎么办？幻觉是 RAG 项目最大的工程挑战。
...
**关键点**：不要只说"用了 Prompt 约束"，要说出用了几种策略组合，以及效果如何。比如"Prompt 约束 + 输出自校验 + 温度调低，幻觉率从 30% 降到了 12%"。
...
**关键点**：需要关注检索侧和生成侧的指标，并知道如何用 Ragas 等框架自动化评估。
...
**关键点**：chunk 大小需要权衡（大→信息稀释，小→上下文丢失），以及 overlap 的作用。最好能举出实际调参的经历，比如"chunk_size 从 1000 降到 500，召回率提升了 15%"。
...
**关键点**：Agentic RAG 适合复杂知识问答场景（法律、医疗、金融），简单问答用普通 RAG 就够了，别过度设计。具备工程判断力很重要。
