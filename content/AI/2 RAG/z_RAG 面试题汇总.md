...
**说明**：不要只背这七个步骤，要说清楚每一步的**关键决策点**。例如，为什么选某种向量数据库而不选另一种，检索延迟要求多少，为什么 Top-K 设某个值而不是另一个。
...
**关键参数**：HNSW 的核心参数 `ef_construction`（建图时搜索宽度，越大图质量越高但建图越慢）和 `M`（每个节点的邻居数，越大图越密但内存越大），实际调参中需要权衡。
...
**核心观点**："检索是粗筛快捞，Rerank 是精排提准。检索用 Bi-Encoder 快但粗，Rerank 用 Cross-Encoder 慢但准。先用检索从百万级捞 Top-20，再用 Rerank 精排取 Top-5，这是生产环境的标配流程。"
