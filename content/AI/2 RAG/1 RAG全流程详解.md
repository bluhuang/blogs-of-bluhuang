...
RAG 不是替代 LLM，而是给 LLM 补充外部知识。LLM 负责理解和生成，RAG 负责提供事实依据。
...
需要能对比不同分片策略的优劣，并知道如何根据文档类型（技术文档、对话记录、法律条文）选择策略。
...
中文场景选 bge-large-zh，因为 MTEB 中文榜单排名靠前，而且开源可以本地部署，不用走 API。如果是英文场景或对延迟不敏感，OpenAI 的 embedding 更方便。
...
召回和重排不合并为一个阶段的原因是：如果直接用 Cross-Encoder 对所有文档评分，时间复杂度 O(文档总数)，对百万级文档不可行。必须先用快速 Bi-Encoder 筛选候选集，再用 Cross-Encoder 精细排序。这是两阶段检索的标准范式。
...
不要只说“用了 Prompt 约束”，要说出多种策略组合及效果。例如“Prompt 约束 + 输出自校验 + 温度调低，幻觉率从 30% 降到了 12%”。
