...
**技术核心点**：RAG 不是替代 LLM，而是给 LLM 补充外部知识。LLM 负责理解和生成，RAG 负责提供事实依据。
...
| 模型 | 维度 | 特点 | 最佳场景 |
|------|------|------|----------|
| `text-embedding-3-small`（某商用） | 1536 | 通用性好，API 调用 | 通用英文 |
| `text-embedding-3-large`（某商用） | 3072 | 精度更高，成本略高 | 高精度英文 |
| `cohere embed-english-v3.0`（某商用） | 1024 | 支持多语言，压缩表示 | 多语言 |
...
**技术选型时通常会问**："你们用的什么 Embedding 模型？为什么选它？和某商用的 ada-002 对比过吗？"
...
| text-embedding-3-large（某商用） | 3072 | 效果好，但 API 调用有成本，中文不如 bge |
| text-embedding-3-small（某商用） | 1536 | 便宜，效果够用，英文场景首选 |

**维度越高越好吗？**  
不是。维度高→表达能力强但存储和检索成本也高。1024 维是当前性价比最好的选择，3072 维的检索效果提升有限但存储翻 3 倍。

**技术思考**："中文场景选 bge-large-zh，因为 MTEB 中文榜单排名靠前，而且开源可以本地部署，不用走 API。如果是英文场景或对延迟不敏感，某商用的 embedding 更方便。"
...
| 类别 | 模型 | 上下文长度 | 特点 |
|------|------|------------|------|
| 闭源 | GPT-4o（某商用） | 128K | 最强通用，成本高 |
| 闭源 | Claude-3.5 Sonnet（某商用） | 200K | 长上下文友好 |
...
常见技术问题："RAG 检索到了正确信息，LLM 还是编造了不存在的内容，怎么办？"
