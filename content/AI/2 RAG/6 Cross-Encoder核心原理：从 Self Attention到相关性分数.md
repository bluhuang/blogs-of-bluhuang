...
> Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。当讨论“为什么不用 Cross‑Encoder 直接做检索”时，核心理由是：Cross‑Encoder 的计算复杂度是 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
> Bi‑Encoder 与 Cross‑Encoder 选择的关键是**速度与精度的权衡**。能用 Bi‑Encoder 解决的场景不要上 Cross‑Encoder，能用规则解决的场景不要上模型。
...
> **选型建议**：选择 Rerank 模型时，应从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE；追求精度但不想自托管选 Cohere API；学习入门用 MiniLM。
...
Cross‑Encoder 将 Query 和 Document 拼接，Transformer 的每一层 Self‑Attention 都让 Query 的每个 token 与 Document 的每个 token 交互，能捕捉到否定词、语序、矛盾信息等深层语义关系。实验表明，Cross‑Encoder 重排序能使 nDCG@10 比纯向量检索提升 15–30%，显著减少 LLM 幻觉。
