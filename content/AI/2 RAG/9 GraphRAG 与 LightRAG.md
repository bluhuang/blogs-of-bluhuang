...
常见问题：如“你们 RAG 系统有没有遇到检索到了但答不对的情况？什么类型的问题答不好？”或者“RAG 检索到了正确信息，但生成的回答还是拼凑感很强，你怎么理解这个问题？”
...
常见问题：“RAG 检索到了正确信息，但为什么答不对？什么类型的问题答不好？”

**核心结论**：传统 RAG 擅长找相似的文本块，但不擅长**跨文档关联推理**、**全局归纳**和**关系理解**。以下三个问题是其本质限制，不是调参能解决的。

**① 碎片化检索——查到的是文本块，不是知识**

传统 RAG 把文档切成 chunk，每个 chunk 独立变成向量。检索时拿到的是“和问题最相似的文本块”。但很多问题的答案不是一个文本块能覆盖的，需要把多个文本块里的信息关联起来。比如“员工甲和员工乙在哪个项目上有合作”，信息可能分散在三个文档里——文档1提到员工甲负责项目X，文档2提到员工乙参与了项目X，文档3提到项目X的具体内容。传统 RAG 最多能捞到其中一个，很难同时把三个都捞出来并关联上。
...
“组织 A 收购了组织 B”在文档1里，“组织 B 和组织 C 有合作”在文档2里。那么组织 A 和组织 C 之间有没有间接关系？传统 RAG 答不了——因为每个 chunk 是独立 embedding 的，chunk 之间没有“关系”这个概念。
...
常见问题：“GraphRAG 和传统 RAG 本质区别是什么？RAG 这条线是怎么演进过来的？”
...
## 3. GraphRAG（发布于 2024.04）

### 3.1 什么是 GraphRAG？为什么会有 GraphRAG？

**GraphRAG** 这个词，其实有两层含义。

狭义上，它指的是**一篇于 2024 年 4 月发布的论文及其配套的开源项目**。论文名字叫 **《From Local to Global: A Graph RAG Approach to Query-Focused Summarization》**（从局部到全局：一种面向查询聚焦摘要的图 RAG 方法）。

广义上，它指的是**所有基于知识图谱做 RAG 的方法**，业界还有很多变种。

咱们这篇文章**主要讲的是这套 GraphRAG**，因为它目前影响力最大，开源社区也最活跃，几乎是所有 GraphRAG 方案的参照对象。后面如果没特别说明，我说的 GraphRAG 都指这一套。
...
假如你有一段文本：「员工甲在 2025 年创立了 X 公司，员工乙是 X 公司的 CTO，X 公司主要做自动驾驶业务。」

传统 RAG 会把这段话整体变成一个文本块，存到向量数据库里。你下次问「X 公司的 CTO 是谁？」，它靠语义相似度检索回这段话，然后让大模型去阅读理解，答出「员工乙」。
...
因为 GraphRAG 已经提前把文档里的实体和关系抽出来了，哪怕原文里「甲类药品属于 ACEI」和「严重肾功能不全者禁用」分散在两个地方，它们在图上也能通过「甲类药品」这个节点连起来。检索时不会丢掉因果关系，大模型也就不容易混淆了。
...
其实「知识图谱」这个东西并不新鲜，早在 2012 年就有人提出过 Knowledge Graph 的概念。把知识图谱和 RAG 结合的想法也有人尝试过，但之前都不温不火，为什么？
...
这套 GraphRAG 的索引阶段，可以拆成 **5 个核心步骤**。咱们用一个具体的例子来理解整个过程，比如说你有一本《三国演义》的电子书，要把它喂进 GraphRAG。
...
LLM 读完一段「人物A和人物B、人物C在桃园结义」的文本，会输出类似这样的结构化内容：

```
("entity"|人物A|人物|蜀汉的建立者，三国时期重要人物)
("entity"|人物B|人物|人物A的义弟，以忠义闻名)
("entity"|人物C|人物|人物A的义弟，勇猛善战)
("relationship"|人物A|人物B|桃园结义为兄弟，关系极为紧密|10)
("relationship"|人物A|人物C|桃园结义为兄弟，关系极为紧密|10)
("relationship"|人物B|人物C|桃园结义为兄弟|10)
```

**这一步跑完，所有文本块都会产出一堆实体节点和关系边**。

但注意，这里会有一个麻烦：**重复和歧义**。比如人物A可能在文本里被叫做「玄德」「先主」「刘皇叔」，LLM 抽取的时候可能把它们抽成了三个不同的实体。这个问题 GraphRAG 后面会通过「实体合并」和「社区检测」来一定程度上缓解，但不能完全解决。这也是我们第 4 章要讲的难点之一。
...
同一个实体（比如「人物A」）会在好多个文本块里被反复抽取，每次抽取的描述可能都不一样：「蜀汉的建立者」「人物B的义兄」「汉室宗亲」……
...
（补充：后来官方还推出了第三种叫 DRIFT Search 的模式，相当于 Local 和 Global 的混合版，咱们不展开讲，感兴趣的同学可以看官方文档。）

> **模式一：Local Search（本地搜索）**

**适用场景**：用户问的是关于**某个具体实体**的问题。比如「人物A都有哪些著名的计策？」「某组织的 CTO 是谁？」。

**工作流程**：

1. **定位入口实体**：先用问题的向量，在所有实体节点的向量索引里做相似度搜索，找到最相关的几个实体。比如问人物A，就先定位到「人物A」这个节点
...
- **第二，关系会碎片化**。假设有 100 份合同涉及某公司，其中 60 份用「IBM」，40 份用「国际商业机器」。你查「IBM 的合同」，系统只会返回那 60 份。**另外 40 份被挂在了另一个节点下，根本查不出来**。
...
比如老图谱里已经有了「某公司」这个节点，新文档里抽出来的叫「国际商业机器」。你需要识别出它们是同一个实体，然后合并。这就是**增量版本的实体消歧问题**，不仅要做一次性消歧，还要在每次增量时都做一遍。
...
有些团队的做法更激进：**索引时不做消歧**，只把所有实体直接加进去，哪怕同一个实体有四个节点也无所谓。然后**查询时再做合并**，或者**异步离线跑消歧任务**。
...
他们在论文里总结了两个关键局限。一个是**现有基于图的 RAG 方法（主要就是指之前的 GraphRAG）缺乏动态更新能力**，很难高效处理新信息的加入，数据一变整个图谱就得大改；另一个是**全局查询的「暴力搜索」实在太低效**，GraphRAG 的 Global Search 是 Map-Reduce 遍历所有社区摘要的，在大规模语料上又慢又贵。
...
举个例子你就明白了。如果你问「甲类药品的禁忌症是什么？」，低层关键词就是「甲类药品」「禁忌症」这种具体的实体和术语，它关注的是细节。而如果你问「国际贸易如何影响全球经济稳定？」，高层关键词则是「国际贸易」「全球经济稳定」「经济影响」这种抽象的主题和概念，它关注的是更宏观的东西。
...
LightRAG 的增量更新，**本质上就是「追加」两个字**。新文档来了，跑一下实体和关系抽取，然后把抽出来的新实体、新关系直接 upsert 到图和向量库里就行了。如果新实体和老实体有重合（比如都叫某种名称），那就合并节点的描述；要是完全是新实体，那就加个新节点。**整个过程完全不需要重新做社区检测、不需要重新生成摘要、更不需要全量重建索引**。
...
这个「关系关键词」是 LightRAG 的一个小巧思，你可以先记着。它描述的是这条关系所代表的主题或概念，比如「员工甲创立某公司」这条关系，对应的关键词可能就是「创业、企业创立」。
...
去重也发生在这一步。**同名的实体会被合并成一个节点，重复的关系会被合并成一条边**。比如「某人」在 82 个文本块里被抽取了 82 次，就会被合并成一个节点，所有 82 次的描述会被聚合成一个更完整的实体描述。
...
**一个细节值得提一下**：LightRAG 目前的增量处理对「语义冲突」并不敏感。比如 Day 1 抽到了「GPT-5.1 是某公司最新模型」，Day 2 又抽到「GPT-5.2 是某公司最新模型」，LightRAG 会把两条关系**同时保留**，不会自动判定哪条更新。如果你的业务需要处理这种时效性冲突，得自己在上层加逻辑。
...
**并不是**。根据 LightRAG 论文和社区的多项基准测试，**LightRAG 在四个主流数据集（Agriculture、Computer Science、Legal、Mix）上的四个评测维度（全面性、多样性、赋能性、整体）上，全部击败了 NaiveRAG、RQ-RAG、HyDE 甚至是之前的 GraphRAG**。尤其是在百万 token 级的大规模语料上，LightRAG 的优势更明显；在「多样性」这个维度上，LightRAG 更是大幅领先，这个得益于双层检索带来的信息广度。
...
- **金融研报深度分析**。分析师问：「某公司的哪些高管最近有变动，其中有没有出现在竞争对手公司的前任高管名单里？」这种跨实体、跨时间、跨文档的关联推理，只有 GraphRAG 能干得好。

**场景三：数据相对静态、更新不频繁的知识库**

因为 GraphRAG 的增量更新成本高，它最适合那些**建好之后长期服务**的静态知识库。像企业内部的规章制度库、历史档案、法律条文库、已经归档的项目资料、年度研究报告集，这类东西几个月甚至一年才更新一次。
...
## 7. 原理追问（GraphRAG/LightRAG方向）

以下是常见的 GraphRAG / LightRAG 方向的追问整理。
...
1. 《GraphRAG: Unlocking LLM discovery》— GraphRAG 概念首次公开
