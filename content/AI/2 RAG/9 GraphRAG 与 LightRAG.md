...
在 RAG 系统的实际应用中，常见的困惑是："检索到了正确信息，但生成的回答还是拼凑感很强，或者对某些类型的问题回答不好"。这背后反映了传统 RAG 的本质限制。
...
GraphRAG 和传统 RAG 的本质区别在于检索范式的转变。RAG 的演进路线清晰地展示了这一变化。
...
狭义上，它指的是**在 2024 年 4 月发布的那篇论文和配套的开源项目**。论文名字叫 **《From Local to Global: A Graph RAG Approach to Query-Focused Summarization》**（从局部到全局：一种面向查询聚焦摘要的图 RAG 方法），作者是 Darren Edge、Ha Trinh 等人。

广义上，它指的是**所有基于知识图谱做 RAG 的方法**，业界还有很多变种（比如 NebulaGraph、HippoRAG、阿里云的 AnalyticDB GraphRAG 等）。

咱们这篇文章**主要讲的是微软这套 GraphRAG**，因为它目前影响力最大，开源社区也最活跃，几乎是所有 GraphRAG 方案的参照对象。后面如果没特别说明，我说的 GraphRAG 都指这一套。
...
索引阶段，可以拆成 **5 个核心步骤**。咱们用一个具体的例子来理解整个过程，比如说你有一本《三国演义》的电子书，要把它喂进 GraphRAG。
...
**LightRAG 是由数据智能实验室的团队在 2024 年 10 月发布的**，核心作者是 Zirui Guo、Lianghao Xia、Yanhua Yu、Tu Ao、Chao Huang 等人。
...
他们在论文里总结了两个关键局限。一个是**现有基于图的 RAG 方法（主要就是 GraphRAG）缺乏动态更新能力**，很难高效处理新信息的加入，数据一变整个图谱就得大改；另一个是**全局查询的「暴力搜索」实在太低效**，GraphRAG 的 Global Search 是 Map-Reduce 遍历所有社区摘要的，在大规模语料上又慢又贵。
...
## 7. 技术深化与场景探讨（GraphRAG/LightRAG方向）

以下是 GraphRAG / LightRAG 方向的常见技术追问与探讨。
