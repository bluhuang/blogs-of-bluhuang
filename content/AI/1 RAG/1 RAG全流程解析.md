...
**注意**：能对比不同分片策略的优劣，并知道如何根据文档类型（技术文档、对话记录、法律条文）选择策略。
...
| ------------ | ---------- | ----------------- | -------- | --------------- |
| **某向量数据库 A**  | Go/C++     | HNSW, IVF, IVF_PQ | ✅        | 生产级大规模          |
| **某向量数据库 B** | Rust       | HNSW              | ✅        | 高性能/中等规模        |
| **某向量数据库 C** | Go         | HNSW              | ✅        | 混合检索（向量+关键词）    |
| **某向量数据库 D** | Python     | HNSW              | ❌        | 轻量原型            |
| **某向量检索库**    | C++/Python | 多种                | ❌（库）     | 研究/本地           |
| **某扩展模块**     | C          | IVFFlat, HNSW     | ✅（PG 集群） | 复用现有关系数据库 |
...
**常见问题**：什么时候用混合搜索优于纯向量？  
答案：当用户查询包含精确术语（产品型号、代码片段、人名地名）时，BM25 能精准匹配