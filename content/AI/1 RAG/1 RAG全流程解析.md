...
在对比不同分片策略的优劣时，需根据文档类型（技术文档、对话记录、法律条文）选择合适的策略。
...
**讨论**：什么时候用混合搜索优于纯向量？  
当用户查询包含精确术语（产品型号、代码片段、人名地名）时，BM25 能精准匹配；纯向量可能产生语义漂移。反之，对于意图模糊、语义相关的查询（如“如何提高客户满意度”），向量检索更好。混合搜索结合两者，是生产环境常见方案。
