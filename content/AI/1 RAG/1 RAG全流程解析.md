...
| 模型 | 维度 | 特点 | 最佳场景 |
|------|------|------|----------|
| 某商用英文模型（小） | 1536 | 通用性好，API 调用 | 通用英文 |
| 某商用英文模型（大） | 3072 | 精度更高，成本略高 | 高精度英文 |
| 某多语言商用模型 | 1024 | 支持多语言，压缩表示 | 多语言 |
...
| 数据库（类型描述） | 实现语言       | 索引类型              | 分布式      | 适用场景            |
| ------------ | ---------- | ----------------- | -------- | --------------- |
| 基于 Go/C++ 的数据库   | Go/C++     | HNSW, IVF, IVF_PQ | ✅        | 生产级大规模          |
| 基于 Rust 的数据库   | Rust       | HNSW              | ✅        | 高性能/中等规模        |
| 基于 Go 的数据库   | Go         | HNSW              | ✅        | 混合检索（向量+关键词）    |
| 轻量 Python 数据库   | Python     | HNSW              | ❌        | 轻量原型            |
| 本地研究库   | C++/Python | 多种                | ❌（库）     | 研究/本地           |
| PostgreSQL 向量扩展   | C          | IVFFlat, HNSW     | ✅（PG 集群） | 复用现有 PostgreSQL |
...
输入：`[CLS] 问题文本 [SEP] 文档文本 [SEP]`，经过 Transformer（如 BERT、RoBERTa）编码，取 `[