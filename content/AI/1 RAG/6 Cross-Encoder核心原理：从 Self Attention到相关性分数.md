...
Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。之所以不用 Cross‑Encoder 直接做检索，核心原因在于其计算复杂度为 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
| **某闭源Reranker模型** | 闭源 API | 精度领先，多语言，按量付费 | 最快落地，不折腾运维 |
...
选择 Rerank 模型时，需从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE，追求精度但不想自托管可选闭源 API，学习入门用 MiniLM。
