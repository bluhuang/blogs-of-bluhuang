...
> Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。其核心在于：Cross‑Encoder 的计算复杂度是 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
| **某商业 API 模型** | 闭源 API | 精度领先，多语言，按量付费 | 快速落地，免运维 |
| **ms‑marco‑MiniLM‑L‑6‑v2** | 22M | 超轻量，CPU 可跑，MIT 协议 | 英文通用，边缘部署，学习入门 |
| **4B 参数开源重排序模型** | 4B | 100+ 语言，Apache 2.0，32K 上下文 | 多语言 + 长文档 + 开源 |
...
Cross‑Encoder 将 Query 和 Document 分开编码，只在最后计算一次点积，交互非常浅；而 Cross‑Encoder 将两者拼接，Transformer 的每一层 Self‑Attention 都让 Query 的每个 token 与 Document 的每个 token 交互，能捕捉到否定词、语序、矛盾信息等深层语义关系。实验表明，Cross‑Encoder 重排序能使 nDCG@10 比纯向量检索提升 15–30%，显著减少 LLM 幻觉。
