...
> Cross‑Encoder 的目的不是“替换”检索，而是“补救”检索的粗糙排序。问“为什么不用 Cross‑Encoder 直接做检索”时，核心是：Cross‑Encoder 的计算复杂度是 O(|Q|×|D|)，无法在百万级文档上实时计算；必须用 Bi‑Encoder 先做粗筛。
...
> **选型建议**：选型时需从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE，追求精度但不想自托管选 Cohere API，学习入门用 MiniLM。
