...
Cross‑Encoder 的目的不是替换检索，而是补救检索的粗糙排序。为什么不直接用 Cross‑Encoder 做检索？因为其计算复杂度为 O(|Q|×|D|)，无法在百万级文档上实时计算，必须先用 Bi‑Encoder 做粗筛。
...
选择时关键在于速度与精度的权衡。能用 Bi‑Encoder 解决的场景不要用 Cross‑Encoder，能用规则解决的不要用模型。
...
| **闭源 Rerank API** | 闭源 API | 精度领先，多语言，按量付费 | 最快落地，不折腾运维 |
...
> **选型建议**：从**精度、延迟、成本、隐私**四个维度权衡。中文技术文档首选 BGE，追求精度但不想自托管选闭源 API，学习入门用 MiniLM。
...
### 8.1 Cross‑Encoder 比 Bi‑Encoder 准在哪里？
