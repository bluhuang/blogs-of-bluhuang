...
BM25 是一个基于词频和文档长度的排序函数，属于词法匹配（lexical matching）引擎。它根据查询词在文档中出现的频率打分，同时惩罚过于常见的词和过长的文档。它是 TF-IDF 的演化版本，但引入了词频饱和与文档长度归一化等优化。
...
BM25 的公式是典型的 IR 概率模型演进的产物。理解其结构即可——它不只是词频计数，而是考虑了**饱和度**和**文档长度**两个核心维度。
...
推荐 k1 在 1.2–2.0，b 取 0.75 左右，这是多年工程实践验证的通用配置。如果语料长度差异极大（比如混杂了几行代码片段和整本手册），可能需要根据实测调高 b 值，强化长度惩罚。
