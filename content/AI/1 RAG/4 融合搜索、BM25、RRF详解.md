...
如果没有平滑常数，排名第 1 的文档得分为 1，排名第 2 得分为 0.5，差距过大。k=60 使排名 1 得 1/61≈0.0164，排名 2 得 1/62≈0.0161，差距缩小。这样单一检索器的极端高分不会压过其他信号。相关研究（2009 年论文）验证了 60 在多数据集上的最优性。
...
- **用你的 NQ 或自定义业务测试集**，跑出量化结果。例如“混合检索使 Hit Rate@3 从 0.65 提升到 0.82”。
...
| **THULAC**   | 结构化感知机        | 快   | 高   | 来自清华大学，兼顾速度和精度     | 大规模文本处理            |
...
| **RRF 相比加权求和有什么优势？** | 不需要分数归一化，不需要人工调权重，只看排名位置，可