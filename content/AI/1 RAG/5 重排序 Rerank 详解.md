...
正确用法是**串联**而非替代——Bi‑Encoder 先快速召回候选集，Cross‑Encoder 再精细排序。可以用一个通俗的例子描述：“Bi‑Encoder 是海选，速度第一；Cross‑Encoder 是决赛，精度第一。”
...
| nv-rerankqa-mistral-4b-v3 | 4B | 512（每对） | 商业 | 问答召回精度高，Recall@5 可达 75.45% | 依赖特定部署环境 |

**数据来源**：综合整理自 FutureAGI（2026）、某技术社区（2026）及模型对比平台。
...
问“如何选择 Rerank 模型？”可从四个维度回答：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。再补充具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想折腾选某托管 API。
...
### 问题3：如何为项目选择 Rerank 模型？
从四个维度权衡：①精度；②延迟；③成本（API 按量 vs 自托管）；④语言与数据隐私。中文技术文档场景推荐 bge-reranker-v2-m3，追求精度且不想自托管选某托管 API。

### 问题4：Rerank 会增加多少延迟？怎么解决？
...
- 主流开源 Rerank 模型解析与选型指南（2026 版）（某技术社区）
