...
> 问“如何选择 Rerank 模型？”可从四个维度回答：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。再补充具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想折腾选对应的 API 服务。
...
**回答**：从四个维度权衡：①精度；②延迟；③成本（API 按量 vs 自托管）；④语言与数据隐私。中文技术文档场景推荐 bge-reranker-v2-m3，追求精度且不想自托管选对应的 API 服务。
...
- RAG 系列（十一）：Rerank——让检索结果按重要性排队（技术博客平台）
- RAG学习之-Rerank 技术详解（技术博客平台）
- Best Rerankers for RAG in 2026: 7 Models Compared（技术评测网站）
- 主流开源 Rerank 模型解析与选型指南（2026 版）（技术社区）
- Top 5 Reranking Models to Improve RAG Results（机器学习教程网站）
- Rerank 4 Fast vs BGE Reranker v2 M3（模型对比平台）
- RAG超越语义搜索：如何用Rerank模型实现检索精度的大幅提升（云服务技术文档）
- 大模型RAG进阶：检索排序Rerank的深度解析与实践（开发者技术中心）