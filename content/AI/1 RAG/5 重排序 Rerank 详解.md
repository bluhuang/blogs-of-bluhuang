...
| **bge-reranker-v2-m3** | 智源 | 567M | 8K | Apache 2.0 | 中文社区主流，量化后 <200MB，混合场景突出 | 约 80ms |
...
1. **理解原理**：精读 Bi‑Encoder 与 Cross‑Encoder 的区别
2. **模型调研**：对比主流 Rerank 模型的选型，明确自己场景的优先选项
3. **轻量实验**：选择轻量级模型（如 bge-reranker-v2-m3）本地跑通 Rerank 流程
4. **集成评估**：集成到现有 RAG 项目，用同一测试集对比有无 Rerank 的指标提升
5. **成本调优**：调整传入 Rerank 的候选数（如 50→30），测量精度与延迟的平衡点
