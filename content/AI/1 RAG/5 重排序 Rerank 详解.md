...
> 可以把 Rerank 理解为两个阶段——“初筛（海选）+精排（决赛）”。检索是“快手阿姨”，30 秒抓 10 个苹果（看外表）；Rerank 是“品果大师”，接过 10 个苹果仔细检查（摸一摸、闻一闻、掂一掂），告诉你哪 3 个最好吃。
...
| **NVIDIA nv-rerankqa-mistral-4b-v3** | 4B | 512（每对） | 商业 | 问答召回精度高，Recall@5 可达 75.45% | 依赖 NVIDIA NIM |
...
> 关于“如何选择 Rerank 模型？”可从四个维度分析：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。再补充具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想折腾选 Cohere API。
