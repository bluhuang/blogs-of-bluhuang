...
> 选择 Rerank 模型可从四个维度考量：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。补充具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想自己托管选 Cohere API。
...
### Bi‑Encoder 和 Cross‑Encoder 的核心区别？
...
Cross‑Encoder 逐对计算确实增加延迟。但工程上可通过**限制传入 Rerank 的候选文档