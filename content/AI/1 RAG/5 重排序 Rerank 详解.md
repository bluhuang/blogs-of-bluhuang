...
| **nv-rerankqa-mistral-4b-v3** | 4B | 512（每对） | 商业 | 问答召回精度高，Recall@5 可达 75.45% | 依赖特定硬件 |

**数据来源**：综合整理自未来技术评测平台（2026）及社区模型对比平台。
...
> 选择 Rerank 模型可从四个维度考虑：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想折腾选托管 API。
...
- Rerank 4 Fast vs BGE Reranker v2 M3
