...
正确用法是**串联**而非替代——Bi‑Encoder 先快速召回候选集，Cross‑Encoder 再精细排序。一个通俗的类比：“Bi‑Encoder 是海选，速度第一；Cross‑Encoder 是决赛，精度第一。”
...
选择 Rerank 模型时，可以从四个维度综合考量：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想自托管选 Cohere API。
...
Cross‑Encoder 逐对计算确实增加延迟。但工程上可通过**限制传入 Rerank 的候选文档数量**（