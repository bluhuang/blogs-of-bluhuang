...
选择 Rerank 模型时，可从四个维度考虑：①精度（看 nDCG/MRR）；②延迟（能否接受 200–500ms）；③成本（自托管 vs API 按量付费）；④语言/数据隐私（是否必须本地部署）。具体推荐逻辑：中文技术文档优先 BGE，追求精度但不想折腾选 Cohere API。
