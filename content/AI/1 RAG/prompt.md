...
我在探索一个 Agent 项目，并从原始代码库迁移了 RAG 部分代码。现在需要为 RAG 部分增加自动化评估能力，以便量化检索和生成质量，支撑后续优化。
...
下面是我的整体思路，请先判断思路是否正确，然后一起讨论具体实现细节。
...
## 二、实现计划
1. **测试集选择**：希望使用公开数据集，避免自行构造。最好能适配 RAGAS。不同数据集之间如何选择更合适？CRUD-RAG 如何？
1. **集成 Ragas**：安装 `ragas`, `datasets`, `langchain-openai` 等。评判 LLM 使用 DashScope（已有 API Key），通过 `ChatOpenAI(base_url="https://dashscope.aliyuncs.com/compatible-mode/v1")` 接入。嵌入模型暂时用 `OpenAIEmbeddings`（需要 OpenAI key）还是换用 DashScope 自己的嵌入？倾向于先使用 OpenAI 的小额度（或临时申请）以保证快速跑通，也可建议免费替代方案。
2. **自动化脚本结构**：
...
## 三、需要判断和实现的关键点
1. 上述思路是否可行？有无遗漏的重要步骤（例如是否需要处理异步？`rag_agent_service.query` 是同步的，无问题）？
2. 使用 DashScope 作为评判 LLM 时，Ragas 的 `evaluate()` 函数需要的 `llm` 参数如何构造？是否需要特殊的包装器？
3. 嵌入模型如果用 DashScope 而非 OpenAI，应如何配置？Ragas 是否支持？
4. 请根据项目结构（已提供关键服务类）生成一份完整的 `tests/evaluate.py` 代码。代码应：
   - 包含必要的 import 和异常处理。
...
请先确认思路，然后进入具体代码生成。如果某些细节需更多信息（例如配置参数名、模型名称等），请指出，会补充。现在请开始分析和建议。