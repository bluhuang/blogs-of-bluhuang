...
1. 一个可执行的脚本 `tests/evaluate.py`，运行后：
   - 加载测试集（例如公开数据集平台上的 `ragas-wikiqa` 或 RAGAs 内置的 `amnesty_qa`）。
...
## 二、实现计划
1. **测试集选择**：希望使用公开的数据集，这样不用自己构造。最好能适配 RAGAs。不同数据集之间选择哪个更合适？CRUD-RAG 如何？
1. **集成 Ragas**：安装 `ragas`, `datasets`, `langchain-openai` 等。评判 LLM 使用已有的 LLM 服务（通过 API 密钥接入），通过 `ChatOpenAI(base_url="<LLM服务端点>/compatible-mode/v1")` 接入。嵌入模型暂时用 `OpenAIEmbeddings` 还是换用 LLM 服务自身的嵌入？倾向于先使用一个可用的小额度以保证快速跑通，也可以建议免费替代方案。
2. **自动化脚本结构**：
...
2. 使用 LLM 服务作为评判 LLM 时，Ragas 的 `evaluate()` 函数需要的 `llm` 参数如何构造？是否需要特殊的包装器？
3. 嵌入模型如果用 LLM 服务而不是通用嵌入模型，应该如何配置？Ragas 是否支持？
