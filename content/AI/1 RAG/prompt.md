...
- RAG 生成服务：`app/services/rag_agent_service.py` 提供 `rag_agent_service.query(question)`，返回最终答案字符串。内部会调用上述检索服务并拼接 Prompt 后调用某 LLM API。
- 配置：`.env` 中有对应的 API Key。
...
## 二、实现计划
1. **测试集选择**：我希望使用公开的数据集，这样不用自己构造。最好能适配 RAGAS。不同的数据集之间如何选择更合适？CRUD-RAG 如何？
2. **集成 Ragas**：安装 `ragas`, `datasets`, `langchain-openai` 等。评判 LLM 使用已有的 LLM API（已有 API Key），通过 `ChatOpenAI(base_url="<API基础地址>")` 接入。嵌入模型暂时用 `OpenAIEmbeddings`（需要对应 key）还是换用某个嵌入服务？我倾向于先使用可用的小额度以保证快速跑通，也可以建议免费替代方案。
3. **自动化脚本结构**：
...
2. 使用现有 LLM API 作为评判 LLM 时，Ragas 的 `evaluate()` 函数需要的 `llm` 参数如何构造？是否需要特殊的包装器？
3. 嵌入模型如果使用其他服务而不是 OpenAI，应该如何配置？Ragas 是否支持？
