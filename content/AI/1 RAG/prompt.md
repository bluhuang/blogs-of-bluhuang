...
下面是我的整体思路，请你先判断思路是否正确，然后一起讨论具体实现细节。
...
## 二、实现计划
1. **测试集选择**：希望使用公开的数据集，这样不用自己构造。最好能适配 RAGs。需要考虑不同数据集之间的适用性，例如 CRUD-RAG 是否合适？
2. **集成 Ragas**：安装 `ragas`, `datasets`, `langchain-openai` 等。评判 LLM 使用已有 API Key 的大模型服务，通过 `ChatOpenAI(base_url="https://api.example.com/v1")` 接入。嵌入模型初步考虑使用开源或已有 API 的免费替代方案，以保证快速跑通。
3. **自动化脚本结构**：
...
## 三、需要分析和实现的细节
1. 上述思路是否可行？有无遗漏的重要步骤（例如是否需要处理异步？`rag_agent_service.query` 是同步的，没问题）？
2. 使用大模型作为评判 LLM 时，Ragas 的 `evaluate()` 函数需要的 `llm` 参数如何构造？是否需要特殊的包装器？
3. 嵌入模型如果使用其他 API 而非 OpenAI，应该如何配置？Ragas 是否支持？
...
请先确认思路，然后进入具体代码生成。如果某些细节需要更多信息（例如配置参数名、模型名称等），请指出，会补充。现在开始分析和建议。