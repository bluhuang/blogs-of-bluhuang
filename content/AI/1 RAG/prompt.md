...
- RAG 生成服务：`app/services/rag_agent_service.py` 提供 `rag_agent_service.query(question)`，返回最终答案字符串。内部会调用上述检索服务并拼接 Prompt 后调用某大模型 API。
...
1. **集成 Ragas**：安装 `ragas`, `datasets`, `langchain-openai` 等。评判 LLM 使用某大模型 API（已有 API Key），通过 `ChatOpenAI(base_url="https://api.llm-service.com/compatible-mode/v1")` 接入。嵌入模型暂时用 `OpenAIEmbeddings`（需要 OpenAI key）还是换用该服务自己的嵌入？我倾向于先使用 OpenAI 的小额度（或临时申请）以保证快速跑通，你也可以建议免费替代方案。
2. **自动化脚本结构**：
...
2. 使用某大模型 API 作为评判 LLM 时，Ragas 的 `evaluate()` 函数需要的 `llm` 参数如何构造？是否需要特殊的包装器？
3. 嵌入模型如果用该服务而不是 OpenAI，应该如何配置？Ragas 是否支持？
