...
上一章已经提到，一个工具 = 函数本体 + 名称 + 描述 + 参数定义，大模型靠工具描述来判断要不要调用这个工具。 
...
这就是 Function Calling 要解决的核心问题。在讲它是什么之前，我们先来看看没有它的时候，开发者需要面对哪些挑战。
...
比如想实现让 AI 能调用一个查天气的工具，System Prompt 里就得这样写：
...
所以 Tool 章节讲的「工具四要素」，是从概念层告诉你一个工具应该有什么；这章讲的 Function Calling，是从协议层告诉你这些要素以什么标准化格式在大模型和 Agent 之间流转。
...
这一步是开发者需要完成的，相当于给大模型一份严格规范的工具说明书，大模型会完全按照这份说明书，判断要不要调用工具、怎么调用工具。
...
当大模型判断需要调用工具时，会严格按照固定格式返回 JSON，不再有任何自由发挥的自然语言。Agent 直接解析这个结构，拿到工具名和参数，不用做任何额外的猜解处理。
