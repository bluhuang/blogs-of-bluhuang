...
让 AI 帮你整理电脑里的文件，它却给出了一套整理方案，分类规则写得头头是道，但最后还是得你自己手动一个个打开文件夹、移动文件；让它帮你查今天的天气，它却说“我的知识截止到某年某月，无法获取实时天气”；让它帮你订明天的机票，它只能告诉你去哪个网站、怎么操作，然后你默默打开了出行 APP。
...
执行 shell 命令、跑脚本、重启服务、部署代码……能直接操作系统。一条错误的 shell 命令可以删掉整个目录，一次错误部署能把线上服务打挂。这类工具的影响范围最大，破坏性最强。

**风险等级：高。** 两件事必须做到：严格限制权限范围（比如只允许执行白名单里的命令）；重要操作必须走人工审批，绝对不能允许 Agent 自主决策。
