...
让 AI 帮你整理电脑里的文件，它却给出了一套整理方案，分类规则详细，但最终仍需手动操作；让它查今天的天气，它回复“我的知识截止到某年某月，无法获取实时天气”；让它订明天的机票，它只能指导你去哪个网站、如何操作，然后你默默打开了订票平台。

这不是大模型不够聪明。大模型非常聪明， **但它天生就被限制在一个无法与外部世界交互的“小盒子”中**，只能接收文字、输出文字，它的“手”根本伸不出来，无法与外部世界产生任何交互。
...
很多同学以为“Agent 就是更厉害的大模型”。这个理解有偏差。 **Agent 本质上是开发者编写的一套程序**，它不是更聪明的 AI，而是一个全程在线的“调度员”，负责在用户、大模型、工具之间协调任务、传递信息、推进流程。
...
大模型只“动脑”，工具才“动手”。Agent 就是将两者打通的中间层。
