...
6. **实战示例（某终端编程工具）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
7. **核心价值**：Skills 让 AI 从“临时问一句”升级为“按标准化流程自动执行”，降低重复沟通成本，实现经验沉淀和团队协作。

***
### 第一阶段：AI 的“破壁人” ， Function Calling（函数调用）

回想早期的对话式 AI，它就像是一个 **被关在小黑屋里的超级学霸**：虽然上知天文下知地理，但它没网（没法查实时数据），也没手（没法帮你真正去执行操作，比如发邮件、查数据库）。
...
开发者 A 用 Python 写了个查数据库的工具，开发者 B 用 Java 搞了个读本地文件的工具。由于没有统一的标准，你的 AI 应用想要接入这些现成的工具，就得挨个去适配它们千奇百怪的数据格式和通信逻辑。这就像你买了个新手机，却发现大家用的充电线有圆头、扁头、方头，简直让人崩溃。
...
举个例子，假设你要让 AI 帮你写一份项目日报。你手里虽然已经有了 MCP 提供的“读取数据库”和“读取任务管理”的标准化工具，但你每次跟 AI 聊天，还是得像个唐僧一样反复叮嘱。
...
### 实战加餐：以某终端编程助手为例，看看 Skills 到底有多强？

光说不练假把式。以某款终端编程神器为例，它直接带火了 Skills 机制。下面就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。具体实现如下：
...
3. **怎么使用？** 配置好之后，在终端里只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：
