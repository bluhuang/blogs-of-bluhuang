...
### 实战加餐：以一个流行的终端编程工具为例，看看 Skills 到底有多强？

光说不练假把式。今年（2026年）某公司发布的终端编程神器，直接把 Skills 机制带火了。咱们就以它为例，看看真实项目里是怎么玩 Skills 的。
...
3. **怎么使用？** 配置好之后，你在工具终端里，只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：
