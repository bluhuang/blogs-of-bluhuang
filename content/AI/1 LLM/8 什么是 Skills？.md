...
6. **实战示例（终端编程工具）**：在项目 `.claude/skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
...
某位开发者用 Python 写了个查数据库的工具，另一位用 Java 搞了个读本地文件的工具。由于没有统一的标准，你的 AI 应用想要接入这些现成的工具，就得挨个去适配它们千奇百怪的数据格式和通信逻辑。这就像你买了个新手机，却发现大家用的充电线有圆头、扁头、方头，让人崩溃。
...
这个问题问得太好了！其实很多人刚接触时都会有这个错觉。来掰扯一下它俩到底是什么关系、有什么区别：
...
### 实战加餐：以最近爆火的终端编程工具为例，看看 Skills 到底有多强？

光说不练假把式。今年（2026年）发布的终端编程神器，直接把 Skills 机制带火了。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 之前说过，一个优秀的 Skill 分为“头部配置”和“技能主体”。在这个工具里，它是这么写的：
...
3. **怎么使用？** 配置好之后，在终端里，只需要像聊天一样发一句话（或者直接输入内置指令 `/image-optimizer` ）：
