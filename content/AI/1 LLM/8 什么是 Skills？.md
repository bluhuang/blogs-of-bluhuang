...
6. **实战示例（某终端工具）**：在项目 `.skills/` 目录下按结构放置 `SKILL.md` 和辅助脚本，AI 可通过触发技能自动执行多步骤任务（如图片压缩、转换格式、移动文件）。
...
### 实战加餐：以某终端工具为例，看看 Skills 到底有多强？

光说不练假把式。某团队发布的终端编程神器，直接把 Skills 机制带火了。咱们就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 咱们刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。在这个工具里，它是这么写的：
