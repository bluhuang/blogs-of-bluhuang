...
举个例子，假设你要让 AI 帮你写一份项目日报。你手里虽然已经有了 MCP 提供的“读取数据库”和“读取需求管理工具”的标准化工具，但你每次跟 AI 聊天，还是得像个唐僧一样反复叮嘱。
...
3. 提取任务进度（自动调用 mcp-issue-reader 获取任务状态）
4. 生成图表和摘要
...
```
...
光说不练假把式。近期某终端编程工具直接让 Skills 机制变得流行。就以它为例，看看真实项目里是怎么玩 Skills 的。
...
2. **SKILL.md 里面写了啥？** 刚才说了，一个优秀的 Skill 分为“头部配置”和“技能主体”。在该工具里，它是这么写的：
...
* **Skills（技能）：** 是上层的业务封装（新近的热点），它将 Prompt 指令和 MCP 工具组合在一起，按清晰的 **目录结构** 管理起来，变成了 AI 可以随拿随用、彻底告别重复劳动的“ **标准工作流**”。
