...
先回想一下早期的 ChatGPT。那时的它就像是一个 **被关在小黑屋里的超级学霸**：虽然上知天文下知地理，但它没网（没法查实时数据），也没手（没法真正去执行操作，比如发邮件、查数据库）。

为了让大模型能连接外部世界，**Function Calling** 诞生了。

用白话说，这就是 **大模型与外部世界打交道的一种“底层暗号”**。
...
开发者 A 用 Python 写了个查数据库的工具，开发者 B 用 Java 搞了个读本地文件的工具。由于没有统一的标准，你的 AI 应用想要接入这些现成的工具，就得挨个去适配它们千奇百怪的数据格式和通信逻辑。这就像你买了个新手机，却发现大家用的充电线有圆头、扁头、方头，让人抓狂。
...
并没有！随着让 AI 干的活越来越复杂，26 年初，一个极其痛点的问题暴露了出来：**每次让 AI 干活，沟通成本太高了！**
...
光说不练假把式。今年（2026年）某公司发布的终端编程工具 **Claude Code**，直接把 Skills 机制带火了。咱们就以它为例，看看真实项目里是怎么玩 Skills 的。
