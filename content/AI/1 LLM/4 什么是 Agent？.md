...
前两章讨论了大模型的底层逻辑，以及如何写好 Prompt。现在，你已经知道大模型是一个"超级大脑"，能够理解意图、进行逻辑推理、生成高质量内容。

然而，它只有表达能力，没有执行能力：它可以建议"你应该查看慢查询日志"，但它自己无法实际查询；它可以分析"这个 API 可能是瓶颈"，但它无法触发警报、关闭开关或打开任何文件。

掌握 Prompt 后，你可以精准控制大模型的输出，但 Prompt 无法解决一个问题：即使大模型给出了完美的分析，它依然无法主动去查数据库、发通知、调用 API。"说得再好"也只是停留在语言层面。
