...
大模型是用历史数据训练的，训练完之后它的知识就冻结了。比如某个模型的训练数据截止到某一时间点，之后发生的事情它一概不知。你去问它「我们系统今天凌晨发生了什么故障」，它根本没有这份信息，只能瞎猜。

**第二类：私有数据**

你们团队的内部文档、产品设计方案、运维手册、客服问答库，这些都是保密的，不可能出现在大模型的训练数据里。模型从没见过这些内容，自然没法基于它们来回答问题。
...
你们团队的内部文档可能有几百个文件，光是运维手册就几十万字，根本塞不进去。更别提那些有几千个历史工单的故障数据库了。
...
| 回答准确性    | 依赖训练数据，隐私领域容易瞎编 | 基于检索到的真实文档，有据可查     |
