...
1. **概念来源**：由一位业界工程师在 2026 年 2 月提出，随后通过官方博客推广。核心思想是“设计一套包裹模型的外部环境，让 Agent 稳定交付”。
2. **AI 工程重心的三次演进**：
...
Harness Engineering 这个词最早被正式叫响，是在 2026 年 2 月 5 号一篇博客里。作者是一位业界知名的工程师。
...
博客发出来一周后，另一家知名 AI 公司紧接着也发布了一篇官方博客，标题就叫《Harness engineering: leveraging Codex in an agent-first world》。
...
提示词没毛病、上下文也没毛病，可它就是做不稳。正是在这个背景下，那篇关于 AI 接纳历程的博客和那个百万行代码实验几乎同时出现，Harness Engineering 就这么被推到了台前。
...
这也顺便解释了一个很多人感到困惑的现象：同样是用那些主流大模型，为啥有的团队做出来的 Agent 又稳又能打，到别人手里却一跑就跑偏？
...
你平时用各种 AI 工具，看到的都是一个对话框，输入一句话，下面给你输出一段话。但如果把这层外壳扒掉，里面其实就一个东西：一个特别大的参数文件，存在硬盘上，加载到显卡的内存里。

![](什么是Harness工程？-4003575f13ade98b2eaefe5e82b1d352.png)

这个文件本身啥也不是。给它配一个 HTTP 接口，它就成了所谓的「大模型 API 服务」。给这个 API 套一个聊天界面，它就成了聊天助手；套一个代码编辑器，它就成了 AI IDE。
...
这就是为啥同样用同一个模型，你用不同的 AI IDE，效果会差挺多。模型是同一个，但每家的上下文工程策略不一样，所以最后的体验也不一样。
...
这里要回扣我们开头讲的某工程师和某公司的一个核心洞察： **Agent 的状态不应该放在上下文窗口里，而应该外化到文件系统**。
...
**某公司把他们的 Terminal Bench 成绩从 52.8 推到 66.5 分、直接从榜单 30 开外冲到前 5，靠的就是基于 Eval 集做 trace 回放和迭代**。

其他顶尖团队内部同样有大量的 Eval 基础设施。这不是锦上添花的可选项，是工程级 Agent 的必需品。
...
还记得开头那一章我们讲过某工程师的「复利效应」吗？他说 Harness 的核心是「每次 Agent 犯错，都把修复沉到环境里」。那个修复到底沉到哪儿？ **答案就是这六层的其中某一层**：
...
下一章我就来讲这个：抓 5 个真实的工程难题，看看各大公司是怎么解的。
...
更诡异的是，某公司（就是做 Devin 那家）在用某模型重做 Devin 的时候，观察到一个特别有意思的现象，他们把它叫做「 **上下文焦虑**」（Context Anxiety）。
...
但某公司在做 Codex 项目的时候，实践告诉他们： **工程师本能反应的这两招，其实都是错的方向**。
...
前面四个难题都还算比较抽象，这一个特别具体、特别接地气，是某公司团队在《Harness engineering》博客里亲口承认的一个「我们一开始做错了」的故事。

先说现象。当 Agent 负责写绝大多数代码之后，会发生一件很自然但也很糟糕的事： **Agent 会疯狂模仿仓库里已有的代码模式**。

好的模式会被复制，坏的模式也会被复制。一旦早期某段代码写歪了，Agent 会把那个歪的写法当成「惯例」，越堆越多，越堆越歪，最后整个代码库开始「腐烂」。这个现象某公司团队给它起了一个很扎心的名字，叫 **AI slop**，AI 代码泔水。

![](什么是Harness工程？-10088e18ec0701bb615c1a68b8cccdb4.png)

某公司团队一开始是怎么解决这个问题的？用最朴素的办法： **靠人工清理**。
...
你品品这个做法。它其实把「还技术债」这件事从 **一周一次的人工集中清扫**，变成了 **每天持续进行的自动偿还**。他们原文里有一句话我特别喜欢：
...
某公司 Codex 团队那几个工程师能撬动百万行代码，靠的就不是键盘敲得快，而是把 Agent 的运行环境设计到位。
...
7. 《Rebuilding Devin for Claude Sonnet 4.5: Lessons and Challenges》,