...
1. **概念来源**：由某位行业专家在 2026 年 2 月提出，随后某知名AI公司官方博客推广。核心思想是“设计一套包裹模型的外部环境，让 Agent 稳定交付”。
2. **AI 工程重心的三次演进**：
...
Harness Engineering 这个词最早被正式叫响，是在 2026 年 2 月 5 号一篇博客里。作者是某位资深工程师。
...
博客发出来一周后，某知名AI公司紧接着也发布了一篇官方博客，标题就叫《Harness engineering: leveraging Codex in an agent-first world》。
...
提示词没毛病、上下文也没毛病，可它就是做不稳。正是在这个背景下，那篇工程师的博客和那个百万行代码实验几乎同时出现，Harness Engineering 就这么被推到了台前。
...
这也顺便解释了一个很多人感到困惑的现象：同样是用一些主流大模型，为啥有的团队做出来的 Agent 又稳又能打，到别人手里却一跑就跑偏？
...
你平时用各种对话模型也好，用代码助手也好，看到的都是一个对话框，输入一句话，下面给你输出一段话。但如果把这层外壳扒掉，里面其实就一个东西：一个特别大的参数文件，存在硬盘上，加载到显卡的内存里。
...
这就是为啥同样用某个模型，你用不同的 AI IDE 产品，效果会差挺多。模型是同一个，但每家的上下文工程策略不一样，所以最后的体验也不一样。
...
**你塞给它越多无关信息，它的注意力就越散**。这个现象某家公司在他们的博客里专门命名了，叫「context rot」（上下文腐化）。他们给的解法是「just-in-time retrieval」，也就是让 Agent 边干活边按需抓信息，而不是一上来就把所有可能有用的东西一股脑塞进去。
...
这里要回扣我们开头讲的某位工程师和某家公司的一个核心洞察： **Agent 的状态不应该放在上下文窗口里，而应该外化到文件系统**。

某家公司团队在《Effective harnesses for long-running agents》里给出的具体做法是让 Agent 维护一个 `claude-progress.txt` 日志、一个 `init.sh` 启动脚本、加上完整的 git history，作为「长期记忆介质」。下一轮换一个全新的、干净的上下文窗口接手时，从这些文件里一读，立刻就知道「现在到哪一步了」。
...
**某知名框架团队把他们的评测成绩从 52.8 推到 66.5 分、直接从榜单 30 开外冲到前 5，靠的就是基于 Eval 集做 trace 回放和迭代**。

某几家头部公司内部同样有大量的 Eval 基础设施。这不是锦上添花的可选项，是工程级 Agent 的必需品。

有了尺子还不够，你还得看到 Agent 每一次的 **真实足迹**，也就是它每一步做了什么决策、调了哪个工具、拿到什么返回、花了多少 token。

这就是一些 trace 系统存在的意义。能看到 trace，你才能定位失败那一步发生了什么，才能往 Harness 里补上对应的修复。
...
对 PR Review Agent 来说，约束可以包括：「一次最多分析 20 个 PR」「不能对已 closed 的 PR 再评论」「不能直接修改 PR 本身」「token 用量超过 10 万就立刻停下」。这些约束最好 **硬编码到代码或 linter 规则里**，而不是写在提示词里靠 Agent 自己遵守。某公司团队在早期项目里把资深工程师的经验固化成他们叫做「Golden Principles」的机制（我们在下一章会专门展开讲），就是这种思路的极致版。
...
更诡异的是，某家做 AI 编程工具的公司（就是做 Devin 那家）在用 Claude Sonnet 4.5 重做 Devin 的时候，观察到一个特别有意思的现象，他们把它叫做「 **上下文焦虑**」（Context Anxiety）。
...
这个思路对不对？对，但某家公司在另一篇博客里挑明了一个更扎心的观察： **光压缩根本不够**。

他们确认了 Sonnet 4.5 确实存在前面说的那种「上下文焦虑」倾向，一旦这种状态上来了，只压缩历史、不把整个上下文窗口彻底换掉，那种「已经累了」的负担感模型还是带着，Agent 还是会在长链路任务里慢慢失焦。

真正解开这个结的关键动作，这家公司把这个做法叫做 **Context Reset**：直接把旧的上下文窗口整个丢掉，换一个干净的接手。

那 Context Reset 具体怎么落地？

这家公司在他们的另一篇博客里给出了一套具体做法： **让 Agent 跨多轮接力跑，状态全部外化到文件系统**。
...
这家公司在博客里为了好讲，把这两种 prompt 启动的 Agent 分别叫 initializer agent 和 coding agent，但他们也在脚注里专门澄清了： **这其实是同一个 Agent，只是首轮和后续轮次用了不同的 user prompt 启动而已**。真正被「换掉」的不是 Agent，是上下文窗口。
...
某公司团队后来想明白了一件事：让干活的和验收的，必须是不同的人。
...
但某知名AI公司在做早期项目的时候，实践告诉我们： **工程师本能反应的这两招，其实都是错的方向**。
...
而这家公司的做法是： **给 Agent 接上 lint、单测、运行环境，让它自己写完自己跑，看见 bug 自己改**。同样一个问题，前者是在求模型发挥，后者是在改造环境，彻底决定了 Agent 下次会不会再犯。
...
这个是某知名AI公司自己亲自踩过的坑，说出来挺打脸的。

这家公司早期做项目的时候，搞了一个特别大的 AGENTS.md 文件，把所有的规范、所有的约定、所有的最佳实践全部塞进去。他们当时的想法是：把规则写得越全越好，Agent 就越不会出错。
...
这家公司后来怎么改的？他们把 AGENTS.md 从一本「百科全书」改成了一个「目录页」：

* **主文件只保留 100 行左右的核心索引**。你没看错，该公司原文博客里特地写明了这个数字：整个 AGENTS.md 控制在大约 100 行，它不告诉 Agent 每条细节规则，只告诉 Agent「你想看什么，去哪儿看」
...
前面四个难题都还算比较抽象，这一个特别具体、特别接地气，是某知名AI公司团队在博客里亲口承认的一个「我们一开始做错了」的故事。
...
这家公司的团队一开始是怎么解决这个问题的？用最朴素的办法： **靠人工清理**。
...
讲完五个难题，我想再顺手补一个某公司博客里特别反直觉的小观察，虽然它不算难题，但值得你记住。

很多人做 AI 编程的时候有一个想当然的认知：AI 是最前沿的东西，那当然应该配上最前沿的技术栈，什么新框架都给 Agent 上一套。但实践发现， **事情正好反过来**：

Agent 对那些被人类称为「boring」的老技术反而掌握得最好。

为啥？该公司原文给了三个原因： **组合性好、API 稳定、训练数据里出现得多**。
...
所以这家公司在项目里，甚至会 **主动选择那些看起来很土的、甚至业界已经有点嫌弃的技术栈**，就因为 Agent 能把它们用得更稳。
...
1. 某资深工程师，《My AI Adoption Journey》
2. 某知名AI公司，《Harness engineering: leveraging Codex in an agent-first world》
3. 某知名AI公司，《Effective harnesses for long-running agents》
4. 某知名AI公司，《Harness design for long-running application development》
5. 某知名AI公司，《Effective context engineering for AI agents》
6. 某知名框架团队，《Improving Deep Agents with harness engineering》
7. 某AI编程工具公司，《Rebuilding Devin for Claude Sonnet 4.5: Lessons and Challenges》