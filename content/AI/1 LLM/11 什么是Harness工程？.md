...
1. **概念来源**：由一位资深工程师在 2026 年 2 月提出，随后某AI研究机构官方博客推广。核心思想是“设计一套包裹模型的外部环境，让 Agent 稳定交付”。
2. **AI 工程重心的三次演进**：
...
你平时用各类聊天工具也好，用各种编程助手也好，看到的都是一个对话框，输入一句话，下面给你输出一段话。但如果把这层外壳扒掉，里面其实就一个东西：一个特别大的参数文件，存在硬盘上，加载到显卡的内存里。

![](什么是Harness工程？-4003575f13ade98b2eaefe5e82b1d352.png)

这个文件本身啥也不是。给它配一个 HTTP 接口，它就成了所谓的「大模型 API 服务」。给这个 API 套一个聊天界面，它就成了各种聊天应用；套一个代码编辑器，它就成了各种 AI IDE。
...
讲到这儿，我想专门聊一下某家公司最近搞的 **Agent Skills**，因为它特别能体现「上下文工程不是塞得越多越好」这个理念。
...
如果你去看几家顶尖团队的产品，会发现一件挺有意思的事：他们的产品形态不一样、技术栈不一样、服务对象也不一样， **但你把他们的 Harness 掀开看内部结构，里面的组件居然惊人地相似**。
...
**你塞给它越多无关信息，它的注意力就越散**。这个现象某AI公司在他们的博客里专门命名了，叫「context rot」（上下文腐化）。他们给的解法是「just-in-time retrieval」，也就是让 Agent 边干活边按需抓信息，而不是一上来就把所有可能有用的东西一股脑塞进去。
...
接上工具之后，PR Review Agent 才真正活过来。但工具不是接得越多越好。某团队在早期踩过这个坑：他们一开始给 Agent 接了一堆工具，想着「选择多总是好的」，结果 Agent 频繁用错工具、用错时机。后来砍掉一大半，效果反而上去了。
...
某AI公司在博客里给出的具体做法是让 Agent 维护一个 `claude-progress.txt` 日志、一个 `init.sh` 启动脚本、加上完整的 git history，作为「长期记忆介质」。下一轮换一个全新的、干净的上下文窗口接手时，从这些文件里一读，立刻就知道「现在到哪一步了」。
...
**某个框架团队把他们的 Terminal Bench 成绩大幅提升、直接从榜单 30 开外冲到前 5，靠的就是基于 Eval 集做 trace 回放和迭代**。

多家大厂内部同样有大量的 Eval 基础设施。这不是锦上添花的可选项，是工程级 Agent 的必需品。
...
对 PR Review Agent 来说，约束可以包括：「一次最多分析 20 个 PR」「不能对已 closed 的 PR 再评论」「不能直接修改 PR 本身」「token 用量超过 10 万就立刻停下」。这些约束最好 **硬编码到代码或 linter 规则里**，而不是写在提示词里靠 Agent 自己遵守。某团队在项目中把资深工程师的经验固化成他们叫做「Golden Principles」的机制（下一章会专门展开讲），就是这种思路的极致版。
...
更诡异的是，某AI创业公司（做 Devin 那家）在用最新模型重做 Devin 的时候，观察到一个特别有意思的现象，他们把它叫做「 **上下文焦虑**」（Context Anxiety）。
...
这个思路对不对？对，但另一家AI公司在博客里挑明了一个更扎心的观察： **光压缩根本不够**。
...
他们在博客里为了好讲，把这两种 prompt 启动的 Agent 分别叫 initializer agent 和 coding agent，但他们也在脚注里专门澄清了： **这其实是同一个 Agent，只是首轮和后续轮次用了不同的 user prompt 启动而已**。真正被「换掉」的不是 Agent，是上下文窗口。
...
但某团队在 Codex 项目的时候，实践告诉我们： **工程师本能反应的这两招，其实都是错的方向**。

他们干了一件在传统程序员看来非常离谱的事：在那个百万行代码的项目里， **人类工程师几乎不写一行代码，全部由 Agent 来写**。
...
而他们的做法是： **给 Agent 接上 lint、单测、运行环境，让它自己写完自己跑，看见 bug 自己改**。同样一个问题，前者是在求模型发挥，后者是在改造环境，彻底决定了 Agent 下次会不会再犯。

![](什么是Harness工程？-3391bb8d08c6d408bdb54d156c66e24d.png)

所以你看，在 Agent 主导的工程体系里，工程师的价值不再是「我一天能写多少行代码」，而是「我能为 Agent 设计多好的一套运行环境」。这才是未来程序员真正的技术含量所在。
...
他们早期做 Codex 的时候，搞了一个特别大的 AGENTS.md 文件，把所有的规范、所有的约定、所有的最佳实践全部塞进去。他们当时的想法是：把规则写得越全越好，Agent 就越不会出错。
...
他们后来怎么改的？他们把 AGENTS.md 从一本「百科全书」改成了一个「目录页」：
...
前面四个难题都还算比较抽象，这一个特别具体、特别接地气，是某团队在博客里亲口承认的一个「我们一开始做错了」的故事。

先说现象。当 Agent 负责写绝大多数代码之后，会发生一件很自然但也很糟糕的事： **Agent 会疯狂模仿仓库里已有的代码模式**。

好的模式会被复制，坏的模式也会被复制。一旦早期某段代码写歪了，Agent 会把那个歪的写法当成「惯例」，越堆越多，越堆越歪，最后整个代码库开始「腐烂」。这个现象他们团队给它起了一个很扎心的名字，叫 **AI slop**，AI 代码泔水。

![](什么是Harness工程？-10088e18ec0701bb615c1a68b8cccdb4.png)

他们团队一开始是怎么解决这个问题的？用最朴素的办法： **靠人工清理**。
...
很多人做 AI 编程的时候有一个想当然的认知：AI 是最前沿的东西，那当然应该配上最前沿的技术栈，什么新框架都给 Agent 上一套。但在实践中发现， **事情正好反过来**：

Agent 对那些被人类称为「boring」的老技术反而掌握得最好。

为啥？原文给了三个原因： **组合性好、API 稳定、训练数据里出现得多**。
...
所以他们在项目中，甚至会 **主动选择那些看起来很土的、甚至业界已经有点嫌弃的技术栈**，就因为 Agent 能把它们用得更稳。
...
1. 某资深工程师博客，《My AI Adoption Journey》, <https://example.com/blog1>
2. 某AI研究机构博客，《Harness engineering: leveraging Codex in an agent-first world>, <https://example.com/blog2>
3. 另一家AI公司博客，《Effective harnesses for long-running agents>, <https://example.com/blog3>
4. 另一家AI公司博客，《Harness design for long-running application development>, <https://example.com/blog4>
5. 另一家AI公司博客，《Effective context engineering for AI agents>, <https://example.com/blog5>
6. 某框架团队博客，《Improving Deep Agents with harness engineering>, <https://example.com/blog6>
7. 某AI创业公司博客，《Rebuilding Devin for the latest model: Lessons and Challenges>, <https://example.com/blog7>