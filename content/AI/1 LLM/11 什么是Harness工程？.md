...
1. **概念来源**：由一位工程师在 2026 年 2 月提出，随后某AI机构官方博客推广。核心思想是“设计一套包裹模型的外部环境，让 Agent 稳定交付”。
2. **AI 工程重心的三次演进**：
...
博客发出来一周后，某AI机构紧接着也发布了一篇官方博客，标题就叫《Harness engineering: leveraging Codex in an agent-first world》。
...
这就是为啥同样用 Claude 模型，你用 Claude Code、Cursor、Trae 这些不同的 AI IDE，效果会差挺多。模型是同一个，但每家的上下文工程策略不一样，所以最后的体验也不一样。

讲到这儿，我想专门聊一下某AI公司最近搞的 **Agent Skills**，因为它特别能体现「上下文工程不是塞得越多越好」这个理念。
...
假设你是一个团队负责人，要派一个刚入职的新人去做一次很重要的客户拜访。
...
如果你去看各大做 Agent 的顶级团队，会发现一件挺有意思的事：他们的产品形态不一样、技术栈不一样、服务对象也不一样， **但你把他们的 Harness 掀开看内部结构，里面的组件居然惊人地相似**。
...
**你塞给它越多无关信息，它的注意力就越散**。这个现象某AI公司在其博客里专门命名了，叫「context rot」（上下文腐化）。他们给的解法是「just-in-time retrieval」，也就是让 Agent 边干活边按需抓信息，而不是一上来就把所有可能有用的东西一股脑塞进去。
...
这里要回扣我们开头讲的那位工程师和某AI公司的一个核心洞察： **Agent 的状态不应该放在上下文窗口里，而应该外化到文件系统**。

某AI公司在《Effective harnesses for long-running agents》里给出的具体做法是让 Agent 维护一个 `claude-progress.txt` 日志、一个 `init.sh` 启动脚本、加上完整的 git history，作为「长期记忆介质」。下一轮换一个全新的、干净的上下文窗口接手时，从这些文件里一读，立刻就知道「现在到哪一步了」。
...
**某个框架团队把他们的 Terminal Bench 成绩从 52.8 推到 66.5 分、直接从榜单 30 开外冲到前 5，靠的就是基于 Eval 集做 trace 回放和迭代**。

各大AI公司内部同样有大量的 Eval 基础设施。这不是锦上添花的可选项，是工程级 Agent 的必需品。
...
但你肯定还有疑问：这些东西听起来都对，但各大团队真的是这么做的吗？具体是怎么落地的？

下一章我就来讲这个：抓 5 个真实的工程难题，看看这些团队是怎么解的。

## Harness 有什么难点？

讲完六层能力的骨架，你可能会觉得这套东西好像挺有体系的。但真要动手做，各大公司都会告诉你一个共同的感受： **概念清晰是一回事，落地是另一回事**。

下面我挑 5 个在真实项目里踩过的、特别典型的坑给你看看。

你会发现 Harness 真正的难度根本不在蓝图，而在这些具体的细节里。每个难题我都会用同一种结构来讲：先看现象是啥，再看这些团队是怎么反常识地解的，最后提炼出一条你可以直接记住的原则。
...
更诡异的是，某AI创业公司在用某个模型重做其AI编程助手的时候，观察到一个特别有意思的现象，他们把它叫做「 **上下文焦虑**」（Context Anxiety）。
...
这个思路对不对？对，但某AI公司在另一篇《Harness design for long-running application development》博客里挑明了一个更扎心的观察： **光压缩根本不够**。

他们确认了某个模型确实存在前面说的那种「上下文焦虑」倾向，一旦这种状态上来了，只压缩历史、不把整个上下文窗口彻底换掉，那种「已经累了」的负担感模型还是带着，Agent 还是会在长链路任务里慢慢失焦。

真正解开这个结的关键动作，某AI公司把这个做法叫做 **Context Reset**：直接把旧的上下文窗口整个丢掉，换一个干净的接手。

那 Context Reset 具体怎么落地？

某AI公司在他们的另一篇《Effective harnesses for long-running agents》博客里给出了一套具体做法： **让 Agent 跨多轮接力跑，状态全部外化到文件系统**。
...
某AI公司在博客里为了好讲，把这两种 prompt 启动的 Agent 分别叫 initializer agent 和 coding agent，但他们也在脚注里专门澄清了： **这其实是同一个 Agent，只是首轮和后续轮次用了不同的 user prompt 启动而已**。真正被「换掉」的不是 Agent，是上下文窗口。
...
某AI公司后来想明白了一件事：让干活的和验收的，必须是不同的人。
...
那他们最后是怎么改的？他们用一个非常 Harness 的思路解决了这个问题。做法分两步：
...
很多人做 AI 编程的时候有一个想当然的认知：AI 是最前沿的东西，那当然应该配上最前沿的技术栈，什么新框架都给 Agent 上一套。但他们在实践中发现， **事情正好反过来**：
...
这五条原则贯穿了各大团队所有的实践，也回答了「Harness 真正难在哪」。
...
3) 某AI公司，《Effective harnesses for long-running agents》, <https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents>

4) 某AI公司，《Harness design for long-running application development》, <https://www.anthropic.com/engineering/harness-design-long-running-apps>

5. 某AI公司，《Effective context engineering for AI agents》, <https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents>

6. 某个Agent框架团队，《Improving Deep Agents with harness engineering》, <https://blog.langchain.com/improving-deep-agents-with-harness-engineering/>

7) 某AI创业公司，《Rebuilding Devin for Claude Sonnet 4.5: Lessons and Challenges》, <https://cognition.ai/blog/devin-sonnet-4-5-lessons-and-challenges>