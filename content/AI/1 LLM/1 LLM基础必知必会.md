...
- **Chat 模型（对话模型）**  
  ...
  **补充解释：**
  - **有监督微调（SFT）**：使用标注数据调整模型，数据形式为“（用户指令，正确的回复）”。模型在预训练（Base模型）已学会语言的基础上，通过对比自身输出与标准答案的差异，逐步调整参数，从而对新指令也能生成类似风格的回复。可理解为“模仿老师给出的标准答案”。
  - **强化学习（RLHF，Reinforcement Learning from Human Feedback）**：在 SFT 之后，模型仍可能生成不安全或不理想的回复。RLHF 引入一个由人类对大量回复的偏好打分训练而成的“奖励模型”。LLM 通过生成回复并根据奖励模型的分数进行优化，学习更符合人类偏好的行为（如更有用、更无害）。可理解为“通过奖惩机制让模型自己摸索出更好的回答策略”。
...
**什么是模型推理？**  
模型推理是指利用**已经训练好**的模型，根据输入（如提示词）来生成新内容的过程。在这个阶段，模型参数不再更新，只进行前向计算。可理解为“模型在回答问题时的工作状态”。
