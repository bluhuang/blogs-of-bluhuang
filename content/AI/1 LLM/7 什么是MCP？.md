...
3. **三大核心角色**：
   - **Host**：用户使用的 AI 应用（如某桌面应用、VS Code），负责发起任务。
...
- **只有 MCP，没有 Function Calling**：  
    Host 能通过 MCP 调用工具，但大模型输出的工具名和参数可能不一致（比如“用 check_the_weather 工具，城市=上海明天”），Host 没法可靠地转换成 MCP 请求。
...
上一章讲完了 Function Calling，大模型通过标准化的 JSON 格式，告诉 Agent 要调哪个工具、传什么参数。到这里，已经理解了工具是什么、工具怎么被调用起来。

但实际开发 Agent 的时候，很快会遇到下一个麻烦： **工具集成本身，是个无底洞。** 
...
光这四件事，就够忙好几天。更麻烦的是，这些集成代码只能用在这个项目里，团队里其他同学做类似的 Agent，还得重新来一遍。

如果 Agent 还需要同时支持多个大模型（今天接某模型、明天接另一模型、后天接另一个），问题就更大了：

**10 个工具 × 5 个大模型 = 50 套集成代码**

不同模型的 Function Calling 格式还可能有细微差别，需要针对每个模型写适配层。哪天 GitHub API 升级了，50 套代码一个个去改，这还是基础设施代码，不是真正做产品该花时间的地方。
...
Host 就是最终使用的那个 AI 应用，可以是某桌面应用、带 AI 功能的 VS Code、或者自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
学到这里，很多读者脑子里会冒出一个问题：上一章学了 Function Calling，说大模型通过 Function Calling 告诉 Agent 调哪个工具；这章又学了 MCP，说 Agent 通过 MCP 来调用工具。这两个东西，到底有什么区别？MCP 是不是把 Function Calling 给替代了？
...
AI 调用 Tool 的机制，底层就是 Function Calling，MCP 在这之上做了一层标准化封装，让开发者不需要手写每个 Tool 的 Function Calling 定义，MCP Server 会自动按标准格式对外声明工具清单。
...
1. **用户提问**：在 Host（比如某桌面应用）里输入「帮我查一下 React 仓库最近的 commit」
...
* **RAG**：解决「大模型看不到私有数据」的问题，把知识库检索封装成工具，让 Agent 能随时访问私有知识，是 Agent 开发中最常见的能力之一