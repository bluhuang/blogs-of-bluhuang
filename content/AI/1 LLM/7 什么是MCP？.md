...
1. **MCP 是什么**：由某AI研究机构推出的开放标准协议（Model Context Protocol），定义了 AI 应用与工具服务之间的标准化通信方式，被誉为“AI 工具世界的 USB‑C”。
...
3. **三大核心角色**：
   ...
   - **Server**：暴露工具能力的轻量级服务程序，负责实际执行工具。
4. **MCP Server 的三种能力**：
...
- **只有MCP，没有Function Calling**：  
    Host能通过MCP调用工具，但大模型输出的工具名和参数可能不一致（比如“用check_the_weather工具，城市=上海明天”），Host没法可靠地转换成MCP请求。
...
但在实际开发 Agent 时，很快会遇到另一个难题：**工具集成本身，是个无底洞。**

***

## 没有 MCP 之前：重复造轮子的困境

假设正在开发一个 Agent，需要它能读取 GitHub 代码仓库、查询公司数据库、操作本地文件系统、还能发 Slack 消息。
...
光这四件事，就足够花费好几天。更麻烦的是，这些集成代码只能在这个项目中使用，团队里其他成员做类似的 Agent，还得重新来一遍。

如果 Agent 还需要同时支持多个大模型（今天接 Claude、明天接 GPT-4、后天接 Qwen），问题就更大了：

**10 个工具 × 5 个大模型 = 50 套集成代码**

不同模型的 Function Calling 格式还可能有细微差别，需要针对每个模型写适配层。如果某天 GitHub API 升级，50 套代码都要逐个修改，而这些是基础设施代码，并不是真正做产品应该花时间的地方。
...
这就是某AI研究机构在 2024 年 11 月推出 **MCP（Model Context Protocol，模型上下文协议）** 的背景。

MCP 要解决的核心问题，可以用一句话概括： **把工具的「写好」和「用起来」彻底拆开。**

![](什么是MCP？-d3a0c94d2277a61d5dbb04adf28824d7.png)

举一个生活中的例子：在 USB-C 统一之前，各家手机厂商各有各的充电口，苹果用 Lightning、安卓早期用 Micro-USB，各品牌之间完全不兼容，出门得带三四根不同的充电线。USB-C 出来之后，只要设备支持 USB-C，任何 USB-C 的线都能用，充电器、数据线全部通用。

**MCP 对于 AI 工具世界的意义，就和 USB-C 对于充电口的意义一模一样。**

![](什么是MCP？-b356960ae62209c7aff10d731b204617.png)
...
![](什么是MCP？-03570e6969812c513243d5e9e0d1bb44.png)

### Host（宿主）

Host 就是最终使用的 AI 应用，可以是 Claude Desktop、带 AI 功能的 VS Code、或者自己开发的 Agent 程序。Host 是整个交互的起点，用户在 Host 里提问，Host 决定要调用哪些工具来完成任务。
...
![](什么是MCP？-646507fd5709adf52c70db42fff09419.png)
...
学到这里，很多同学会产生一个疑问：上一章学了 Function Calling，说大模型通过 Function Calling 告诉 Agent 调哪个工具；这章又学了 MCP，说 Agent 通过 MCP 来调用工具。这两个东西，到底有什么区别？MCP 是不是把 Function Calling 给替代了？
...
![](什么是MCP？-a3ce8c4020e14607885212500f44c540.png)
...
AI 调用 Tool 的机制，底层就是 Function Calling，MCP 在这之上做了一层标准化封装，使得不需要手写每个 Tool 的 Function Calling 定义，MCP Server 会自动按标准格式对外声明工具清单。
...
Prompts 是预定义的可复用提示词模板。当有一些常用的、固定结构的提示词（比如「代码 Review 模板」「会议纪要生成模板」），可以将其封装成 MCP Prompts，在不同 Agent 项目里直接复用，不用每次重新写。
...
* **MCP 是什么**：由某AI研究机构推出的开放标准协议，定义了 AI 应用和工具服务之间如何标准化通信，是 AI 工具世界的「USB-C」。
