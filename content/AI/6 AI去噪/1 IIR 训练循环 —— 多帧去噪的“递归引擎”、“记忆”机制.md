...
在代码里，IIR 有两种常见的实现方式，**无人机的实现**和**小模型的实现**正好代表了这两种。
...
> **IIR 是一种“记忆传递”机制**：每一帧的去噪结果都会作为下一帧的参考（参考帧），让模型能利用前后帧的信息更准确地区分噪声和真实纹理。无人机的实现是“简单递归”（上一帧输出直接传），小模型的实现是“背景管理”（累积背景 + 运动检测）。**你迁移的核心任务，就是把无人机“简单递归”的模型调用方式，改成小模型“背景管理”的调用方式，并让模型能用 3 个参数接收输入。**